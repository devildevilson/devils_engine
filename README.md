...
`subprojects/tile_frontier`, стоит разобрать отдельным документом позже.
...
actor topology пока живут в `subprojects/tile_frontier`, а не в `libs/simul`.
...
проверяется в `subprojects/tile_frontier` через broker на `utils/thread`.
...
- Главная интеграционная проверка сейчас живет в `subprojects/tile_frontier`.
