...
Для `RND-31` готов отдельный design proposal: проект сначала выбирает graph recipe/набор технологий, затем
независимые настройки (`ao/shadows/dof/...`) разрешаются в runtime data и build patches; общий quality preset
лишь массово записывает эти choices, а concrete variant является результатом resolver:
...
| `RND-44` | Depth of field | `M` | **СДЕЛАНО в `PF03` 2026-08-20.** Размытие делает ОБЪЕКТИВ, поэтому пасс стоит после накопления и ДО замера экспозиции: сенсор видит уже размытый свет, значит и замер, и пирамида bloom работают по размытому кадру. Passthrough при `--dof=0` ПОБИТОВЫЙ. CoC считается по формуле тонкой линзы (`A = f/N`, `coc = A*f*|D-S|/(D*(S-f))`, в пиксели через высоту сенсора), а не как «|глубина − фокус| × сила» — у физической формулы есть ЕДИНИЦЫ, значит есть и проверяемые следствия, и все три измерены: обратная пропорциональность диафрагме (средний CoC 4.200/8.367/16.706 при f/8-f/4-f/2, то есть ровно удвоение на стоп с расхождением 0.4%); у ЗАДНЕГО плана размытие ОГРАНИЧЕНО пределом `A*f/(S-f)` — измерено 21.58/10.79/5.52 px при аналитических 21.37/10.55/5.24 для фокуса 2/4/8 м (точность в один шаг квантования вида); у ПЕРЕДНЕГО предела нет — при отодвигании фокуса 8/12/20 м задний максимум падает 5.52/3.76/2.26, а передний растёт 6.53/8.28/9.79. Из этой асимметрии и следует устройство эффекта. Сбор — рассеяние наизнанку (`r <= |CoC(пробы)|`, а не центра), аккумуляторов два: задний план обязан уважать силуэты (без проверки фон протекает на резкий передний план — измерено ручкой: 24.3% кадра, RMSE 3.70, локально до 137 уровней), передний обязан ПРОТЕКАТЬ за свой силуэт. ПИРАМИДА БОКЕ ОКУПАЕТСЯ, в отличие от SSR, но не всегда, и граница измерена: уровень выбирается по расстоянию между пробами, поэтому при CoC до ~20 px и 24 пробах он выходит нулевым и пирамида просто не включается; на большом размытии (CoC до 64 px) при равных пробах она точнее на 27% (16 проб) и 15% (32 пробы) против сбора с нулевого уровня, то есть экономит ~1.5x по пробам, и вдобавок на 12% дешевле (тексели вдвое мельче, кэш лучше). Правило: пирамида полезна там, где радиус много больше шага выборки — у SSR скользящий луч не давал пропустить ни клетки, здесь пятно крупнее шага. Цена линейна по пробам: 1.962/2.880/6.416 мс при 16/24/64 пробах, и `--dof=0` стоит 0.534 мс, потому что пасс исполняется — тир качества обязан менять форму графа. Не сделано сознательно: сбор в половинном разрешении с depth-aware апсемплом (срез проверял физику и разделение планов) |
| `RND-45` | Motion blur | `M` | **СДЕЛАНО в `PF03` 2026-08-20.** Место в цепочке по физике: сначала объектив (DoF), потом шторка, и всё до замера экспозиции — сенсор видит уже смазанный свет. Длина смаза = motion-вектор кадра на долю открытой шторки (0.5 = 180 градусов при любой частоте). ИНВАРИАНТ, ловящий почти любую ошибку: на статичной сцене со статичной камерой размытие обязано не менять НИЧЕГО — измерено ПОБИТОВО (RMSE 0). Длина смаза линейна по шторке (8.253/16.481/32.936 при 0.25/0.5/1.0, расхождение 0.15%) и по скорости объектов (5.872/12.059/26.468 при 1/2/4), причём первый замер по скорости оказался СПУТАННЫМ: положение кубов само зависит от скорости, поэтому сравнивались разные картинки; чистый замер — одинаковая фаза (кадр × скорость = const). Остаток нелинейности (2.05x и 2.19x вместо 2) лежит не в размытии, а в векторах: motion это конечная разность, а движение круговое, поэтому при большем шаге хорда сильнее отличается от дуги (та же слабая нелинейность видна в срезе 1 у камеры). РАСШИРЕНИЕ МАКСИМУМА MOTION ПО ПЛИТКАМ — условие корректности, а не оптимизация: у неподвижного фона позади летящего объекта собственный вектор нулевой, и без расширения шлейф обрывается на границе плитки (артефакт реализации по сетке плиток). Измерено: площадь изменённого кадра 4.74% против 2.82% при том же пиковом изменении, то есть расширение добавляет ПЛОЩАДЬ, а не силу. Эти же 4.74% при неподвижной камере подтверждают per-object векторы. Веса — та же структура, что у боке (передний вклад: проба ближе и её смаз достаёт до центра; задний: разрешён настолько, насколько смазан сам центр), потому что motion blur это глубина резкости с пятном, вырожденным в отрезок. Цена СУБЛИНЕЙНА по пробам (0.942/1.078/1.358 мс при 6/12/24: большинство пикселей выходит сразу), `--shutter=0` стоит 0.427 мс просто за существование пасса. Экспозиция и шторка в реальной камере — одна ручка, здесь разведены сознательно (иначе ручка размытия меняла бы яркость). ОБЩИЙ СЧЁТ срезов 10-12: вся цепочка со всем включённым (17 пассов) стоит 10.989 мс на 1280x720 Iris Xe — то есть «включено всё» не конфигурация для 60 fps, и это ровно случай для тиров `RND-31` |
| `RND-46` | TAAU: накопление с апскейлингом | `L` | **ИНФРАСТРУКТУРА СДЕЛАНА в `PF03` 2026-08-20, РЕКОНСТРУКЦИЯ НЕТ — измерено.** Разделение разрешений работает: всё до накопления живёт в разрешении рендера (глубина/нормали/motion/шейдинг/туман/отражения/пирамида глубины, SSAO в половине рендера), всё после — в разрешении дисплея; TAA стал реконструирующим (вход мельче цели, подъём Catmull-Rom, окрестность отбраковки в текселях ИСТОЧНИКА). Мелочи, обязанные следовать за разделением: размер блока плиток motion выводится из отношения размеров, проверка пирамиды индексируется по uv. АМПЛИТУДА ДЖИТТЕРА оказалась контрактом с РЕКОНСТРУКЦИЕЙ, а не свойством сетки рендера: теория требует гулять внутри пикселя рендера, но при `--render-scale=0.5` это дало заметное дрожание камеры, потому что реконструкция усредняет по сетке ДИСПЛЕЯ и позиции отсчётов не разрешает. Измерено (дрожание кадр к кадру / отклонение от эталона на статичной сцене): при 0.67 в пикселях рендера 4.89/6.36, в пикселях дисплея 4.26/6.23; при 0.5 — 5.50/6.11 против 4.18/5.97, то есть лучше по ОБЕИМ метрикам. Оставлено в пикселях дисплея; вернуть в пиксели рендера надо будет вместе со счётчиком набранных отсчётов. Страховка держится: при `--render-scale=1.0` кадр ПОБИТОВО тот же, валидация чиста на 0.75/0.67/0.5. ЦЕНА: пассы до накопления дешевеют пропорционально пикселям (пирамида 3.7x, шейдинг 4.5x, SSR 3.3x при половинном разрешении), пассы после НЕ дешевеют вовсе (TAA 0.730→0.717, DoF 0.540→0.537, компоновка 1.776→1.677) — итого вся цепочка 8.166 → 6.401 → 5.894 → 4.970 мс при 1.0/0.75/0.67/0.5, то есть 1.64x при половинном разрешении, а не 4x. Вывод, который надо знать заранее: **TAAU ускоряет ровно ту половину цепочки, которая живёт в разрешении рендера**, а после DoF/motion blur/компоновки это меньшая половина. КАЧЕСТВО — отрицательный результат: TAAU против эталона полного разрешения даёт 5.95/6.36/6.11 при 0.75/0.67/0.5, а простой апскейл (`--taa=0`, Catmull-Rom + резкость) 4.02/4.24/4.74, то есть накопление чуть ХУЖЕ. Вторая метрика подтверждает независимо: высокочастотная энергия эталона 3.51, у TAAU 3.99, у апскейла 4.01 — у обоих низких вариантов её БОЛЬШЕ, чем у эталона (лестницы, а не деталь), и между собой они неразличимы. Две попытки, обе измерены: (1) первая версия сэмплировала кадр в одной точке каждый кадр — накопление усредняло один и тот же отфильтрованный отсчёт и добавляло только размытие (по типу та же ошибка, что с маршем SSR: реализация не была алгоритмом); (2) вес истории по расстоянию от фактической позиции отсчёта до центра пикселя дисплея сделал хуже (6.19 против 5.95) и замедлил сходимость (средний вес ~0.99). Гипотеза «мешает neighbourhood clamp» НЕ подтвердилась: без отбраковки резко хуже (24.27). ДИАГНОЗ: фиксированный вес истории — неверный орган управления; нужен СЧЁТЧИК набранных отсчётов на пиксель, отбраковка в сетке рендера и отдельная логика disocclusion на сетке дисплея. Под эту работу теперь есть инфраструктура, эталон и две метрики |
...
| `RND-23` | Stencil effect path | `M–L` | **сделано 2026-08-22:** PF04 использует `D24S8`; selection/debug владеют bit `0x01`, local tint — `0x02`, spatial window с локальным depth clear и rigid-mapped remote camera — `0x04`. Fixed alternate view был отвергнут: при main-camera rotation aperture и content жили в разных clip spaces. Явные source/destination plane transforms задают `M=T_destination·Ry(π)·T_source⁻¹`; каждый кадр использует `V_remote=V_main·M⁻¹`, поэтому `P·V_remote·M·X=P·V_main·X`; translation даёт parallax, rotation не скользит под рамкой. Destination plane передаётся как world clip equation и отсекает camera-side geometry через `gl_ClipDistance`; recursion отложена. Core dynamic `StencilReference/CompareMask/WriteMask` задаются обычным config list и runtime step constant; selection channel/read/write меняются на следующей записи command buffer без rebuild. Внутри `material` states хранятся как `array<uint32_t,16>` с `UINT32_MAX` sentinel вместо растущего набора bool-полей; `DEVILS_ENGINE_PAINTER_DYNAMIC_STATE_LIST` в `common.h` единожды связывает human names с `VK_DYNAMIC_STATE_*` и генерирует enum/string/`to_vulkan` mapping. Asymmetric fixture одним draw показывает front `replace → 0x10` и back `invert → 0x30`. Inverted-hull outline после эксперимента удалён: даже замкнутая six-face box оставалась перспективной world-space поверхностью. Финал хранит selection coverage и ближайшую reverse-Z depth в одной `sf2` mask: `VK_BLEND_OP_MAX` выбирает ближайшую поверхность order-independently, поэтому отдельный selection D32 не нужен и результат на fixed view побитово совпадает с D32-вариантом. Трёхпиксельная dilation переносит depth в `gl_FragDepth`, а локальный outline проходит и пишет обычную scene depth, поэтому foreground и portal относятся к нему как к selected object. `O`/`--outline-overlay` отдельно включает жёлтый through-wall вариант без depth test ПОСЛЕ green/red wallhack draws, поэтому scene geometry больше не перекрывает selector. Hidden-target bit `0x40` строится через reverse-Z `greater_or_equal` + `depth_fail_op=replace`; красный consumer проверяет `(stencil & 0x44)==0x40`, поэтому wallhack не протекает в portal. Fixed-view occluder явно различает local/overlay A/B. Parser regressions покрыты 14 cases/157 assertions; PF03-style dumps и Vulkan validation чисты; лаборатория закрыта. |
...
| `RND-51` | Предел групп submit'а поднят с 16 до 24 | `S` | **сделано 2026-08-20 в `PF03`.** Цепочка среза 12 доросла до семнадцати пассов, и `render_graph_instance::submit` упёрся в жёсткий предел шестнадцати групп. Обход «слить два эффекта в один пасс» стоил бы раздельного измерения их цены, а вся площадка на этих измерениях и стоит. Число задавало только размер стековых массивов (`signal_storage`, `infos`), поэтому поднять его дешевле, чем ужимать граф; при 24 группах и 8 сигналах это 1.5 КБ стека |
...
| `RND-30` | MSAA в конфиге графа | `M` | **СДЕЛАНО 2026-08-21.** Подтвердилось буквально: MSAA не тумблер, а ФОРМА конвейера, поэтому в `PF03` он сделан отдельным ГРАФОМ (три графа: `post`, `forward` с MSAA, `forward-plain` без — честная база сравнения), и «выключить MSAA» означает граф без цели resolve, а не ноль в UBO. Словарь конфига оказался в два поля, и оба про аллокацию: `samples = N` у РЕСУРСА (как `mips`; смена = пересоздание вложений, рычаг «размеры целей» из `RND-31`) и уже существовавший юсадж `resolve_attachment` у render target. `rasterizationSamples` у материала НЕ объявляется — выводится из render target пасса, потому что Vulkan требует их совпадения, а два места для одного факта разъедутся. МНОГОСЭМПЛОВАЯ ГЛУБИНА В 1.0 ЗАКОННА — нельзя только РЕЗОЛВИТЬ её (нужен depth_stencil_resolve, ядро с 1.2), а forward-рендеру резолв глубины не нужен, поэтому запрета «MSAA + глубина» в движке нет. Проверок ровно столько, сколько требует спецификация (степень двойки до 64; один мип-уровень; согласованность вложений пасса; многосэмпловый источник у resolve) — остальное решает автор. ИЗМЕРЕНО: MSAA трогает меньше процента кадра (0.53/0.79/0.92% при 2/4/8 сэмплах), энергия аляйсинга 4.648 → 4.390 → 4.276 → 4.232; предсказание про шейдинговый аляйсинг подтвердилось — на шахматном полу 0.74% площади и RMSE 1.93, на силуэтах 1.06% и RMSE 4.65 при максимуме 250, то есть лечится ровно геометрия. ЦЕНА: 0.769 мс без MSAA, 0.975/1.012/1.206 при 2/4/8 (+27/+32/+57%) — время дешёвое, шейдер работает раз на пиксель; дорога ПАМЯТЬ: при 4 сэмплах вложения занимают 48 байт на пиксель, 133 МБ на три копии per_frame (при 8 — 265 МБ), и число копий стоит уменьшать override'ом `type`. Главный аргумент развилки: forward с четырьмя сэмплами дешевле полной цепочки PF03 В ВОСЕМЬ РАЗ (1.012 против 8.349). ПЯТЬ ЛАТЕНТНЫХ ДЕФЕКТОВ ДВИЖКА, вытащенных срезом: (1) `recreate_screensize_resources` не пропускал ресурсы неактивного графа и создавал вид чужого формата на чужой картинке (стало видно только со вторым графом); (2,3,4) `resolve_attachment` отсутствовал в `convertps`, `convertam` и `convertiuf`; (5) состояние блендинга считало цель resolve цветовым вложением, и `attachmentCount` расходился с `colorAttachmentCount`. Плюс добавлена внятная диагностика вместо жалобы драйвера: resolve при односэмпловом источнике падает с объяснением, что выключенный MSAA — это другой граф |
| `RND-31` | Независимые настройки выбранного render graph | `M`, fragments `L` | **конструкция уточнена 2026-08-21:** production сначала выбирает renderer/graph recipe и технологии; затем graph-specific оси `shadows/ao/reflections/dof/...` независимо выбирают named choices. `performance/balanced/quality` — только макрос, записывающий несколько choices; concrete variant — канонический результат resolver, а не полный authored profile. Наружу достаточно двух цен: runtime UBO/SSBO data применяется на следующем кадре, всё остальное (specialization constants, defines, material state, resource shape, topology) строит новое поколение graph. Одна ось может иметь обе цены: DoF gameplay/cinematic — runtime, DoF off/on — build. MVP: `$declare_value[.component]` в step `shader_constants` и material `definitions`; typed partial `values/materials/steps` patches; graph-specific schema choices, defaults/preset, ownership/conflict checks и полный resolve+validation до commit. Optional patch fields обязательны: нынешние mirror defaults не различают absent и explicit false/0. Следующий structural слой — ограниченные named graph slots/fragments (`none|ssao`, `none|dof`) с явными resource contracts; это избегает и произвольного удаления pass'ов, и ручных `2^N` полных графов. Подробная форма: `PF03_post_processing/RENDER_PROFILES.md`. Старые per-value `presets`/`scale_presets` парсятся, но не применяются и этим контрактом заменяются. |
...
`RND-16/19/21` и последующие самостоятельные temporal slices, `PF04` — `RND-23`; `PF05` отдельно доказывает
scene effects (SDF/decals/particles/weather/cel shading/billboards/world-space UI). `PF06/07` выбирают
...
| `RND-14` | Device-local draw groups and GPU indirect path | `L` | profiling-backed consumer. **ПОТРЕБИТЕЛЬ С ЗАМЕРОМ НАЙДЕН (GN03, 2026-09-04):** у шага `draw_indirect` `drawCount` зашит единицей (`libs/painter/src/painter/common_steps.cpp`), поэтому «список вызовов по строке на чанк» выразить нечем, и потоковая арена рисуется ОДНИМ вызовом на весь занятый диапазон. Замер GN03: цена пасса пропорциональна вершинам В ДИАПАЗОНЕ ОТРИСОВКИ (~0.65 нс на вершину), а не видимым, поэтому отсечение чанков по пирамиде видимости не даёт НИЧЕГО (видно 29-30% чанков, пасс 4.65 против 4.74 мс). При многократном косвенном вызове это разница между 2.1 и 0.6 мс потока вершин на окне из 405 чанков |
| `RND-15` | Mips/arrays/MSAA/render-target role formats | `L` | согласовать с `RES-07` и resource validation |
| `RND-32` | Цена компиляции шейдеров: один компилятор вместо одного на шейдер | `S` | **основное сделано 2026-09-04, остаток назван.** `shaderc::Compiler` создавался ВНУТРИ `shader_crafter::compile`, то есть состояние glslang поднималось заново на КАЖДЫЙ шейдер движка. Замер на вычислительном контексте: `100`–`124` мс на программу, вторая не дешевле первой (значит не раскрутка), оптимизатор SPIR-V ни при чём (`99`–`105` мс без него), драйвер ни при чём (дисковый кэш пайплайнов `111.8` холодный против `107.1` горячего). Один компилятор на поток (`static thread_local`): первая компиляция `92` мс — одноразовая инициализация glslang, вторая `2.3` мс, без оптимизатора `1.1`. То есть **98% времени компиляции уходило на то, чтобы завести компилятор и тут же его выбросить**, и это касалось всего кода: и материалов render-графа, и `glsl_source_file::prepare_spirv`, и вычислительного контекста. Поле у `shader_crafter` не помогло бы — почти все потребители создают крафтер на один вызов. ОСТАТОК: (1) замерить, что это дало СТАРТУ настоящих лабораторий, где шейдеров десятки, а не один — числа сейчас есть только по вычислительному контексту; (2) `compile()` гоняет источник ДВАЖДЫ (`PreprocessGlsl`, затем `CompileGlslToSpv`), и вторая половина этих 1–2 мс, возможно, лишняя; (3) решить, остаётся ли `thread_local` окончательной формой или компилятор должен принадлежать кому-то явно — глобал в функции виден плохо |
| `RND-33` | Кэш пайплайнов на вычислительном пути | `S` | **сделано 2026-09-04.** `compute_pipeline_maker::create` звал `createComputePipeline` с `nullptr` вместо кэша, то есть вычислительный путь painter кэшем пайплайнов НЕ ПОЛЬЗОВАЛСЯ вовсе — притом что `graphics_base` его создаёт и заполняет с диска, а графический `pipeline_maker` кэш передаёт. Заметить это по поведению нельзя: пайплайн без кэша работает точно так же, только создаётся дольше. Добавлена перегрузка `create(name, cache, layout, ...)`, ею пользуются и `compute_step_instance::create_pipeline` (шаг render-графа), и `painter::compute_context`. Замер показал, что на GLSL→SPIR-V кэш пайплайнов не влияет (цена там, см. `RND-32`), поэтому выигрыш от него надо искать на SPIR-V→ISA и на повторных запусках |
...
`GN01_generator_contract` **закрыта закрывающим аудитом 2026-08-31**, а `GN02_planet_generator`
**закрыла первый вертикальный срез 2026-08-31**: планета целиком от тектоники вниз по причинам, семь
шагов из конфига, пакет на диске с отпечатком, `37/37` проверок на четырёх зёрнах. Уточнение исходной модели по итогам среза:
...
| ID | Задача | Сложность | Результат |
| --- | --- | --- | --- |
| `GEN-01` | Typed artifact/pass descriptors | `M–L` | **частично 2026-08-31.** `buffer_layout` (именованные поля + написание форматов painter + aos/soa) и `step_description` (`reads`/`writes`/`params`/`body`) есть; добавлены ДИАПАЗОНЫ значений (`ranges = { имя = [минимум, максимум, шаг] }` в том же документе, `parse_value_ranges`) — из них хост строит настройки генератора, ничего не зная о смысле числа, а значение без диапазона просто не настраивается; versions, schemas и budgets прохода — нет |
...
| `GEN-03` | Reusable C++ building-block facade | `L` | **в основном сделано 2026-08-31.** Встроенные: fill/remap/classify/box_blur + четыре свёртки, плюс арифметика полей `blend`/`modulate`/`decay`/`maximum`/`minimum`/`index` и `absolute` у `remap`, а по графу `graph_slope` (средний перепад до соседей: наклонная плоскость от своего размытия почти не отклоняется, поэтому «отклонение от сглаженного» не отличает нагорье от склона) (GN02: без неё правило в конфиге не собирается — программа `ds` возвращает одно значение в одно поле, а вызов функции не является операндом выражения). Замкнутая поверхность и граф (`add_graph_tools`, GN02): `sphere_points` (решётка Фибоначчи), `sphere_adjacency` (K ближайших, СИММЕТРИЗОВАННЫЕ в канонический CSR), `graph_blur`, `graph_frontier`, `graph_flood` (заливка от нескольких источников по стоимости И проходимости — запрет отдельно от цены, иначе область через пролив выходит разорванной), `graph_vote`, `poisson_seeds`, `lookup`. Scatter: `group_by` (CSR через подсчёт-префикс-заполнение) и `accumulate` (приватные срезы + слияние по номеру чанка) — оба побитово детерминированы и параллелятся 4.6x/4.2x. Внешние (`originator_primitives`): FastNoise2 `noise_grid`/`noise_at` (дерево узлов = закодированная строка в конфиге) и jc_voronoi `voronoi_label` (gather, kd-запрос) / `voronoi_adjacency` (scatter, соседство из Делоне) / `voronoi_polygons` (scatter, контуры как ПЛАНАРНАЯ СЕТКА С ОБЩИМИ вершинами; проверка — сумма площадей равна площади карты). Объём (`add_volume_tools`, GN03): `marching_cubes` — поверхность нулевого уровня поля плотности; апертура `scatter` при `chunk_local`, три фазы с фиксированным разбиением (случай куба → префиксная сумма → запись своего отрезка), поэтому результат побитово один при любом числе потоков. Таблица 256 случаев ВЫВОДИТСЯ из правила соединения на грани куба (правило — функция только четырёх знаков грани, поэтому непрерывность между кубами по построению), а не переписывается из статьи. Плюс `component` у `remap`: правило на ds компоненту вектора достать не может, а объёму она нужна на каждом шаге. Нет: эрозия, watershed, spline-коридоры |
| `GEN-03a` | devils_script над плотным буфером | `L` | **сделано 2026-08-31.** `originator::script_program`: скоуп = элемент, поля по именам привязок, пороги через `ctx:arg:` из параметров шага (типы проверяются при компиляции, забытый параметр — громкая ошибка), апертура следует из регистрации (писать и достать соседа нечем), PRNG от индекса, небезопасные опкоды. Замер: `53.34` нс/элемент в один поток и `8.69` на 11 против `4.29`/`0.92` у нативного ядра и `108.12` у поэлементного lua. **Вывод, которого не ждали:** выигрыш ds перед lua не в скорости интерпретации (там всего 2x), а в том, что ds распараллеливается и проверяется по типам на разборе |
| `GEN-04` | Dedicated deterministic headless Lua environment | `L` | **сделано 2026-08-31, дополнено.** `originator::script_host`: свой `sol::state`, своё окружение, нет `math.random`/os/файловой системы, отдельная цель сборки от `visage`. Бюджет тела шага теперь НЕ бесконечный по умолчанию (200 млн инструкций lua; часы выключены, потому что инструкции не считают время внутри нативного инструмента, а часы не отличают цикл от честной работы), шаг хука выводится из лимита, а исчерпание бюджета помечается флагом и проваливает шаг ДАЖЕ ЕСЛИ ошибку поймал `pcall` внутри тела. Тогда же нашлось, что whitelist не работал вовсе: ключ `std::string_view` в проксировании sol2 молча промахивался, и базовые функции (`error`, `pcall`, `math.*`) оседали в окружении как `nil` — тела GN01 этого не замечали, потому что единственный `error()` стоял на несрабатывающем пути. Пропажа ядра списка теперь громкая ошибка при создании хоста |
| `GEN-05` | Lua pipeline glue and typed artifact handles | `L` | **пересмотрено + частично 2026-08-31.** Lua не собирает граф — она ТЕЛО шага. `script_buffer_view`/`field_ref`: имя поля разрешается один раз, тяжёлой памятью скрипт не владеет. Добавлены общие значения пайплайна (`values.tavl`, аналог `declare_values` painter): число, нужное нескольким шагам, объявляется один раз, а шаг может переопределить его своим `params` |
| `GEN-06` | Pass templates | `L` | raster/graph/unit/volume templates with explicit iteration and ordering. **Объёмный случай пройден 2026-09-03 (GN03), но шаблоном не стал:** выход ПЕРЕМЕННОЙ длины выразился без нового механизма — ёмкость объявлена размером буфера, занятая длина приезжает обычным буфером на один элемент, переполнение отказывается вслух. То есть шаблон здесь не понадобился, и заводить его до второго потребителя незачем |
| `GEN-07a` | Чанкованная генерация | `L` | **сделано 2026-08-31.** `pipeline::set_chunk` без перевыделения буферов; правило «результат чанка зависит только от (зерно мира, ключ)». **ДВА зерна с разными контрактами** (выяснилось на сверке с картой целиком, где одно зерно давало шум с разъездом 1.99 на швах): `step.seed` без ключа для непрерывного через границу, `step.chunk_seed` с ключом для независимого в чанке; по умолчанию непрерывное. Замерено на 2x2/4x4/8x8: pointwise чанкуется ТОЧНО (0 на 262144 клетках), gather расходится строго в полосе шириной radius и ровно 0 внутри => нужна полоса перекрытия. Следствие для авторов: самокалибровка по измеренному диапазону несовместима с чанкованием, режим выбирает конфиг. Фоновый поток — у ПОТРЕБИТЕЛЯ, не в движке. Носитель ключа у scatter (`chunk_local`/`global`) — единственный контракт, который ОБЪЯВЛЯЕТСЯ, а не выводится (из одного чанка смысл ключа не проверить); по умолчанию глобальный, то есть при чанковании scatter отклоняется, пока автор не скажет обратное. Кросс-чанкового слияния нет намеренно: замер показал, что при float-аккумуляторе порядок прихода чанков меняет 816/1024 групповых сумм, а при фиксированной точке — ровно 0, поэтому слияние (если понадобится) должно быть либо в каноническом порядке ключей, либо в фиксированной точке |
| `GEN-07` | Deterministic scheduler | `L–XL` | **частично 2026-08-31.** serial-vs-MT побайтовое совпадение по ВСЕМ буферам, свёртки и scatter с фиксированным разбиением; зерно шага = hash(зерно, имя шага). Апертура задаёт и то, к чему относится диапазон (у scatter — ко входам). **Найдено замером:** FastNoise2 `GenUniformGrid2D` накапливает позиции внутри строки (отрезок с середины строки расходится на 3e-07 → генерируем строку целиком; `GenPositionArray2D/3D` устойчив к любому разбиению), а набор SIMD-инструкций выбирался в рантайме (`max` vs `SSE2` = 1.8e-07) → закрыто штатной опцией `FASTNOISE2_STRICT_FP`, после которой выход всех наборов побитово одинаков при неразличимой на уровне пайплайна цене. Seal/order пакета — нет |
...
| `GEN-10` | Generated-content inspection tooling | `M–L` | **в основном сделано 2026-08-31 (GN02).** Время по шагам, диапазоны любых полей (`--stats`), ASCII-карта климата, широтная сводка суши и ОКНО (`--view`): сфера с двенадцатью полями на цифрах плюс просмотр по шагам генерации (`[`/`]` пересчитывают мир до шага N, поздние поля остаются нулевыми и оверлей об этом говорит). Сетка просмотра намеренно не совпадает с клетками: у решётки Фибоначчи нет полигонов, поэтому рисуется икосфера, а вершина берёт значение ближайшей клетки — тот же вопрос, который задаёт потребитель пакета. Окно сразу нашло два дефекта, которых числа не показывали: расселение культур считалось в шагах графа (зависело от разрешения) и земля без метки красилась как вода. Снимок кадра — `--frames=N --frame-dump=PATH`. Замер по шагам сразу дал вывод: у генератора планеты время распределено ровно по семи шагам, а параллельность даёт всего 1.6-1.8x на 11 потоках, потому что заливки и выбор затравок последовательны по природе. Нет: batch-отчётов и сравнения двух пакетов |
