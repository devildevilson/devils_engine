...
| `GEN-10` | Generated-content inspection tooling | `M–L` | **частично 2026-08-31 (GN02).** Время по шагам, диапазоны любых полей (`--stats`), ASCII-карта климата и широтная сводка суши. Замер по шагам сразу дал вывод: у генератора планеты время распределено ровно по семи шагам, а параллельность даёт всего 1.8x на 11 потоках, потому что заливки и выбор затравок последовательны по природе. Нет: batch-отчётов и сравнения двух пакетов |
| `GEN-11` | 2D region/planet reference pipeline | `XL` | **в основном сделано 2026-08-31 площадкой `GN02_planet_generator`** (для СФЕРЫ, а не для 2D): тектоника → суша → климат → сезоны → провинции и морские зоны → культуры и история → пакет на диске с отпечатком. Топология — решётка Фибоначчи и симметризованный CSR соседства, поэтому швов нет и суммы по клеткам не требуют весов площади. Нет: рек и эрозии, государств и торговли, чтения пакета потребителем |
