...
| `RND-42` | Depth pyramid (Hi-Z) | `M` | планируется в `PF03` срезом 10. Поверх мип-цепочек (`RND-38`): цепочка минимумов/максимумов глубины. Нужна и SSR, и отсечению, и depth of field — то есть это инфраструктура, а не эффект, и она идёт впереди своих потребителей. Вход в `RND-20` |
| `RND-43` | Screen-space reflections | `L` | планируется в `PF03` срезом 10 поверх `RND-42`: марш по пирамиде глубины, темпоральный денойз (история уже есть), fallback на небо, маска по шероховатости |
...
| `RND-50` | Пасс графа по требованию, а не каждый кадр | `M` | НАЙДЕНО в `PF03` 2026-08-20 (срез 9): все пассы графа выполняются каждый кадр, и сказать «выполни, когда изменились входы» нечем. Таблица грейда — чистая функция настроек, то есть кэш; перезапекать её каждый кадр не нужно. Цена измерена и не абстрактна: `0.018` мс при `32³` и `0.094` при `64³`, причём во втором случае запекание полностью съедает выигрыш таблицы против аналитического грейда (+0.247 против +0.185). То же понадобится тени статичной геометрии, предвычисленной атмосфере и любому кэшу внутри графа. Смыкается с `RND-31`: «пасс не выполняется вовсе» — это выбор ФОРМЫ графа, а «пасс выполняется, но ничего не делает» — число в UBO, и путать их нельзя. **ФОРМА СОГЛАСОВАНА 2026-08-20:** это не новый механизм исполнения, а третий вид счётчика — условный, который двигает ХОСТ по требованию (`inc_counter` уже есть, со стороны хоста нового кода ноль). Счётчик остаётся тупыми данными: «делал ли я это поколение» — состояние ПАССА (`observed`), и пасс объявляет привязку ЯВНО (`counter = cache_counter`), чтобы читателю конфига было видно, что исполнение зависит от внешних событий, и чтобы пасс с ресурсом были логически связаны одним счётчиком. Сравнение `observed != counter.value` (а не `<`: счётчик swapchain выставляется через `set_value` номером образа и НЕ монотонен, плюс снимается вопрос переполнения); все счётчики стартуют с 1, локальная память пасса с 0 — тогда сентинел «никогда не исполнялся» не нужен, а сброс при смене графа и resize делается занулением памяти ПАССА. Ресурс кэша обязан жить на ТОМ ЖЕ счётчике (иначе loud error): индекс копии и так выводится из значения счётчика (`history_copy_index`), поэтому сдвиг сам даёт новый слот, а кадры без сдвига читают тот же. Период объявляет автор (хостовый счётчик), но `singlebuffer` для кэша, который пишет GPU, запретить громко — в момент перезаписи кадры в полёте ещё читают старое содержимое, нижняя граница честно 2. ОДИН счётчик на пасс: при двух пасс, писавший бы ресурс второго счётчика без его сдвига, попал бы в слот, который ещё читают; два кэша = два пасса, а «зависит и от настроек, и от resize» — это один счётчик, который хост двигает по любой из причин. `history = N` на ресурсе условного счётчика означает предыдущее ПОКОЛЕНИЕ, а не предыдущий кадр, и арифметика копий работает как есть — ломается только выведенный кросс-кадровый порядок, поэтому это WARNING, а не запрет (точным его не сделать: решает частота сдвигов, а её мы сознательно не объявляем; текст должен предлагать `wait_previous`). Гейт — свойство ПАССА, не шага: если внутри есть шаги, обязанные идти каждый кадр, это признак, что пасс надо разделить. Отдельно нужно понятие «пасс в этом кадре не исполнялся» для `gpu_timestamp_profiler` — иначе число измеряемых пассов перестаёт быть постоянным и индексы едут (накопитель PF03 на этом прямо падает) |
