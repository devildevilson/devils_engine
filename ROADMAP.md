...
| `RND-23` | Stencil effect path | `M–L` | **два среза 2026-08-22:** отдельный PF04 executable использует `D24S8`; selected geometry пишет только bit `0x01`, expanded shell читает только его и даёт outline, fullscreen debug визуализирует его без sampled stencil. Независимая невидимая world-space proxy пишет bit `0x02` без color writes, а отдельный fullscreen consumer ограничивает им cyan local tint; overlap равен `3`, outline остаётся корректным. Исправлены два production parser-дефекта: явный color mask теперь заменяет RGBA (`none` отключает writes), а отсутствующие blend expressions больше не становятся invalid Vulkan enums; покрыто 147 assertions. Кадр проверен визуально, восьмисекундный прогон обеих масок с Vulkan validation чист. Runtime reference/compare/write masks ещё отсутствуют. Осталось: window mask, dynamic masks/reference и намеренно различимые front/back operations. |
