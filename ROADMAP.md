...
- ✅ **`local_sequence` как часть детерминированного ключа (первый срез)** — каждый source получает монотонный
  номер своих effect-вызовов в порядке обхода одного скрипта. Нельзя использовать порядок прихода задач
  или общий atomic ticket: они зависят от планировщика. Базовый порядок collect:
  `(group_key, source_id, local_sequence)`; дополнительные tie-breakers задаёт policy. Переполнение
  sequence/buffer — громкая ошибка, не drop и не недетерминированный fallback. `mt::record_scope`
  инжектит source/index и общий для всех strategies sequence; executor имеет multi-record slot-table.
...
- 🟡 (н) **аргументный storage/codec только для памяти текущего шага** — первый срез копирует copyable
  value args и превращает `string_view` в owned string; ссылки запрещены. Дальше: custom codecs,
  member-functions и переиспользуемая arena/SBO вместо одной heap allocation на call. Это НЕ serializer.
