...
7. **Две шкалы времени + настраиваемый игровой календарь + отложенные эффекты** (B-ж) —
   **[необходимо]** · **ближайшая задача, дизайн уточнён 2026-07-13**.
   ...
   - **Game time** — шкала симуляции, останавливается ровно тогда, когда поставлены на паузу игровые
     системы. Gameplay duration/expiry по умолчанию живут здесь.
...
15. **Спавн-архитектура + per-entity brain** — **[необходимо, но НЕ следующий implementation slice]** ·
    ...
    - **per-entity brain** — GOAP/FSM сейчас slice-level (один `acumen`/`mood` на всех акторов). Сделать
      per-entity: компонент `brain_ref{goap_id, fsm_id}` (заполняет prefab `reference<>`) + пул goap/fsm
      систем по имени конфига + `decide_actor`/`apply` берут системы актора. Разблокирует мультитиповый
      мир (хищник/жертва/герой с разными мозгами из префабов). `reference<>` в префабе уже есть — мешает
      монолитный мозг слайса.
...
(п.11–12) остаются необходимыми, но не перебивают эту ближайшую последовательность.
...
- 🟡 (н) **native↔ds фасад и полный call contract** — `script_function<bool/number/void>` уже работает;
  отделить mutable args/out/lists от immutable execution context, провести единый registration path,
  определить string/object-маршалинг. `libs/act` скрывает источник функции, а не дублирует ds runtime.
