...
- реестры по концернам (act/acumen/mood/aesthetics-serial/demiurg/catalogue) — каркасы готовы и
  ...
  тоже закрыты. Остались общий trace service/UI, удаление мёртвого `effect_sink` seam и расширение
...
- `intent` остаётся границей входа в детерминированную мутацию. Старый `effect_sink` после аудита не
...
- **статус интеграции:** `act::building_blocks`, DS RNG/target scopes, config effects и базовый
  ...
  удалены. Следующий срез определяется реальным потребителем: cost formulas/богатое объяснение GOAP,
  player intents либо spawn signatures;
...
1. ✅ **Закрыть узкий devils_script ↔ act registration/effect slice (п.14)** — **ядро готово 2026-07-18.**
   ...
   - ⏳ Остаток п.14: cost formulas/богатое cost explanation; string/object/vector marshalling по
     фактическим сигнатурам; удалить неиспользуемый `effect_sink` seam.
...
1. **GPU asset lifetime + command coverage.** Реализовать реальное освобождение texture/buffer slots:
   hot-unload помечает слот, renderer после безопасной frame-generation/fence границы заменяет descriptor
   на placeholder, уничтожает VMA image/buffer/view и возвращает слот в `empty` для повторного использования.
   Рядом закончить/покрыть тестами оставшиеся реально нужные draw/dispatch/transfer команды.
2. **Query rebuild после snapshot.** `load_world` уже эмитит один `snapshot_loaded_event`; перенести тип
   события в общий header, подписать `query_t` и дать ему общий `rebuild()` по целому загруженному миру.
   Это отдельный малый correctness slice; lazy view пересборки не требует.
3. **Узкий spawn slice.** Простые spawn points для еды + script-facing абстракция выбора точки;
   координатный `spawn_at` сохранить. Сложные filter/pick/spatial policies отложены.
4. **Player entity + UI intent queue.** UI пишет только `act::intent`, gameplay потребляет на границе
   тика; обязательный dedup pending-intent по action type. Это первый правильный mutating UI→sim путь.
5. **UI budgets как engine settings.** Error isolation уже имеет protected Lua entry, восстановление
   незакрытых Nuklear windows и instruction/time hook. Остаток — вынести instruction/wall-time/GC и
   при необходимости convert-buffer budgets из hardcode в настройки, измерять Lua update + `nk_convert`
   через общий catalogue perf service и определить реакцию warn/skip/disable state на превышение.
6. **Replay — медленное наращивание, не отдельный мегапроект.** Сначала зафиксировать поток входов
   (`act::intent`, rational game-speed/pause/state transitions) + периодические существующие snapshots
   и проверку deterministic hash. Catalogue остаётся execution/trace слоем, не replay serializer.
...
framework без ассетов/первого потребителя. Asset worker может блокироваться на тяжёлой загрузке; важно,
чтобы pending/failed state, диагностика и shutdown оставались корректными, а не вводить artificial
per-tick budget или дополнительный pool без измеренной проблемы.
...
14. **Интеграция devils_script ↔ act** — **[необходимо; ядро готово, расширять от потребителя]**.
    ...
    production fallback и отдельная native predicate/action таблица удалены. Остаток — удалить
    неиспользуемые `effect_sink.h`/`exec_context.sink`, а string/object/vector/custom codecs и cost
    formulas добавлять под первый реальный spawn/player/GOAP consumer. Меж-энтити эффекты уже идут
...
15. **Спавн-архитектура** — **[необходимо; следующий практический slice — простой]**.
    ...
    - ⏳ **Ближайший slice:** добавить в `tile_frontier` суперпростые spawn-point сущности/компоненты,
      сначала для респавна еды. Скрипт должен обращаться к абстракции выбора/разрешения точки, а не
      обязательно знать сырые координаты; `spawn_at` остаётся низовым escape hatch и тестовым путём.
      Первый вариант не требует spatial query, cooldown/capacity/filter DSL или универсального manager-а.
...
16. **Меж-энтити взаимодействия — deferred-call эффекта через `catalogue`** — **[необходимо]** ·
    ...
    **Дальний data-driven follow-up из раннего claim-дизайна:** отдельный per-target `claim_buffer`
    ...
    (1) удалить старый `effect_sink`; (2) member/custom bounded codecs по реальным
...
- **Пробелы в тестах** — focused-тесты уже есть у `catalogue` introspection, `mood`, `sound::system2`,
  ...
  acceptance по фактическим сигнатурам и расширение `utils`; мёртвый effect_sink не требует acceptance.
...
- 🟢 (н) **удалить/свернуть старый `effect_sink` seam.** Аудит 2026-07-19: concrete реализации и
  вызовы `emit` отсутствуют; live native/ds effects идут через catalogue typed deferred wrappers,
  dry-run UI вызывает только pure categories, а spawn использует отдельный project `spawn_sink`.
  Значит `exec_context.sink`, `effect_sink.h` и оставшиеся комментарии — кандидат на удаление, не на
  перенос в catalogue. Если позже replay потребует input recorder, это будет отдельный owning слой.
...
- 🟡 (н) **довести metrics/goals/actions через конфиги** — структура, predicates и effects готовы;
  добавить script costs/priorities и богатое объяснение выбора через обновлённый act call тогда, когда
  появится действие с действительно динамической стоимостью.
...
### `libs/aesthetics` — ECS
- 🟡 (н) **зафиксировать и проверить policy structural changes под MT.** Текущий рабочий контракт:
  ...
  Ближайшее действие не новый scheduler, а документированный contract + debug guard/assert там, где
  он дёшев; полноценный structural command buffer нужен только при втором паттерне.
...
- ✅ **schema detection/version guard уже есть:** component key = hash type name, recursive consteval
  ...
  требовался для текущей стадии. Настоящая migration = преобразование старой схемы в новую; отложить до
  первого реального backward-compatible save, затем добавить явные per-version adapters, не эвристику.
...
- 🟢 (н) **удалить старый `act::effect_sink` seam** — аудит не нашёл implementations/`emit` calls;
...
- ⏸️ **loader budget/pool не нужен без измеренной проблемы:** assets уже имеет отдельный поток и может
  провиснуть на тяжёлом ресурсе. Нужны корректные pending/failed states, диагностика и shutdown;
  дополнительный pool/per-tick slicing, cancellation и priorities отложены.
- 🟡 (qol) migration/versioning metadata при первом несовместимом формате ресурса.
...
- 🔴 (н, ближайшее) **освобождение GPU-ассетов:** `unload_hot` пока только сбрасывает index, а
  `mark_remove/clear_*_storage` не завершают descriptor→placeholder, fence-safe destroy VMA/view и
  возврат slot state в `empty`. Закрыть texture и mesh/buffer симметрично, с reuse-тестом.
