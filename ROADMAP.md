...
1. ✅ **Закрыть узкий devils_script ↔ act registration/effect slice (п.14)** — **ядро готово 2026-07-18.**
   ...
   - ⏳ Остаток п.14: ближайшая чистка — удалить неиспользуемый `effect_sink` seam. Общий
...
Активный ближайший пакет — пункты 1–5; выполнять последовательными проверяемыми slices.
...
1. **GPU asset lifetime + command coverage.** Texture slot при hot-unload после безопасной
   ...
   уничтожаются, а slot становится доступен для reuse. Mesh/buffer нужно убрать из всех пар/списков
   `draw_group`, безопасно уничтожить storage и вернуть slot в `empty`. Рядом пройти и покрыть тестами
   остальные реально нужные draw/dispatch/transfer команды.
...
3. **Узкий spawn slice.** Простые spawn points для автономного появления еды + script-facing
   абстракция выбора точки; координатный `spawn_at` сохранить для явно выбранной позиции. Сложные
   filter/pick/spatial policies отложены.
4. **Player entity + UI intent queue; первый consumer — spawn food мышью.** Mouse input преобразует
   ...
   Lua/UI не получает прямого mutating доступа к world.
...
**Ближайшая попутная чистка:** удалить мёртвый `act::effect_sink` и заменить оставшуюся проверку
`sink == nullptr` явной семантикой dry-run там, где она действительно нужна. Сделать до/вместе с
player-intent slice, чтобы новый mutating путь не опирался на старый seam.
...
14. **Интеграция devils_script ↔ act** — **[необходимо; ядро готово, расширять от потребителя]**.
    ...
    production fallback и отдельная native predicate/action таблица удалены. Остаток — ближайшей
    чисткой удалить неиспользуемые `effect_sink.h`/`exec_context.sink`, а string/object/vector/custom
    codecs добавлять под первый реальный spawn/player/relation consumer. Меж-энтити эффекты уже идут
...
- **Пробелы в тестах** — focused-тесты уже есть у `catalogue` introspection, `mood`, `sound::system2`,
  ...
  acceptance по фактическим сигнатурам и расширение `utils`; удаление effect_sink — ближайшая чистка.
...
- 🟢 (н, ближайшая чистка) **удалить/свернуть старый `effect_sink` seam.** Аудит 2026-07-19: concrete реализации и
