...
| `RND-23` | Stencil effect path | `M–L` | **три среза 2026-08-22:** PF04 использует `D24S8`; selection/outline/debug владеют bit `0x01`, невидимая local-effect proxy и cyan tint — независимым `0x02`. Spatial window добавляет aperture bit `0x04`: stencil-ограниченный fullscreen draw очищает внутри него цвет и reverse-Z depth, затем geometry повторно рисуется с alternate camera и собственной корректной глубиной. Это window proof, не recursive portal: camera transform/oblique clipping/recursion вне scope. Исправлены parser-дефекты color mask (`none`, partial masks, absent blend defaults), покрыто 147 assertions. Добавлены PF03-style `--frames/--dump`; final `scene_color` остаётся `TRANSFER_SRC`, fixed camera освобождает GLFW cursor. A/B framebuffer dumps и четырёхкадровый validation dump чисты. Runtime reference/compare/write masks ещё отсутствуют; затем остаётся asymmetric front/back fixture. |
