...
| `RND-23` | Stencil effect path | `M–L` | **сделано пятью срезами 2026-08-22:** PF04 использует `D24S8`; selection/outline/debug владеют bit `0x01`, local tint — `0x02`, spatial window с локальным depth clear и alternate camera — `0x04`. Core dynamic `StencilReference/CompareMask/WriteMask` задаются обычным config list и runtime step constant; selection channel/read/write меняются на следующей записи command buffer без rebuild. Внутри `material` states хранятся как `array<uint32_t,16>` с `UINT32_MAX` sentinel вместо растущего набора bool-полей; `DEVILS_ENGINE_PAINTER_DYNAMIC_STATE_LIST` в `common.h` единожды связывает human names с `VK_DYNAMIC_STATE_*` и генерирует enum/string/`to_vulkan` mapping с `static_assert`. Финальный fixture одним draw показывает асимметрию: front `replace → 0x10` зелёный, back `invert → 0x30` жёлтый; write mask `0x30` сохраняет нижние effects bits. Перенос selection `0x01→0x08` даёт `AE=0`; write=0 убирает debug mask; compare=0 даёт fullscreen `equal` и гасит `not_equal`. Parser regressions покрыты 14 cases/157 assertions. PF03-style dump, normal cursor при fixed camera и A/B fixture проверены с Vulkan validation; лаборатория закрыта. |
