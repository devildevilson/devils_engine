...
| `RND-49` | Трёхмерные ресурсы render graph | `M` | НАЙДЕНО в `PF03` 2026-08-20 (срез 9): ресурсы графа только двумерные (`imageType` жёстко `e2D`, `extent` двухкомпонентный, глубина 1), а 3D-картинка — естественная форма для таблицы цветокоррекции и понадобится дальше объёмным техникам (froxel-туман `RND-47`, 3D-шум). Обход в площадке — раскладка полосой `N*N x N` с ручной интерполяцией по третьей оси: работает ТОЧНО (тождество через таблицу ≤ 1 уровня), но стоит двух выборок вместо одной и приносит полтексельную ловушку, которая молча даёт правдоподобную неверную картинку (проверено намеренной поломкой: ошибка растёт в 33 раза). `value_type::fixed_3d` в конфиге уже парсится и даже считает размер буфера по трём осям, а `image_extent` третью ось отбрасывает — то есть половина формы уже есть. Нужно: extent по трём осям, `imageType`/`viewType` из глубины, `image3D` в дескрипторах |
| `RND-50` | Пасс графа по требованию, а не каждый кадр | `M` | НАЙДЕНО в `PF03` 2026-08-20 (срез 9): все пассы графа выполняются каждый кадр, и сказать «выполни, когда изменились входы» нечем. Таблица грейда — чистая функция настроек, то есть кэш; перезапекать её каждый кадр не нужно. Цена измерена и не абстрактна: `0.018` мс при `32³` и `0.094` при `64³`, причём во втором случае запекание полностью съедает выигрыш таблицы против аналитического грейда (+0.247 против +0.185). То же понадобится тени статичной геометрии, предвычисленной атмосфере и любому кэшу внутри графа. Смыкается с `RND-31`: «пасс не выполняется вовсе» — это выбор ФОРМЫ графа, а «пасс выполняется, но ничего не делает» — число в UBO, и путать их нельзя |
