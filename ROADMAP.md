...
| `GEN-03` | Reusable C++ building-block facade | `L` | **в основном сделано 2026-08-31.** Встроенные: fill/remap/classify/box_blur + четыре свёртки. Scatter: `group_by` (CSR через подсчёт-префикс-заполнение) и `accumulate` (приватные срезы + слияние по номеру чанка) — оба побитово детерминированы и параллелятся 4.6x/4.2x. Внешние (`originator_primitives`): FastNoise2 `noise_grid`/`noise_at` (дерево узлов = закодированная строка в конфиге) и jc_voronoi `voronoi_label` (gather, kd-запрос) / `voronoi_adjacency` (scatter, соседство из Делоне) / `voronoi_polygons` (scatter, контуры как ПЛАНАРНАЯ СЕТКА С ОБЩИМИ вершинами; проверка — сумма площадей равна площади карты). Нет: эрозия, watershed, spline-коридоры |
