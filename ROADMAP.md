...
- реестры по концернам (act/acumen/mood/aesthetics-serial/demiurg/catalogue) — каркасы готовы и
  ...
  готовы. Остались config-loaded void scripts, arena/SBO payload и общий trace service/UI.
...
| Trace/log + deferred MT-dispatch gameplay-вызовов | `libs/catalogue` | introspection + collect/elect wrappers/executors + live tile pipeline готовы; arena/SBO/config-effect pending |
...
1. **Обобщить ownership deferred system pipeline после второго потребителя** — сейчас tile_frontier
...
Dense deferred journal уже закрывает второй расход: type-erased call хранится в 128-byte inline payload,
а storage размеряется по фактическому budget после select, не по разреженному
`world.index_capacity × local_sequence`; semantic order восстанавливается как
`(key, source, local_sequence, function)`. Перед возвратом к списку отдельно оценить pool barrier после
`cognition`/`collect`/`integrate`/`drives`. `integration + drives` не соединять локальным трюком:
описание этой и следующих независимых пар требует общего scheduler/phase-group контракта.

1. Разделить identity domain tag и переиспользуемую strategy policy.
2. Добавить нейтральные presets в `catalogue::mt::preset`.
3. Спроектировать multi-participant `reserve`/write-set.
4. Добавить общий scheduler для нескольких независимых MT-систем под одним pool barrier
   (`schedule/phase-group`, а не bespoke-пара методов).
5. Первым живым parallel system group запустить вместе `integration + drives`.
6. Добавить benchmark 1/2/4/8 workers с обязательной проверкой одинакового world hash.
...
14. **Завершить интеграцию devils_script ↔ act** — **[необходимо]** · **центральный оставшийся столб**.
    ...
    **Уточнение MT-регистрации эффектов (2026-07-17):** `devils_script::on_effect` НЕ является MT-hook —
    ...
    вида `catalogue::mt::domain<STRATEGY>::fn_traits<&fn,...>::fn_deferred_ptr`. В record-фазе он
    копирует функцию+аргументы в буфер стратегии и НЕ вызывает тело; настоящее тело вызывается executor-ом
    после barrier. Strategy-domain получает не владеющий executor через `set_executor`; executor должен
    быть установлен до MT record-фазы, оставаться неизменным до barrier и быть явно снят/заменён только
    вне параллельного прохода. Trace-domain ОРТОГОНАЛЕН strategy-domain: один отвечает за наблюдение,
...
16. **Меж-энтити взаимодействия — deferred-call эффекта через `catalogue`** — **[необходимо]** ·
    ...
    копирует typed arguments в предаллоцированные `(source_index, local_sequence)` slots.
    ...
    **Зафиксированная семантика составного effect-скрипта (2026-07-17):** все effect-ветки с истинным
    ...
    Ближайшие шаги: (1) загружаемый `act::script_function<void>` для GOAP action, чьи ds building
    blocks зарегистрированы теми же `fn_deferred_ptr`; pipeline уже готов принять несколько
    effect-вызовов из одного script pass; (2) вынести ownership/order commit lanes из project-слайса
    в общий pipeline helper после второго потребителя; (3) arena/SBO payload вместо heap object/call;
    (4) решить границу со старым `effect_sink`; (5) `join` только при реальном gameplay-кейсе.
...
- **Пробелы в тестах** — focused-тесты уже есть у `catalogue` introspection, `mood`, `sound::system2`,
  ...
  Дальше нужны config-loaded void-script acceptance, arena/SBO stress и расширение `utils`.
...
- 🟡 (н) **MT function decorator через domain strategy** — ✅ первый срез реализован в
  ...
  free void-функции, но record-ит owned функцию+аргументы в `catalogue::mt::domain<STRATEGY>` и
  откладывает тело до соответствующей pipeline-фазы.
  Strategy-domain получает executor через `set_executor`; trace-domain остаётся отдельной, независимой
  осью. ds регистрирует именно wrapper; `ds::on_effect` остаётся gameplay-реакцией, не транспортом MT.
- 🟡 (н) **описать strategy как policy type**, например:
...
- ✅ **`local_sequence` как часть детерминированного ключа (первый срез)** — каждый source получает монотонный
  номер своих effect-вызовов в порядке обхода одного скрипта. Порядок прихода задач/atomic append-index
  нельзя использовать как semantic order: они зависят от планировщика. Базовый порядок collect:
  ...
  инжектит source/index и общий для всех strategies sequence; executor имеет dense multi-record journal.
...
- ✅ **тесты первого среза** — mirrored ds-call signature, два независимых ds-effect,
  ...
  1-vs-4 worker bit identity и resume bit identity в live tile_frontier.
