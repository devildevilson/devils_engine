...
6. 🟡 **Следующий performance-кандидат — `sense_tree`.** Сначала снять Release-тайминги отдельных
   catalogue-фаз (`sense.tree`, cognition, apply, `integration+drives`, structural tail) и подтвердить
   долю дерева в полном тике: aggregate benchmark сам по себе ещё не доказывает bottleneck. Если доля
   существенна, разделить задачу на (а) сбор стабильного snapshot позиций и (б) bulk-build kD-tree,
   оценить параллельный gather/build. Отдельно рассмотреть double-buffer: строить дерево следующего тика
   параллельно независимым поздним фазам текущего, но только после явной фиксации семантики snapshot
   (позиции начала текущего тика или конца предыдущего). Любая версия обязана проходить тот же
   1/2/4/8 benchmark с одинаковыми world hash и snapshot bytes; детерминизм важнее ускорения.
