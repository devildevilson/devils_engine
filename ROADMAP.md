...
- ✅ **image Стадия 1** (`visage::image` + `nk.image` + `nk.placement` + host `app.image`) + **кодировка id** (тип/mirror/индекс) + **Стадия 2** (стенсил cooldown/mix через маск-текстуру, `tex[16]`). ⏳ далее: `nearest`-семплер для масок; composite/геральдика (type=3); demiurg-ресурс как источник картинки; полный descriptor_indexing (>16 слотов).
