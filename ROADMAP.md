...
| `RND-38` | Мип-цепочки у ресурсов render graph | `M` | НАЙДЕНО в `PF03` 2026-08-19: ресурсы однослойные по мипам (`res_mips = 1` жёстко), поэтому bloom-пирамида собрана из ОТДЕЛЬНЫХ ресурсов на уровень с размерами через `declare_values` + `scale`. Работает и читается, но мип-цепочка одним ресурсом дешевле по памяти, дескрипторам и числу объявлений; понадобится любому эффекту с пирамидой (bloom, SSR, depth pyramid для `RND-20`) |
