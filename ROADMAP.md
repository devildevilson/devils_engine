...
| `RND-23` | Stencil effect path | `M–L` | **первый срез 2026-08-22:** отдельный PF04 executable использует `D24S8`, выбранная geometry пишет static reference 1 после reversed-Z depth test, expanded back-face shell проверяет `!=1` и даёт outline, fullscreen triangle проверяет `==1` и визуализирует маску без sampled stencil. Кадр проверен визуально, Vulkan validation чиста. Config уже проводит static front/back ops/masks/reference в pipeline; runtime reference/compare/write masks ещё отсутствуют (dynamic state сейчас знает только depth bias). Осталось: локальный post-effect на отдельном bit, window mask, dynamic masks/reference и намеренно различимые front/back operations. |
