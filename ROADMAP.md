...
1. **Детерминизм — только в пределах одной ОС и одного билда.** Кросс-платформенная/кросс-компиляторная
   ...
   [[determinism-replication-architecture]]).
2. **Карточная очередь — ПРОЕКТНАЯ система, не движковая.** `catalogue` collect/elect — это гарантия
   MT-ИСПОЛНЕНИЯ, а карточный пайплайн — проектная система вычисления игрового порядка (однопоточная,
   упорядоченная, реактивная). От движка тут требуется ТОЛЬКО (a) инструментарий и (b) поддержка чётких
   переходов состояний. Не тащить сюда catalogue-MT как механизм очереди и не пытаться обобщить пайплайн
   в движок.
3. **`cardgame` = ВТОРОЙ живой gameplay-потребитель**: минимальное headless-ядро уже подключено отдельным
   ...
   Переиспользуемая в движок часть УЖЕ, чем казалось: движковая дельта CG-1 — только resumable step-каркас
   и presentation checkpoints; UI-инструментарий взаимодействия (CG-4/CG-13), tooltip+
   глубина в painter (CG-7), шаблонная сцена-гексокарта (CG-10). Сам боевой пайплайн, реакции, ресурс,
   preview-логика и AI — проектные.
...
**[проект]** проставлена по уточнениям автора (2026-07-21): движок даёт ИНСТРУМЕНТЫ и чёткие переходы
состояний, боевой пайплайн/реакции/ресурс/preview/AI — проектные.
...
- **CG-2. Вычисление порядка внутри карточного пайплайна** — 🟢 **[необходимо] · [проект]**. Ничего
  ...
  это гарантия MT-исполнения, а НЕ механизм этой очереди. От движка нужны только чёткие переходы состояний
  (CG-1) и трассировка. Источник: combat_pipeline §«Стабильный порядок», §«Общие правила очереди».
...
Остаток CG-1 — проектный: записать ds карты в сериализуемую последовательность тактов, заменить локальный
fake-render реальными broker-каналами render/flow и расширить стабильный приоритет (CG-2). Ниже — контракт,
по которому проект продолжает наполнять уже живой каркас.
...
2. **CG-5** (ресурс + предпросмотр как параллельное S+1) — прямая ближайшая цель дизайна: сравнить ману vs
   две инициативы на одном наборе 30–40 карт.
3. **CG-13 + CG-4** (переиспользуемый UI-list слева + инструментарий выделения/навигации + прокидка
...
4. **CG-6/CG-7/CG-8** — по мере появления карт, требующих реакций/поля боя/предпросмотра S+1.
5. **CG-10/CG-11/CG-12/CG-14** — на «Этапе 2» (вертикальный срез забега): гекс-сцена, resume, профиль,
...
gameplay-FSM + ход/действие (CG-1), UI-инструментарий (CG-4/CG-13), tooltip+глубина в painter (CG-7),
шаблонная гекс-сцена (CG-10); боевой пайплайн и очередь остаются проектными. Полный контент-объём
