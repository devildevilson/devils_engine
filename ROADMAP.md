...
| `RND-23` | Stencil effect path | `M–L` | **четыре среза 2026-08-22:** PF04 использует `D24S8`; selection/outline/debug владеют bit `0x01`, local tint — `0x02`, spatial window с локальным depth clear и alternate camera — `0x04`. Painter теперь поддерживает core dynamic `StencilReference/CompareMask/WriteMask`: material объявляет states, step ссылается на runtime-константу из трёх uint, все graphics command paths вызывают `vkCmdSetStencil*`; несогласованные объявления падают громко. PF04 меняет selection channel/read/write следующей записью command buffer без rebuild. Перенос `0x01→0x08` даёт `AE=0`; write=0 убирает debug mask; compare=0 даёт fullscreen `equal` и гасит `not_equal`. Color-mask/parser regressions и dynamic parsing покрыты 14 cases/155 assertions. PF03-style dump, `TRANSFER_SRC` final layout и normal cursor при fixed camera работают; validation чиста. Остался asymmetric front/back fixture. |
