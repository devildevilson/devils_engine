...
- **CG-1. Фазовый gameplay-FSM боя в C++ + понятия ход/действие** — 🟡/🔴 **[grouped срез готов] · [движок: каркас
...
- **CG-2. Вычисление порядка внутри карточного пайплайна** — 🟢 **[group-order срез готов] · [проект]**. Ничего
...
**✅ Первый порядок как иерархия групп реализован и проверен scenario tests (2026-07-22); TODO — наполнить
группы реальными rule/status programs и только после этого фиксировать API.** Перед расширением текущего
`cardgame` ещё раз пройти весь расчёт урона и
...
1. ✅ **Первый CG-1/CG-3 kernel** — новый растянутый по кадрам FSM, grouped project-owned effect program,
   ...
   identity уже проверены. Сериализуемый action cursor теперь проходит ActionBegin, обе party follow-up
   группы, source ActorStateTick, countdown boundary, зеркальные enemy groups и ActionEnd; кража, forced
   cycle token и resume проверены `cardgame_action_pipeline_test`. Следом наполнить эти границы реальными
   rule/status programs, заменить hard-coded typed recipes ds-картами и подключить broker вместо fake render.
