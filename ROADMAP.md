...
16. **Меж-энтити взаимодействия — deferred-call эффекта через `catalogue`** — **[необходимо]** ·
    ...
    **Зафиксированная семантика составного effect-скрипта (2026-07-17):** все effect-ветки с истинным
    ...
    shipped `goap/actor` и проверяет 1-vs-4 bit identity. Ближайшие шаги: (1) вынести ownership/order
    commit lanes из project-слайса в общий helper после второго потребителя; (2) решить границу со старым
    `effect_sink`; (3) member/custom bounded codecs; (4) `join` только при реальном gameplay-кейсе.
