...
12. **A-3 / A-4 — окно + настройки → общий devils_engine (app-shell слой)** — **[необходимо]**.
    ...
    **Foundation готов 2026-07-13:** `simul::app_runtime<Traits>` владеет broker/системами/jthread,
    `standard_broker` и стандартные sound/assets/render контракты вынесены, window/settings/loading
    helpers и optional render/sound работают. Осталось убрать project boilerplate traits/bindings,
    сделать assets optional по факту и определить расширяемый список систем поверх фиксированных 4 slots.
...
- 🟡 (н) закончить границу движковых событий и project bindings; обобщить фиксированные
  main/render/assets/sound slots; довести optional assets (render/sound уже optional).
