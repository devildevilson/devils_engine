...
| `RND-41` | Взрослый замер экспозиции | `M` | планируется в `PF03` срезом 8: гистограмма с перцентилем вместо log-среднего (устойчивость к выбросам), центровзвешенность, АСИММЕТРИЧНАЯ адаптация (к яркому секунды, к тёмному десятки секунд — как глаз) и exposure-волюмы с min/max. Это же механизм «эффекта тёмной комнаты»: узкий проход работает визуальным барьером именно потому, что экспозиция глобальная и авторски ограниченная. Локальный tone mapping эффект ЛОМАЕТ и поэтому в кинематографичных проектах не берётся |
