...
// Алгоритм: контрольный Visage path декодирует тип bindless texture id. Для текста median RGB MTSDF
// превращается в coverage с derivative-based screen range; payload управляет bold/outline/softness.
// Solid ветка нужна обычным Nuklear panels, поэтому один pass показывает UI и тот же atlas, что в мире.
