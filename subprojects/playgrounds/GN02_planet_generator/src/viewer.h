...
struct viewer_options {
  ...
  // Ширина ядра заливки карты в шагах решётки: масштаб, на котором сглаживаются границы областей.
  // Меньше — угловатее, ближе к решётке; больше — плавнее. Верхний предел не вкусовой: за 1.244
  // одноклеточная область пропадает с карты. Перебирается кнопкой в окне.
  float border_smoothing = 1.10f;
  ...
  bool uncapped = false;
  // Пороги географии нужны просмотрщику ровно для того же, для чего отчёту: синтез названий выбирает
  // по ним слово («остров» против массива, «море» против океана).
