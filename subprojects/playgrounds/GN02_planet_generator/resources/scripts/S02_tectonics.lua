...
--
-- ХЕШ ПРИХОДИТ ИЗ ДВИЖКА (таблица `base`), а не пишется здесь: те же функции считают случайность в
-- остальном движке, поэтому одно зерно даёт согласованный результат везде, а не «свой splitmix в
-- каждом теле шага». Своего состояния у них нет — число выводится из аргументов, то есть из зерна
-- шага и номера элемента, и запрет на `math.random` этим не обходится.
--
-- Приведение к целому обязательно: поля буфера читаются как числа lua, то есть double, а хеш работает
-- с целыми. Метка «3» из поля — это 3.0, и без tointeger умножение уехало бы во float, где старшие
-- биты теряются.
local function mix(a, b)
  return base.prng64_2(math.tointeger(a) or 0, math.tointeger(b) or 0)
end

-- Значение в [0, 1) из хеша: нормализация тоже движковая — она собирает double из старших бит
-- напрямую, а не делит на 2^53 руками.
local function unit(a, b)
  return base.prng64_normalize(mix(a, b))
end
