...
public:
  ...
  // Длина цикла, через который взаимное положение двойной и планеты повторяется, в ЦЕЛЫХ годах.
  uint32_t cycle_years() const;
  ...
  struct calendar_time {
    uint32_t year = 0;        // абсолютный номер года от начала отсчёта
    uint32_t cycle_year = 1;  // год внутри повторяющегося цикла, от единицы
  ...
  // Обратное преобразование: год цикла (от единицы), сутки года (от нуля) и час.
  double from_calendar(const uint32_t cycle_year, const uint32_t day, const double hour) const;
