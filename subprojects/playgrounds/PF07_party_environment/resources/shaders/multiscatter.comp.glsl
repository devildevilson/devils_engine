...
const int pf07_multiscatter_directions = 64;
const int pf07_multiscatter_steps = 20;
...
void main() {
  ...
  float radius;
  float mu_sun;
  pf07_multiscatter_from_uv(uv, ground_radius, top_radius, texel_count, radius, mu_sun);
  ...
  for (int index = 0; index < pf07_multiscatter_directions; ++index) {
    ...
    const float azimuth = 2.0 * pf07_pi * u2;
    const vec3 direction = vec3(sine * cos(azimuth), cosine, sine * sin(azimuth));

    const float ground_hit = pf07_sphere_hit(position, direction, ground_radius);
    const float top_hit = pf07_sphere_hit(position, direction, top_radius);
    const float march_length = ground_hit > 0.0 ? ground_hit : top_hit;
    if (march_length <= 0.0) continue;

    const float step_length = march_length / float(pf07_multiscatter_steps);
    vec3 throughput = vec3(1.0);
    for (int i = 0; i < pf07_multiscatter_steps; ++i) {
      const vec3 point = position + direction * ((float(i) + 0.5) * step_length);
      const pf07_medium medium = pf07_sample_medium(sky_data.sky, length(point) - ground_radius);
      ...
      const vec3 sun_transmittance =
        pf07_transmittance_to_light(sky_data.sky, transmittance_lut, point, sun_direction);
    ...
    if (ground_hit > 0.0) {
      ...
      const vec3 sun_transmittance =
        pf07_transmittance_to_light(sky_data.sky, transmittance_lut, ground_point, sun_direction);
      second_order += throughput * ground_albedo * sun_transmittance * cosine_sun / pf07_pi;
  ...
  second_order /= float(pf07_multiscatter_directions) * 4.0 * pf07_pi;
  scatter_fraction /= float(pf07_multiscatter_directions);
