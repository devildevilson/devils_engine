// Освещение поверхности PF07: единственное описание того, как свет неба и светил ложится на предмет.
//
...
#ifndef PF07_SURFACE_GLSL
#define PF07_SURFACE_GLSL

#include "pf07_atmosphere.glsl"
#ifndef PF07_SURFACE_NO_SHADOWS
#include "pf07_shadow_sample.glsl"
#endif
...
vec3 pf07_scene_to_planet(const pf07_sky_block sky, const vec3 scene_position) {
...
vec3 pf07_sky_illuminance(const sampler2D sky_view_lut, const vec3 normal) {
  ...
  vec3 total = texture(sky_view_lut, pf07_sky_view_uv(normal, size)).rgb;
  ...
  total += texture(sky_view_lut, pf07_sky_view_uv(normal * straight + tangent * tilt, size)).rgb;
  total += texture(sky_view_lut, pf07_sky_view_uv(normal * straight - tangent * tilt, size)).rgb;
  total += texture(sky_view_lut, pf07_sky_view_uv(normal * straight + bitangent * tilt, size)).rgb;
  total += texture(sky_view_lut, pf07_sky_view_uv(normal * straight - bitangent * tilt, size)).rgb;
  return total * (pf07_pi / 5.0);
}
...
int pf07_shadow_slot(const pf07_sky_block sky, const float body_code) {
...
vec3 pf07_surface_illuminance(const pf07_sky_block sky, const sampler2D transmittance_lut,
  ...
  for (int s = 0; s < PF07_STAR_COUNT; ++s) {
    ...
    const vec3 light_transmittance =
      pf07_transmittance_to_light(sky, transmittance_lut, planet_point, light_direction);
