...
// потребителя — геометрия и аналитическая земля — обязаны читать карту ОДНИМ кодом.
//
...
float pf07_cascade_visibility(const int index, const vec3 world_position, const vec3 normal) {
  ...
  const vec3 offset_position = world_position + normal * texel_world * 1.5;
...
float pf07_shadow_visibility(const int slot, const vec3 world_position, const vec3 normal,
  ...
  float visibility = pf07_cascade_visibility(index, world_position, normal);
  ...
  if (has_next && view_distance > splits.z) {
    ...
    visibility = mix(visibility, pf07_cascade_visibility(index + 1, world_position, normal), blend);
  }
