...
return function(step)
  ...
  for attempt = 1, p.prop_attempts do
    if written >= capacity then
      -- ГРОМКИЙ ОТКАЗ, А НЕ ТИХАЯ ОБРЕЗКА, как и у всякой объявленной ёмкости в этой площадке: у
      -- обрезанного чанка часть вех просто не появится, а память мира привязана к номеру попытки —
      -- то есть мир молча станет другим при том же ключе. Ёмкость объявлена под весь диапазон
      -- `prop_attempts`, поэтому дойти сюда можно только сузив её вручную.
      error(string.format("chunk wants more than %d entities: raise prop_capacity or lower prop_attempts",
                          capacity))
    end

    -- Столбец выбирается ХЕШЕМ, а не перебором сетки: сетка дала бы видимую регулярность на швах,
    -- потому что кандидаты в каждом чанке начинались бы от его начала. Узлы самих границ исключены:
    -- веха на границе принадлежала бы двум чанкам сразу.
    --
    -- ДИАПАЗОН СИММЕТРИЧЕН, и это не косметика. Здесь было `cells - 2`, то есть узлы 2..cells-1: от
    -- ближней границы веха отступала на одну клетку, а от дальней на две, и в мире появлялась
    -- полоса без вех шириной в три клетки вдоль каждого шва — ровно та регулярность на швах, из-за
    -- которой столбцы и выбираются хешем. Теперь узлы 2..cells, то есть по клетке с каждой стороны.
    local x = 2 + math.tointeger(math.floor(random(attempt, 1) * (cells - 1)))
    local z = 2 + math.tointeger(math.floor(random(attempt, 2) * (cells - 1)))
