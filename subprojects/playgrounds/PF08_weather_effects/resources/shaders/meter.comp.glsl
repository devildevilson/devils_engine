...
const int pf08_meter_samples_per_thread = 4;
const uint pf08_meter_threads = 256;

shared float partial_sum[pf08_meter_threads];
...
void main() {
  ...
  float sum = 0.0;
  for (int y = 0; y < pf08_meter_samples_per_thread; ++y) {
    for (int x = 0; x < pf08_meter_samples_per_thread; ++x) {
      const vec2 offset =
        ((vec2(float(x), float(y)) + 0.5) / float(pf08_meter_samples_per_thread) - 0.5) / 16.0;
  ...
  partial_sum[index] = sum / float(pf08_meter_samples_per_thread * pf08_meter_samples_per_thread);
  barrier();

  for (uint stride = pf08_meter_threads / 2; stride > 0; stride >>= 1) {
    if (index < stride) partial_sum[index] += partial_sum[index + stride];
    barrier();
  }

  if (index == 0) {
    const float mean_log = partial_sum[0] / float(pf08_meter_threads);
