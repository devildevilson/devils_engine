...
const int pf08_aerial_steps_per_slice = 3;
  ...
  for (int slice = 0; slice < size.z; ++slice) {
    ...
    if (segment > 0.0) {
      ...
      const vec3 segment_scattering =
        pf08_march_scattering(sky_data.sky, transmittance_lut, multiscatter_lut,
                              origin + direction * (slice_distance - segment), direction, segment,
                              pf08_aerial_steps_per_slice, segment_transmittance);
