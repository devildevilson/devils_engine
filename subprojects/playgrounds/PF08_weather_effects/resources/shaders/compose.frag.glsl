...
const vec3 pf08_luma_weights = vec3(0.2126, 0.7152, 0.0722);
...
vec3 apply_night_vision(const vec3 color, const float scene_luminance, const float strength) {
  ...
  const vec3 rod_response = vec3(dot(color, pf08_luma_weights)) * vec3(0.82, 0.94, 1.24);
...
void main() {
  ...
  const float scene_luminance = dot(radiance, pf08_luma_weights);
  ...
  const float grey = dot(color, pf08_luma_weights);
