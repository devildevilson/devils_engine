...
void main() {
  ...
  const float ground_distance = pf08_ground_hit(origin, view_direction, planet_radius, camera_height);
  ...
  if (ground_distance <= 0.0) {
    ...
    const vec4 sampled = texture(sky_view_lut, pf08_sky_view_uv(view_direction, sky_view_size));
  ...
  if (ground_distance <= 0.0) {
    ...
    for (int s = 0; s < PF08_STAR_COUNT; ++s) {
      ...
      const float solid_angle = pf08_disc_solid_angle(angular_radius);
