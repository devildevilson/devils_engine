...
void main() {
  ...
  const vec3 direction = pf08_sky_view_direction(uv, texel_count);
  ...
  const float ground_distance = pf08_sphere_hit(origin, direction, ground_radius);
  if (ground_distance > 0.0) {
    imageStore(sky_view_image, pixel, vec4(0.0, 0.0, 0.0, 1.0));
    return;
  }

  const float march_length = pf08_sphere_hit(origin, direction, top_radius);
  vec3 view_transmittance;
  const vec3 in_scattering = pf08_march_scattering(sky_data.sky, transmittance_lut, multiscatter_lut, origin,
