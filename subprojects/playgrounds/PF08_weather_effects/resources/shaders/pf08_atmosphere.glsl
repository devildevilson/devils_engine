// Модель среды PF08: единственное описание рассеяния и поглощения.
//
...
#ifndef PF08_ATMOSPHERE_GLSL
#define PF08_ATMOSPHERE_GLSL

#include "pf08_records.glsl"

const float pf08_pi = 3.14159265358979323846;
...
const vec3 pf08_rayleigh_scattering = vec3(5.802e-3, 13.558e-3, 33.100e-3);
const float pf08_mie_scattering = 3.996e-3;
const float pf08_mie_extinction = 4.440e-3;
const vec3 pf08_ozone_absorption = vec3(0.650e-3, 1.881e-3, 0.085e-3);
...
pf08_medium pf08_sample_medium(const pf08_sky_block sky, const float height_km) {
  ...
  pf08_medium result;
  result.scattering_rayleigh = pf08_rayleigh_scattering * rayleigh_density;
  result.scattering_mie = pf08_mie_scattering * mie_density;
  result.extinction = pf08_rayleigh_scattering * rayleigh_density + vec3(pf08_mie_extinction * mie_density) +
                      pf08_ozone_absorption * ozone_density;
  return result;
}

float pf08_rayleigh_phase(const float cosine) {
  return 3.0 / (16.0 * pf08_pi) * (1.0 + cosine * cosine);
}

float pf08_mie_phase(const float cosine, const float g) {
  const float gg = g * g;
  const float denominator = 1.0 + gg - 2.0 * g * cosine;
  return (1.0 - gg) / (4.0 * pf08_pi * max(denominator * sqrt(max(denominator, 1e-6)), 1e-6));
}
...
float pf08_disc_solid_angle(const float angular_radius) {
  const float half_sine = sin(angular_radius * 0.5);
  return 4.0 * pf08_pi * half_sine * half_sine;
}
...
float pf08_sphere_hit_offset(const vec3 origin, const vec3 direction, const float c) {
...
float pf08_sphere_hit(const vec3 origin, const vec3 direction, const float radius) {
  return pf08_sphere_hit_offset(origin, direction, dot(origin, origin) - radius * radius);
}
...
float pf08_ground_hit(const vec3 origin, const vec3 direction, const float radius, const float height) {
  return pf08_sphere_hit_offset(origin, direction, height * (2.0 * radius + height));
}

// Доля света, доходящая от точки до светила: одна выборка вместо вложенного марша.
vec3 pf08_transmittance_to_light(const pf08_sky_block sky, const sampler2D table, const vec3 position,
  ...
  if (pf08_ray_hits_ground(radius, mu, ground_radius)) return vec3(0.0);

  const vec2 size = vec2(textureSize(table, 0));
  return texture(table, pf08_transmittance_uv(radius, mu, ground_radius, top_radius, size)).rgb;
}
...
vec3 pf08_multiscatter(const pf08_sky_block sky, const sampler2D table, const vec3 position,
                       const vec3 light_direction) {
  const float ground_radius = sky.atmosphere_geometry.x;
  const float top_radius = sky.atmosphere_geometry.y;
  const float length_to_centre = max(length(position), 1e-6);
  ...
  const vec2 size = vec2(textureSize(table, 0));
  return texture(table, pf08_multiscatter_uv(radius, mu_sun, ground_radius, top_radius, size)).rgb;
}
...
vec3 pf08_march_scattering(const pf08_sky_block sky, const sampler2D transmittance_table,
  ...
  for (int i = 0; i < steps; ++i) {
    ...
    const pf08_medium medium = pf08_sample_medium(sky, length(point) - ground_radius);
    ...
    for (int s = 0; s < PF08_STAR_COUNT; ++s) {
      ...
      const vec3 light_transmittance =
        pf08_transmittance_to_light(sky, transmittance_table, point, light_direction);
      const float cosine = dot(direction, light_direction);
      const vec3 phase_weighted = medium.scattering_rayleigh * pf08_rayleigh_phase(cosine) +
                                  vec3(medium.scattering_mie * pf08_mie_phase(cosine, mie_g));
      ...
      const vec3 higher_orders = pf08_multiscatter(sky, multiscatter_table, point, light_direction) *
