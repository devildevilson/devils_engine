...
//
// Зависит только от параметров среды, поэтому пересчёт каждый кадр стоит копейки и сразу подхватывает
// ручную смену мутности. В PF08 это нижняя атмосферная граница будущего погодного состояния; облачная
// среда не должна тайком подменяться одним множителем аэрозоля.
...
void main() {
  ...
  float radius;
  float mu;
  pf08_transmittance_from_uv(uv, ground_radius, top_radius, texel_count, radius, mu);
  ...
  for (int i = 0; i < steps; ++i) {
    ...
    optical_depth += pf08_sample_medium(sky_data.sky, sample_radius - ground_radius).extinction * step_length;
  }
