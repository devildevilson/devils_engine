#ifndef DEVILS_ENGINE_PF08_CELESTIAL_H
#define DEVILS_ENGINE_PF08_CELESTIAL_H

// Небесная механика PF08: две звезды P-type двойной, планета вокруг барицентра, произвольное число лун.
//
...
struct planet_config {
  ...
  // Сколько СОЛНЕЧНЫХ суток укладывается в год, целым числом. Это не описка и не упрощение: длина
  // суток — единственный по-настоящему свободный параметр планеты, тогда как год целиком задан
  // орбитой и массами. Поэтому целым делается год, а сутки подгоняются под него.
  //
  // Единица игрового времени — ровно эти солнечные сутки. Прежде ею были ЗВЁЗДНЫЕ сутки в 86400
  // секунд, и часы уползали от солнца на четыре минуты в день, как на Земле: закат приходился на
  // 05:52, а восход на 19:23.
  uint32_t solar_days_per_year = 365;
...
struct body_view {
  ...
  // Незатменённая освещённость БЕЗ горизонта. Восстановить её делением фактической на видимую долю
  // нельзя при полном затмении: обе величины в числителе обращаются в ноль ровно в самой важной фазе.
  double space_unocculted_lx = 0.0;
  ...
  // Для луны — доля каждого звёздного диска, видимая ИЗ ТОЧКИ ЛУНЫ. Общая `occluded_fraction` годится
  // для бюджета света, но не для рисования диска: если Aurin закрыта, Ember продолжает окрашивать
  // луну в оранжевый, и один серый множитель эту информацию уже не восстановит.
  std::array<double, 2> star_visibility{1.0, 1.0};
};
...
public:
  ...
  // Длина солнечных суток в секундах. Величина ПРОИЗВОДНАЯ: год делится на заданное число суток.
  double day_seconds() const;
  // Звёздный оборот планеты в солнечных сутках. Всегда чуть короче единицы: за сутки планета успевает
  // сместиться по орбите, и чтобы светило вернулось на меридиан, ей надо повернуться чуть больше.
  double sidereal_rotation_days() const;
  // Момент ближайшей к нулю полуночи, то есть нижней кульминации главного светила. От него считаются
  // и номер суток, и часы: иначе ноль часов приходится на произвольную фазу вращения.
  double midnight_epoch_days() const;
  // Лучшее целочисленное приближение повторения двойной внутри окна в 24 года. Это BEAT, а не период
  // всей системы: даже двойная возвращается с остаточной ошибкой, а фазы лун в критерий не входят.
  uint32_t binary_beat_years() const;
  double binary_beat_error_days() const;

  // Гражданский календарь. Живёт здесь, а не в трёх местах вывода: эпоха полуночи и длина года
  // принадлежат небесной механике, и повтори их снаружи — часы разойдутся между окном и отчётом.
  struct calendar_time {
    uint32_t year = 1;        // абсолютный номер года от начала отсчёта, от единицы
    uint32_t beat_year = 1;   // положение внутри приближённого ритма двойной, от единицы
    uint32_t day = 0;         // сутки внутри года, от нуля
    uint32_t hour = 0;
    uint32_t minute = 0;
  };

  calendar_time to_calendar(const double time_days) const;
  // Обратное преобразование: абсолютный год (от единицы), сутки года (от нуля) и час.
  double from_calendar(const uint32_t year, const uint32_t day, const double hour) const;
...
private:
  ...
  double day_seconds_ = 86400.0;
  double midnight_epoch_days_ = 0.0;
  uint32_t binary_beat_years_ = 1;
  double binary_beat_error_days_ = 0.0;
