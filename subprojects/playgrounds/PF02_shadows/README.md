...
1. **Поднять плотность дальних каскадов** — единственный настоящий рычаг для контактной зоны на
   дистанции: больше каскадов, выше atlas, меньше `cascade_far`, либо неравные размеры регионов в atlas
   (последнее упирается в п.8 ниже — atlas allocation contract). Screen-space лучи здесь не помогут:
   их длина `0.24 м` на 15–20 м меньше пикселя, это измерено.
