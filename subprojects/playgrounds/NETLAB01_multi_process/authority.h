...
struct lab_schedule {
  ...
  // Authority closes the connection explicitly. The follower then learns of the
  // loss from the transport itself and skips its silence budget entirely.
  uint64_t explicit_close_tick = 20;
  // Authority stops sending WITHOUT closing. Nothing reports anything, so the
  // only thing that can notice is the follower's silence budget.
  ...
  // The run is not over until the scheduled reconnect happened. Without this
  // the authority's exit races the follower's return, and the stand would be
  // reporting the scheduler rather than the protocol.
  uint64_t expected_admissions = 2;
...
inline constexpr uint64_t lab_principal = UINT64_C(0x70726e63'6c616231); // "prnclab1"
...
public:
  explicit authority_run(const lab_schedule& schedule)
    ...
    pending_.reserve(lab_max_intents_per_tick * lab_max_intent_lead);
    journal_.recycle(std::vector<lab_intent_record>(lab_max_intents_per_tick));
  }

  lab_link::listen_outcome listen() {
    return link_.listen_any();
  }
  ...
  bool step() {
    ...
    poll_transport(now);
    drain_messages(now);
    // The clock starts at the first admission. A follower which joins fresh has
    // no state at all, so a session which had already advanced would owe it a
    // baseline transfer -- a real requirement, and the next slice's, not
    // something to fake by pretending tick zero is wherever we happen to be.
    if (!started_) return true;
    pump_transfer();
    if (host_.state.tick >= schedule_.final_tick) {
      if (transfer_active_) return true;
...
private:
  ...
  void on_peer_lost(const net::gns_peer peer, const uint64_t now) {
    ...
    (void)link_.close(peer);
    if (peer_live_ && peer_ == peer) {
      peer_live_ = false;
      hold_current_session(now);
    }
  }
  ...
  // peer controls.
  void hold_current_session(const uint64_t now) {
    if (!has_session_) return;
    const auto status =
      holds_.hold(session_, principal_, epoch_, now, now + schedule_.hold_window_ms);
  ...
  handshake_slot* handshake_for(const net::gns_peer peer) {
    for (auto& value : slots_) {
      if (value.used && value.peer == peer) return &value;
    }
    return nullptr;
  }
  ...
  void on_handshake_message(const net::gns_peer peer, const std::span<const std::byte> bytes,
    ...
    attach_peer(peer, now);
    close_handshake(peer);
    linger_until_ = 0;
    send_grant(now);
    if (resumed_) send_recovery(now);
  }

  void attach_peer(const net::gns_peer peer, const uint64_t now) {
    if (peer_live_ && !(peer_ == peer)) {
  ...
  void send_grant(const uint64_t now) {
    net::reconnect_ticket ticket;
    ticket.session = session_;
    ticket.principal = principal_;
    ...
    verify_.require(net::derive_session_secret(lab_issuer, session_, principal_, mac_,
                                               credential_scratch_, grant.session_secret) ==
                      net::credential_status::accepted,
  ...
  void send_recovery(const uint64_t now) {
    ...
    if (feasibility != net::recovery_feasibility::recoverable) {
      ...
      (void)link_.send(peer_, lab_lane_control, scratch_);
      return;
    }
    ...
    // The transfer owns its bytes: the ring is free to evict that checkpoint
    // while the transfer is in flight, and a transfer holding a borrowed blob
    // would have the ground pulled out from under it.
    transfer_bytes_ = blob;
    transfer_offset_ = 0;
  ...
  void pump_transfer() {
    if (!transfer_active_ || !peer_live_) return;
    for (size_t sent = 0; sent < lab_chunks_per_pass && transfer_active_; ++sent) {
      const auto length =
        std::min(lab_checkpoint_chunk_bytes, transfer_bytes_.size() - transfer_offset_);
      verify_.require(
        encode_chunk(uint32_t(transfer_offset_),
                     std::span<const std::byte>(transfer_bytes_).subspan(transfer_offset_, length),
                     scratch_),
        "checkpoint chunk did not encode");
      const auto result = link_.send(peer_, lab_lane_bulk, scratch_);
      // Bulk backpressure is a status, not a fault: the transfer simply waits.
      if (result != lab_send_result::sent) return;
      ++counters_.chunks_sent;
      transfer_offset_ += length;
      if (transfer_offset_ == transfer_bytes_.size()) transfer_active_ = false;
    }
  }
  ...
  // HERE rather than in the journal, whose duplicate rejection is a fault.
  void admit_intent(const uint64_t tick, const net::intent& value) {
    ...
    for (const auto& held : pending_) {
      if (held.tick == tick && held.value.kind == value.kind) {
  ...
  void advance_tick(const uint64_t now) {
    ...
    const auto tag = journal_.begin(tick, lab_max_intents_per_tick);
    ...
    lab_bundle bundle;
    bundle.tick = tick;
    bundle.intents.reserve(lab_max_intents_per_tick);
    for (const auto& record : batch.records()) bundle.intents.push_back(record.value);
    ...
    if (peer_live_ && !quiet_) send_bundle(bundle);
  }

  void send_bundle(const lab_bundle& bundle) {
    bundle_scratch_.reserve(lab_max_message_bytes);
    verify_.require(encode_bundle(bundle, bundle_scratch_), "bundle did not encode");
    const auto result = link_.send(peer_, lab_lane_control, bundle_scratch_);
  ...
  void apply_schedule(const uint64_t tick, const uint64_t now) {
    ...
    if (schedule_.quiet_from_tick != 0 && tick == schedule_.quiet_from_tick && peer_live_) {
      // No close, no notification: the connection stays nominally up and simply
      // carries nothing.
      quiet_ = true;
      hold_current_session(now);
    }
...
private:
  net::session_refusal_reason admit_join(const net::client_response& response,
                                         net::session_accepted& accepted) {
    // The join credential is an injected policy, and a shared laboratory token
    // is a laboratory policy. It proves the seam exists, nothing more.
    const std::span<const std::byte> expected{
      reinterpret_cast<const std::byte*>(lab_join_token.data()), lab_join_token.size()};
    if (response.credential.size() != expected.size()) return net::session_refusal_reason::identity_rejected;
    if (!net::equal_in_constant_time(response.credential, expected))
      return net::session_refusal_reason::identity_rejected;
    // One follower per laboratory run, so a second joiner is refused for
    // capacity -- a different answer from a rejected identity. A session which
    // is merely retained counts as occupied: minting a new one over it would
    // discard the hold its owner is on their way back to claim.
    if (has_session_ && (peer_live_ || holds_.size() != 0))
      return net::session_refusal_reason::no_capacity;
    has_session_ = true;
    session_ = ++next_session_;
    principal_ = lab_principal;
    fill_accepted(accepted);
  ...
  net::session_refusal_reason admit_resume(const net::client_response& response,
    ...
    if (net::try_decode_credential(response.credential, credential) !=
        net::credential_status::accepted)
      return net::session_refusal_reason::identity_rejected;
    if (!has_session_ || *response.resumed_session != session_)
    ...
    // authority.
    const net::reconnect_expectation expectation{session_, principal_, epoch_};
    ...
    if (!peer_live_) {
      const auto resolved = holds_.resolve(session_, principal_, epoch_, admission_now_);
  ...
  void fill_accepted(net::session_accepted& accepted) const {
    accepted.session = session_;
    accepted.local_peer = uint64_t(admission_peer_.generation);
...
private:
  ...
  net::session_hold_table holds_{4};
  ...
  lab_host host_;
  std::array<handshake_slot, 2> slots_;
  ...
  std::vector<std::byte> transfer_bytes_;
  size_t transfer_offset_ = 0;
  net::gns_peer peer_;
  net::gns_peer admission_peer_;
  uint64_t nonce_seed_ = 0, nonce_counter_ = 0;
  uint64_t session_ = 0, principal_ = 0, epoch_ = 1, next_session_ = 0;
  uint64_t admission_now_ = 0, next_tick_at_ = 0, linger_until_ = 0;
  bool has_session_ = false, peer_live_ = false, quiet_ = false, resumed_ = false;
  bool started_ = false, transfer_active_ = false;
};
