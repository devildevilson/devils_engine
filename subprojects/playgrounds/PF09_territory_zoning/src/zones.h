...
// Кто держит территорию, насколько тут опасно и насколько богато.
//
// Главное решение: ответ НЕСЁТ РОВНО ОДИН УРОВЕНЬ, и не тот, на котором спрашивают. У комнаты нет своего
// уровня преступности — есть у района, и комната его наследует. У района нет своей торговли — она есть у
// поселения. Поэтому запрос это ПОДЪЁМ ПО ВЛОЖЕННОСТИ до носителя, а не поле в каждой записи. Хранить
// преступность у комнаты значило бы завести десять тысяч чисел, которые обязаны меняться вместе, — и
// однажды они разойдутся.
//
// Отсюда же то, ради чего иерархия вообще существует: «местный криминальный авторитет держит район» —
// это утверждение о РАЙОНЕ, и показать на район нужно уметь. Уровни при этом ВПРАВЕ РАСХОДИТЬСЯ: район
// держит одна сила, баронию — другая, и именно в этом расхождении живёт игра.
struct zone_control {
  uint32_t faction = 0;      // ноль — ничей
  uint16_t crime = 0;        // 0..1000
  uint16_t prosperity = 0;   // 0..1000
};

constexpr uint32_t no_control = 0xffffffffu;

// Что именно спрашивают. Поле знает, какой вид зоны его несёт, и подъём идёт до него.
enum class control_field : uint32_t {
  faction,      // район (interior) и владение (political)
  crime,        // район
  prosperity,   // поселение (local) и торговый округ (regional)
  count
};
...
struct zone_sector {
  ...
  std::vector<zone_control> controls;
...
public:
  ...
  // ВНИМАНИЕ ко времени жизни: всё, что ниже возвращает указатель или `span`, смотрит ВНУТРЬ резидентного
  // сектора и живёт ровно до следующего `focus`. Выгрузка уничтожает векторы сектора, и сохранённый
  // `zone_record*` становится висячим — молча, потому что память ещё принадлежит процессу. Через границу
  // `focus` носят КЛЮЧИ, а не указатели.
  ...
  // Кто держит, насколько опасно, насколько богато. Ответ ищется НА ТОМ УРОВНЕ, который его несёт, —
  // подъёмом по вложенности от того места, о котором спросили.
  const zone_record* carrier_of(const zone_key key, const control_field field) const;
  zone_control control_at(const zone_key key, const control_field field) const;
