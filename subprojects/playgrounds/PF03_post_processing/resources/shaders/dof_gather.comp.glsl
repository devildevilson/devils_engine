...
// Алгоритм: gather-аппроксимация рассеяния тонкой линзы. Для пикселя собираются точки диска, но вклад даёт
// не любая соседняя точка, а только та, чей собственный CoC накрывает центр; вес обратно пропорционален
// площади её пятна. Передний и задний планы копятся раздельно: близкое боке может выходить за силуэт, тогда
// как дальний фон отсекается глубиной переднего объекта. Цвет берётся с подходящего LOD пирамиды боке.
...
void main() {
  ...
  const float neighbourhood = textureLod(pyramid_image, uv, float(max(levels - 1, 0))).a;
