...
// Алгоритм: короткий display pipeline для forward-графа. После hardware MSAA resolve alpha показывает долю
// покрытых samples, поэтому по ней геометрия смешивается с процедурным небом. Затем применяется только
// ручная экспозиция, выбранная tone-map кривая, дизер и при необходимости sRGB encode. Экранных эффектов,
// истории и автоэкспозиции здесь нет по построению — это контрольная простая ветвь.
