...
// Алгоритм: screen-space light shafts как радиальный blur яркой маски к положению солнца. По пути собираются
// только samples с нулевой depth, то есть свет, прошедший через видимые разрывы между затенителями; вклад
// экспоненциально затухает вдоль луча. Метод сознательно ничего не знает об off-screen геометрии и полностью
// выключается, когда солнце вне кадра — для этого ограничения нужен shadow-map/volumetric marching.
