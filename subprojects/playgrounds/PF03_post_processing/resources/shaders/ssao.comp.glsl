...
// Алгоритм: stochastic SSAO в половинном разрешении. Вокруг восстановленной мировой точки распределяются
// Hammersley-пробы в screen-space диске, но затенение решается по положению найденной точки над касательной
// плоскостью, а не по одной разнице depth — поэтому плоская поверхность не затеняет себя. Радиус ограничен
// в пикселях для устойчивости на скользящих углах, а per-pixel/per-frame rotation превращает полосы в шум,
// который затем убирают depth-aware blur и temporal accumulation.
