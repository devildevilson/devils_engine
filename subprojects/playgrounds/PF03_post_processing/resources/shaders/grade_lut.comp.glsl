...
// Алгоритм: запекание аналитической цветокоррекции в настоящую 3D LUT. Каждый invocation соответствует узлу
// RGB-куба: координата декодируется shaper'ом в линейный HDR, проходит общий grade, затем кодируется обратно.
// Узлы лежат на концах диапазона, а не в центрах, поэтому нейтральная таблица является точным тождеством при
// аппаратной трилинейной интерполяции. Пасс запускается только при смене поколения настроек.
