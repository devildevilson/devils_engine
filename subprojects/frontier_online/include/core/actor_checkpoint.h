#ifndef FRONTIER_ONLINE_CORE_ACTOR_CHECKPOINT_H
#define FRONTIER_ONLINE_CORE_ACTOR_CHECKPOINT_H
...
// Число секций документа. Публично, потому что построитель дайджеста ОТКАЗЫВАЕТ при недостаточной
// ёмкости вместо того, чтобы расти в горячем цикле: место под секции резервирует вызывающий.
inline constexpr std::size_t actor_checkpoint_section_count = 3;

// Корень причинного состояния — по КАНОНИЧНЫМ байтам документа, вместе с корнями секций.
//
// Живёт здесь, а не у потребителя, по двум причинам. Схема документа приватна для реализации, и
// снаружи её не назвать. И это ровно тот корень, который сравнивают два процесса: считать его
// двумя разными способами на двух сторонах значило бы сравнивать две реализации хеша, а не два
// состояния мира. `buffers` переиспользуются вызывающим, документ остаётся в них после возврата.
[[nodiscard]] bool actor_checkpoint_digest(
  const actor_world_slice& actors,
  const devils_engine::utils::timelines& timelines,
  actor_checkpoint_buffers& buffers,
  devils_engine::network::state_digest_report<std::uint64_t>& output);
