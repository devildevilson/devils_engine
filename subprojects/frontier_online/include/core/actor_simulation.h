#ifndef FRONTIER_ONLINE_CORE_ACTOR_SIMULATION_H
#define FRONTIER_ONLINE_CORE_ACTOR_SIMULATION_H
...
// Параллельный слот в хвосте тика.
//
// Хвост тика уже занят построением kD-дерева следующего тика в пуле, и измерения показали, что
// второй независимый ЧИТАТЕЛЬ мира в этом окне не стоит почти ничего. Клиент кладёт туда сборку
// GPU-инстансов; авторитет не кладёт НИЧЕГО и не платит за презентацию, которой у него нет.
//
// Причинная часть знает про этот слот ровно две вещи: читателя можно позвать с готовым миром и
// номером тика, и он может сказать, сколько записей произвёл (только для метрик). Что именно он
// производит — байты для GPU, дамп для отладки или ничего — её не касается. Именно это и делает
// headless-авторитет возможным: раньше `update` ТРЕБОВАЛ ссылку на упаковщик инстансов, то есть
// причинный тик нельзя было выполнить, не собрав кадр.
//
// Читатель обязан быть только читателем: он работает параллельно с построением дерева и не имеет
// права менять мир (иначе это гонка, а не перекрытие).
class tick_tail_reader {
public:
  virtual ~tick_tail_reader() = default;
  virtual void read(const devils_engine::aesthetics::world& world, uint64_t tick) = 0;
  virtual uint32_t produced() const noexcept = 0;
};
