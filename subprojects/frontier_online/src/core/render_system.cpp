...
template <>
struct blend_traits<::frontier_online::core::actor_instance> {
  using actor_instance = ::frontier_online::core::actor_instance;
...
render_simulation::render_simulation(const size_t frame_time, render_simulation_config config) noexcept : simul::render_system<::frontier_online::core::broker>(frame_time),
...
void render_simulation::set_broker(struct broker* b) {
  simul::render_system<::frontier_online::core::broker>::set_broker(b);
  if (!container) {
    return;
  }
  container->br = b;
