...
bool actor_checkpoint_digest(const actor_world_slice& actors, const utils::timelines& timelines,
                             actor_checkpoint_buffers& buffers,
                             network::state_digest_report<std::uint64_t>& output) {
  if (!write_actor_checkpoint(actors, timelines, buffers)) return false;
  if (output.sections.capacity() < actor_checkpoint_section_count)
    output.sections.reserve(actor_checkpoint_section_count);
  return network::try_murmur64_digest<checkpoint_schema>(
           std::span<const std::byte>(buffers.document), output) ==
         network::state_digest_build_status::built;
}
