...
bool enter_block(tavl::parser& p, const ev_t begin) {
  for (;;) {
    const auto e = p.peek();
    if (e.type == begin) { p.poll_event(); return true; }
    if (e.type == ev_t::eof || e.type == ev_t::not_enought_data) { p.poll_event(); return false; }
    p.poll_event();
  }
}
...
std::vector<std::string> read_key_list(tavl::parser& p) {
  ...
  if (!enter_block(p, ev_t::array_begin)) return out;
  std::string prefix;
  for (;;) {
    const auto e = p.peek();
    if (e.type == ev_t::array_end || e.type == ev_t::eof) { p.poll_event(); break; }
    if (!is_ident(e.type)) { p.poll_event(); continue; } // row_begin/row_end/comma
    const auto [tev, terr] = p.poll_event();
    const std::string tok = p.to_string(tev.token);
    if (tok == "!")        { prefix = "!"; continue; }
    if (tok == "not")      { prefix = "not "; continue; }
...
void parse_metrics(const devils_script::system& sys, tavl::parser& p, goap_config& cfg) {
  if (!enter_block(p, ev_t::array_begin)) return;
  for (;;) {
    const auto e = p.peek();
    if (e.type == ev_t::array_end || e.type == ev_t::eof) { p.poll_event(); break; }
    if (!is_ident(e.type)) { p.poll_event(); continue; } // row_begin/row_end
...
void parse_object(tavl::parser& p, const Consume& consume_field) {
  if (!enter_block(p, ev_t::object_begin)) return;
  for (;;) {
    const auto e = p.peek();
    if (e.type == ev_t::object_end || e.type == ev_t::eof) { p.poll_event(); break; }
    if (!is_ident(e.type)) { p.poll_event(); continue; } // row_begin/row_end
...
void parse_actions(tavl::parser& p, std::vector<goap_action_config>& out) {
  ...
  for (;;) {
    const auto e = p.peek();
    if (e.type == ev_t::array_end || e.type == ev_t::eof) { p.poll_event(); break; }
    if (e.type != ev_t::object_begin) { p.poll_event(); continue; } // разделители между объектами
    goap_action_config a;
    parse_object(p, [&](const std::string& field) {
      if (field == "name")              { const auto [v, err] = p.poll_event(); a.name = p.to_string(v.token); }
      else if (field == "requirements") a.requirements = read_key_list(p);
      else if (field == "next_state")   a.next_state   = read_key_list(p);
      else if (field == "weight_state") a.weight_state = read_key_list(p);
    });
...
void parse_goals(tavl::parser& p, std::vector<goap_goal_config>& out) {
  ...
  for (;;) {
    const auto e = p.peek();
    if (e.type == ev_t::array_end || e.type == ev_t::eof) { p.poll_event(); break; }
    if (e.type != ev_t::object_begin) { p.poll_event(); continue; }
    goap_goal_config g;
    parse_object(p, [&](const std::string& field) {
      if (field == "name")              { const auto [v, err] = p.poll_event(); g.name = p.to_string(v.token); }
      else if (field == "requirements") g.requirements = read_key_list(p);
      else if (field == "goal")         g.goal         = read_key_list(p);
    });
...
void goap_resource::load_cold(const utils::safe_handle_t&) {
  if (sys_ == nullptr) utils::error{}("goap resource '{}': devils_script::system не впрыснут", id);
...
void goap_resource::unload_warm(const utils::safe_handle_t&) { config_ = goap_config{}; }
