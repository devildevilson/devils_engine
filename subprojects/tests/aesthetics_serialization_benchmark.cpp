...
struct b_inventory {
  ...
  gtl::flat_hash_map<int32_t, int32_t> stacks;
};
...
void populate(aesthetics::world& w, const size_t n) {
  for (size_t i = 0; i < n; ++i) {
    ...
    if (i % 2 == 0) {
      w.create<b_name>(e, b_name{"entity_" + std::to_string(i)});
    }
    if (i % 4 == 0) {
      b_inventory inv;
...
row measure(const aesthetics::world& w, const char* mode, aesthetics::serial::sink_policy pol, const int reps) {
  ...
  const double p = ms([&] {
    packed = aesthetics::serial::pack(&w, pol);
                      ...
                      reps);
  const double u = ms([&] {
    aesthetics::world tmp;
    aesthetics::serial::unpack(packed, &tmp);
  },
                      reps);
...
void run(const size_t n) {
  ...
  const double dump_ms = ms([&] {
    raw = aesthetics::serial::dump_world(&w);
  },
                            reps);
...
int main(int argc, char** argv) {
  ...
  if (argc > 1) {
    for (int i = 1; i < argc; ++i) {
      run(size_t(std::strtoull(argv[i], nullptr, 10)));
    }
  } else {
    for (const size_t n : {size_t(1000), size_t(20000), size_t(100000)}) {
      run(n);
    }
  }
  return 0;
