...
double acc_hunger(bench_scope s)   { return s.hunger; }
double acc_strength(bench_scope s) { return s.strength; }

// нативные аналоги скриптов — прямой C++.
bool   native_pred(bench_scope s) { return s.hunger > 0.5; }
double native_num(bench_scope s)  { return s.hunger * 2.0 + s.strength; }
double native_big(bench_scope s)  { return s.hunger * 2.0 + s.strength * 3.0 - s.hunger + s.strength; }
...
void bench(const std::string_view name, const std::size_t iterations, F&& f) {
  ...
  for (std::size_t i = 0; i < iterations; ++i) local += f(i);
...
int main() {
  ...
  bench("native", N, [&](std::size_t i){ return std::uint64_t(native_pred(scope_of(i))); });
  bench("script", N, [&](std::size_t i){
    vm.clear(); vm.set_arg(0, scope_of(i)); pred_c.process(&vm);
    return std::uint64_t(vm.get_return<bool>());
  });

  std::cout << "[number  hunger*2 + strength]\n";
  bench("native", N, [&](std::size_t i){ return std::uint64_t(native_num(scope_of(i))); });
  bench("script", N, [&](std::size_t i){
    vm.clear(); vm.set_arg(0, scope_of(i)); num_c.process(&vm);
    return std::uint64_t(vm.get_return<double>());
  });

  std::cout << "[number-big  hunger*2 + strength*3 - hunger + strength]\n";
  bench("native", N, [&](std::size_t i){ return std::uint64_t(native_big(scope_of(i))); });
  bench("script", N, [&](std::size_t i){
    vm.clear(); vm.set_arg(0, scope_of(i)); big_c.process(&vm);
    return std::uint64_t(vm.get_return<double>());
  });
