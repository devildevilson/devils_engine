...
## Painter campaign: восемь лабораторий
...
`PF01`–`PF07` закрыты. PF07 зафиксировал P-type систему, календарь, атмосферу, экспозицию/colour script,
тени двух светил, proxy-долину и инстансные заросли; его headless `--verify` содержит `41` численную проверку.
Активная площадка — `PF08_weather_effects`: независимый clear baseline, единый weather-state, приземный туман,
конечный облачный слой и дождевой срез 4A закрыты. Объём `160x90x96` получает обе звезды, луны и их каскадные
тени; общий высотный профиль гасит прямой свет на земле, а 3D world-space облака движутся общим направлением
ветра и дают совпадающую движущуюся тень. Композиция идёт до экспонометра. Baseline-проверка запускает только
PF08 против сохранённых PNG PF07, поэтому закрытая площадка больше не переснимается. Дождь имеет persistent
near-pool, current-depth impacts и mid/far extinction; дальше идут снег и видимое укрытие, не меняя PF07.
