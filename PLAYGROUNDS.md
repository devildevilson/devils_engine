...
Активная площадка — `PF08_weather_effects`: независимый clear baseline, единый weather-state и высотно-затухающая
локальная среда в `160x90x32` froxel-сетке закрыты (`64/64` headless-проверок). Объём получает обе звезды,
луны и их каскадные тени, общий высотный профиль гасит прямой свет на земле, а world-space ячейки плотности
движутся общим направлением ветра; композиция идёт до
экспонометра, clear остаётся побитно равен PF07. Дальше идут
пространственная плотность/облака и осадки, не меняя закрытый PF07.
