...
| 2 | `GN02_planet_generator` *(active)* | планета целиком: плиты, суша, климат, сезоны, провинции, морские зоны, культуры, история, пакет на диск | planetary strategy / `PF10` |
...
Семь шагов — топология, тектоника, поверхность, климат, сезоны, люди, области, — и каждый читает то,
...
`37/37` проверок на четырёх зёрнах, `479 мс` на 65536 клеток и `1929 мс` на 262144 в Release.

Три измеренных ограничения `devils_script`, изменившие форму правил: вызов функции не является
операндом выражения; у контекста восемь слотов аргументов (то есть не больше семи `ctx:arg`); **перенос
строки внутри выражения молча меняет смысл программы** — та же формула роста населения в одну строку
дала `0..0.49`, а в две `0.00013..1`, без ошибки. Последнее — заявка на правку самого `devils_script`.
