...
локальная среда в `160x90x32` froxel-сетке закрыты (`60/60` headless-проверок). Объём получает обе звезды,
луны и их каскадные тени, а общий высотный профиль гасит прямой свет на земле; композиция идёт до
