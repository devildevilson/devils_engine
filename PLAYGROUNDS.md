...
`67/67` проверок на пяти зёрнах, `5.5 с` на 262144 клетках (разрешение по умолчанию) и `25.0 с` на
миллионе в Release.
...
Сглаживание (пункт 7) свелось к тому, что вершина сетки берёт ЧЕТЫРЕ ближайшие клетки с весами вместо
одной: с одной у рельефа выходят террасы по клеткам, а у границы полигональная ломаная — и то и другое
показывает решётку Фибоначчи, которой в мире нет. Граница области при этом рисуется ОТДЕЛЬНЫМ ПОЛЕМ,
размытым по графу соседства, и это третья попытка: разность весов двух ближайших клеток обводит каждую
клетку решётки, та же разность между разными областями обводит границу по ломаной, и только усреднение
поля по графу сглаживает саму ФОРМУ границы. Цена — `1380 → 186` кадров без ограничения, то есть там,
где её не видно.

Тактильность (пункт 6): подписи мест на планете отдельным проходом (щиток по направлению, MSDF, два
порога — по размеру на экране и по занятому прямоугольнику, иначе у горизонта подписи налезают и не
читается ни одна) и выделение лучом из камеры в клетку с подсветкой её области. Главное свойство
выполнено: что выделяется и подписывается, определяет ВЫБРАННОЕ ПРЕДСТАВЛЕНИЕ, а уровень объявлен рядом
с самим представлением, а не выведен второй формулой.
