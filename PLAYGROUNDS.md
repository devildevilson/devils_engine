...
для `submarine_coop` (граф маршрутов грубым проходом, кишка чанковым) и 2D/planet reference pipeline
для `MMO`; обе требуют решений уровня проекта, поэтому берутся вместе с проектом, а не раньше него.
