...
`PF01`–`PF06` закрыты. Активная площадка — `PF07`: топология системы выбрана P-type, объём ограничен срезами
1–5 (небесная механика, атмосфера, экспозиция/colour script, тени двух светил, простые ландшафт и заросли).
Срез 1 закрыт 2026-08-25 отдельным headless executable: аналитический Кеплер для двух звёзд и трёх лун, цвет
и световая отдача из спектра Планка, топоцентрические положения, затмения через перекрытие дисков, календарь
событий и `23` численные проверки без единого пикселя. Погода, froxel-объём и экранные эффекты решаются
после среза 5 — здесь или в `PF08`.
