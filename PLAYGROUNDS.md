...
| Порядок | Директория | Наблюдаемый результат | Первый consumer |
| --- | --- | --- | --- |
| 1 | `GN01_generator_contract` *(active)* | буферы/шаги/lua-тела, апертуры, замер уровней исполнения | все проекты с генератором |
...
нативное ядро `3.92` нс/элемент против `108.72` у поэлементного lua. Отсюда правило: lua обходит
только те множества, которые сама перечислила. Не закрыт третий уровень — `devils_script` по именам
полей на той же задаче.
