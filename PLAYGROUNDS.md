...
`66/66` проверок на пяти зёрнах, `5.8 с` на 262144 клетках (разрешение по умолчанию) и `25.5 с` на
миллионе в Release.
Есть и окно (`--view`): сфера, двадцать полей на цифрах, просмотр ПО ШАГАМ генерации (`[`/`]`
...
Итог по площадке: `66/66` проверок на пяти зёрнах, `5.8 с` на 262144 клетках и `25.5 с` на миллионе в
Release, пакет `117 MB`. Остались пункты 6 (названия и выделение областей надписями на поверхности по
образцу `PF10`) и 7 (сглаживание картинки и границ) — работа графическая, данные для неё уже отданы.
