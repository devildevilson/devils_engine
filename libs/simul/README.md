...
`enter(next)`. В `tile_frontier` boot поднимает минимальный UI/Lua и запрашивает GPU-атласы;
после готовности CPU-метрик первого шрифта начинается loading. Только при входе в loading
создается окно, запрашиваются проектные текстуры/звуки/чанки и создается actor world. Переход в
game происходит после готовности стартовых ресурсов и чанков; gameplay update до этого не идет.
