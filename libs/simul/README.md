...
- менять frame time безопасно через mutex scheduler-state; `update()`/`stop_predicate()` выполняются
  без этого mutex и могут вызывать синхронизированные accessor-ы `advancer` без self-deadlock;
...
- ✅ Mutex `advancer` защищает только scheduler-state; `update()`/`stop_predicate()` не выполняются под
  ним. Межсимуляционное общение идёт через lock-free channels/snapshots, а не через общий lock на тик.
