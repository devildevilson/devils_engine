...
inline void standard_prepare_runtime_state(
  ...
  if (runtime_state == nullptr) utils::error{}("runtime state: invalid target handle");
  ...
  for (const auto& id : cfg.resources) {
    ...
    if (s.pending_ui_scope->contains(handle)) continue;
    ...
    if (pre_external_only) s.pending_boot_targets.emplace_back(ref, target);
...
inline bool standard_request_runtime_state(standard_loading_state& s, const demiurg::resource_handle target) {
  if (s.lifecycle.phase() != app_state::game) return false;
  if (target.get<runtime_state_resource>() == nullptr) return false;
  if (target.system == s.current_runtime_state.system && target.hash == s.current_runtime_state.hash) return false;

  if (!s.lifecycle.request_loading()) return false;
...
inline bool standard_ui_resources_ready(const standard_loading_state& s, const bool external_steps_available) {
  return std::all_of(s.pending_ui_resources.begin(), s.pending_ui_resources.end(),
    [external_steps_available](const resource_ref& ref) { return resource_ready(ref, external_steps_available); });
}
