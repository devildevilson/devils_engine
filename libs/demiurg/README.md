...
## Документы в одном файле (`//---`)

`.tavl`-файл может нести несколько документов, разделённых `//---`. Такой файл НЕ становится ресурсом
сам: вместо него появляются суб-ресурсы `path:name` (имя берётся из поля `name` документа) или
`path:index`, а `path` без хвоста не резолвится вовсе.

Три правила, каждое из которых стоило молчаливой ошибки:

- **Отделитель — это СТРОКА**, а не подстрока: `//---`, перед которым на строке нет ничего, кроме
  пробелов. Поиском подстрокой файл резался по УПОМИНАНИЮ отделителя внутри комментария, то есть по
  тексту про формат.
- **Заголовок отделителя принадлежит отделителю.** `//--- имя документа` — это целиком комментарий
  `tavl`, и остаток строки не должен попадать в следующую секцию. Пока попадал, он становился её
  ПЕРВЫМ значением, и документ читался не тем обходом — без единой диагностики.
- **Базовый id исчезает.** Файл с отделителями адресуется только через `:`, и потребитель, который
  ждал `configs/abc`, получает «нет такого ресурса», хотя файл лежит на месте. Поэтому потребителям
  стоит называть это в тексте ошибки (так делает `originator`).

## Resource Path

Ссылка на ресурс, написанная человеком в конфиге или скрипте, и id, которым ресурс адресуется, — не
одно и то же. Перевод между ними живёт в `demiurg/resource_path.h` и один на весь движок:

- `resource_parent_path(id)` — «папка» ресурса: id без последнего сегмента.
- `absolute_resource_path(current_module, path)` — абсолютный id из написанного: снимает расширение,
  сворачивает `.`/`..`, отделяет и приклеивает обратно хвост `:name`.

Относительным считается только путь, начинающийся с точки (`./x`, `../x`), и считается он от папки
того ресурса, внутри которого написан. Всё остальное — уже абсолютный путь от корня модуля, поэтому
обычная ссылка не зависит от того, где лежит ссылающийся файл.

Раньше эти функции лежали в `simul` рядом с lua-биндингами. Место было неверное: правило принадлежит
адресации ресурса, а не lua, и второй потребитель (`originator`, точка входа генератора) немедленно
завёл бы вторую копию — то есть второе мнение о том, что значит `../common/values.tavl`.
...
- давать один общий перевод «написанный путь -> id ресурса» (`resource_path.h`);
...
- `originator` читает через demiurg ТОЧКУ ВХОДА генератора: один id (`generator/planet`) называет
  документы значений и буферов, тела шагов и программы `devils_script`;
