...
public:
  ...
  // Задать/заменить желаемое состояние ресурса. Побеждает последний вызов.
  // Идемпотентно; если ресурс уже в target и не in-flight — запрос игнорируется.
  void request(resource_interface* res, state::values target);
