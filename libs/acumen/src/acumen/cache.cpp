...
size_t plan_key_hash::operator()(const plan_key& k) const noexcept {
  ...
  for (const uint64_t w : k.bits) h = utils::hash_combine(h, w);
...
void solution_cache::clear() noexcept { entries.clear(); hit_count = 0; miss_count = 0; }

size_t solution_cache::size() const noexcept { return entries.size(); }
size_t solution_cache::capacity_entries() const noexcept { return max_entries; }
bool solution_cache::full() const noexcept { return entries.size() >= max_entries; }
size_t solution_cache::hits() const noexcept { return hit_count; }
size_t solution_cache::misses() const noexcept { return miss_count; }
