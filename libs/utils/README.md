...
- `prng_string.h` генерирует случайные строки из заданных алфавитов.
...
- `serializable.h` - CRTP-like удобный интерфейс `to_string/from_string`, `to_binary/from_binary`,
  `dump/load` с выбором JSON по расширению `.json`.
...
Эти примитивы уже стали основой message broker в `tests/tile_frontier`: mailbox для
...
### События, загрузочные стадии и старый actor path
...
- `actor_ref.h` содержит старую схему `message_receiver`/`actor_ref` с отправкой сообщений
  по sequential type id.
- `loader.h`/`load_stage.h` описывают цепочку стадий загрузки с прогрессом и простым
  semaphore-like ожиданием.

`actor_ref` и старые dispatcher-подходы нужно рассматривать как legacy/experimental слой.
В актуальном `tile_frontier` межпоточный контракт уже перенесен на единый broker поверх
`thread::mailbox`, `thread::spsc_queue` и `thread::payload_channel`.
...
- `double_buffer.h` - shared double buffer и `size_buffer` для staging/current данных.
- `interpolation_buffer.h` - triple-buffer-like хранение `prev/cur/staging` с timestamp и
  коэффициентом интерполяции.
- `shared_data_system.h` - обертка над набором буферов и consumer-функций.
- `context_stack.h` - pool контекстов на atomic intrusive stack; требует аудита перед
  активным использованием.
- `perf.h` - измерение длительности вызова функции без встроенного логирования.
...
2. Разделить публичный и experimental API.
   Для `actor_ref`, старых event dispatcher, `context_stack`, части loader-слоя и похожих
   заготовок нужно явно решить: поддерживаем, переносим в `legacy/experimental` или удаляем
   после миграции пользователей.
