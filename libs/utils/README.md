...
механика, которую можно переиспользовать в других местах, ее стоит переносить сюда.
Байтовый codec из `aesthetics` и секционная схема из `network` теперь находятся здесь;
доменные библиотеки отвечают только за свою проекцию состояния.
...
- `serialization.h`, namespace `utils::serial`: little-endian `writer`/`reader`,
  `serialize`/`deserialize` агрегатов, контейнеров и `adapter<T>`, layout fingerprint.
  Writer дописывает в конец `vector<byte>`; для новой записи владелец делает `clear()`.
  `writer(buffer, false)` запрещает рост capacity; `good()==false` означает частичный
  scratch, который нельзя публиковать. Входные spans не должны ссылаться на выходной буфер.
  Reader ограничивает байты, число декодируемых значений и глубину (`decode_limits`),
  принимает только 0/1 для bool/presence. Он изменяет кандидат, а не обеспечивает rollback.
- `state_schema.h`: полный документ из явно версионированных секций с устойчивыми id.
  `write(host, document, section_scratch)` переиспользует обе capacity и допускает рост;
  `try_write` — ограниченный вариант. Декодирование и проверки идут в `Host::staging_type`,
  затем единственный `noexcept` callback публикует готовое состояние. Потенциально падающую
  подготовку runtime нужно делать до публикации. Сортировка map и пользовательские adapters
  могут выделять память: повторное использование байтовых буферов не означает zero-allocation.
  `emit_canonical` и варианты `write` с выходным writer/буферами возвращают `bool`;
  отказ секции распространяется до вызывающего. Вариант `write(host)`, возвращающий vector,
  выдаёт пустой vector при отказе (валидный секционный документ всегда непустой).
  После `writer::fail()` обычные записи и backpatch ничего не меняют.
- `serialization_sink.h`: `seal`/`unseal` для произвольного канонического payload,
  checksum, zstd и необязательное preview. `seal` имеет вариант с двумя переиспользуемыми
  буферами. `unseal` публикует payload и preview только после полной проверки; лимит
  распакованного payload по умолчанию 256 MiB, задаётся последним аргументом.
  Checksum обнаруживает повреждение, но не аутентифицирует отправителя.
- `named_serializer.h` оборачивает JSON через glaze и умеет
...
ECS payload остаётся в `aesthetics::serial::dump_world/stage_world`; композиция полного
checkpoint принадлежит проекту, пример — `tile_frontier/core/actor_checkpoint.h`.
Zpp удалён из исходников и зависимостей сборки. Glaze оставлен: JSON используется в demiurg.
...
### Смягчение движения: easing.h

`easing.h` — header-only, состоит из двух намеренно разделённых половин, и разделение здесь несущее.

**Кривые** (весь набор с easings.net: `in`/`out`/`in_out` для sine, quad, cubic, quart, quint, expo, circ,
back, elastic, bounce плюс linear) переводят долю пройденного ВРЕМЕНИ в долю пройденного пути. Они
описывают переход из A в B за ИЗВЕСТНУЮ длительность: щелчок колеса, открытие панели, перелёт камеры к
выбранной точке. Без начала, конца и продолжительности кривая бессмысленна. Есть `enum class easing` и
`ease(kind, t)`, чтобы выбор кривой лежал в конфиге, а не в вызове конкретного имени; и `tween<T>`,
который ведёт переход и при перенацеливании на ходу берёт за новое начало ТЕКУЩЕЕ значение, а не прежнее.

**Фильтры** нужны для удерживаемого ввода — зажатой клавиши, стика, мыши. У него нет ни цели, ни
длительности, есть только скорость, которая включается и выключается; подставить в кривую нечего, кроме
выдуманного «времени с начала нажатия», и на отпускании всё равно выйдет разрыв. Здесь работают
`approach` (экспоненциальное приближение с периодом полураспада) и `smooth_damp` (критически
задемпфированная пружина с памятью скорости). Оба НЕЗАВИСИМЫ ОТ ЧАСТОТЫ КАДРОВ: наивное
`current += (target - current) * 0.2f` даёт разную плавность на 60 и 144 герцах, то есть разную игру на
разных машинах, и поймать это глазами почти невозможно.

Практическое правило: **дискретное событие с длительностью — кривая; удерживаемый ввод — фильтр.**
Смешивать их — обычная причина камеры, которая «плывёт» и не слушается. Хорошая камера пользуется обоими:
колесо зума ведёт `tween`, а газ панорамирования — `approach` (так сделано в `PF09_territory_zoning`).

Проверки в `utils_general_test`: у каждой кривой закреплены концы (`f(0)=0`, `f(1)=1`) и отсутствие NaN,
`in`/`out` одного семейства зеркальны, «мягкие» семейства монотонны, а у фильтров проверена именно
независимость от частоты кадров — одна секунда даёт `0.9375` и на 30, и на 60, и на 144 герцах.

### Наследование по иерархии и подмены: inherited.h

`inherited.h` — две мелочи, извлечённые из `PF09_territory_zoning`, где первая отвечала на «кто держит
этот район», а вторая была написана там ЧЕТЫРЕ раза подряд.

`nearest_carrier(start, missing, parent_of, carries, max_hops)` поднимается по иерархии до ближайшего
узла, который НЕСЁТ ответ. Смысл в том, что ответ несёт ровно один уровень, и не тот, на котором
спрашивают: у комнаты нет своего уровня преступности, он есть у района. Соблазн держать поле у каждого
узла и «обновлять сверху вниз» — это и есть ошибка: десять тысяч чисел, обязанных меняться вместе,
однажды разойдутся. Подъём ограничен по числу шагов: порченые данные бывают, зависший на них поток —
необязательно.

`override_table<Key, Value>` — разреженные рантайм-подмены поверх данных с диска: дверь заперли, район
отжали, титул узурпировали. Файл говорит НАЧАЛЬНОЕ состояние, игра — текущее, и второе обязано переживать
выгрузку сектора, в котором лежит первое. Отсортированный вектор, а не хеш-таблица: подмен всегда мало,
локальность лучше, а порядок обхода детерминирован — что важно, если по состоянию считается контрольная
сумма. `find` возвращает УКАЗАТЕЛЬ: «подмены нет» и «подменено значением по умолчанию» — разные
утверждения, и стирать это различие нельзя.
