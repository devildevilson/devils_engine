...
namespace utils {
load_stage::load_stage(std::string name) noexcept : name(std::move(name)) {}
...
size_t loader2::counter() const noexcept {
  lock l(_mutex);
  return _counter;
}
size_t loader2::size() const noexcept {
  lock l(_mutex);
  return _stages.size();
}
bool loader2::finished() const noexcept {
  lock l(_mutex);
  return _counter >= _stages.size();
}
std::string_view loader2::stage_name() const noexcept {
  lock l(_mutex);
  return _stage != nullptr ? std::string_view(_stage->name) : std::string_view();
}
...
void loader::add_waiter(thread::semaphore_interface* inter) {
  if (waiters_count >= max_waiters) {
    utils::error{}("Too many waiters for loader '{}'", name);
  }
...
size_t loader::counter() const noexcept {
  return _counter.load(std::memory_order_relaxed);
}
size_t loader::size() const noexcept {
  return _stages.size();
}
bool loader::finished() const noexcept {
  return counter() >= size();
}
std::string_view loader::stage_name() const noexcept {
  return finished() ? std::string_view() : std::string_view(_stages[counter()]->name);
}
...
thread::semaphore_state::values loader::state() const {
  return static_cast<thread::semaphore_state::values>(finished());
}
