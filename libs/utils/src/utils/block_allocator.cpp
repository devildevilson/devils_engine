...
namespace utils {
block_allocator::block_allocator(const size_t memory_size, const size_t block_size, const size_t block_align) noexcept : m_align(std::max(block_align, sizeof(void*))),
...
block_allocator::block_allocator(block_allocator&& move) noexcept : m_align(move.m_align),
...
size_t block_allocator::alignment() const noexcept {
  return m_align;
}
size_t block_allocator::allocation_size() const noexcept {
  return m_block_size;
}
size_t block_allocator::size() const noexcept {
...
block_allocator_mt::block_allocator_mt(const size_t memory_size, const size_t block_size, const size_t block_align) noexcept : m_align(std::max(block_align, sizeof(void*))),
                                                                                                                               m_block_size(utils::align_to(block_size, m_align)),
                                                                                                                               m_current_memory(m_align),
                                                                                                                               m_memory_size(utils::align_to(memory_size, m_align)),
...
block_allocator_mt::block_allocator_mt(block_allocator_mt&& move) noexcept : m_align(move.m_align),
                                                                             m_block_size(move.m_block_size),
                                                                             m_current_memory(move.m_current_memory),
                                                                             m_memory_size(move.m_memory_size),
...
void* block_allocator_mt::allocate() noexcept {
  ...
  while (!m_free_memory.compare_exchange_strong(tmp, nullptr)) {
    ...
    if (m_free_memory.compare_exchange_strong(tmp, next)) {
      return tmp;
    }
...
size_t block_allocator_mt::alignment() const noexcept {
  return m_align;
}
size_t block_allocator_mt::allocation_size() const noexcept {
  return m_block_size;
}
size_t block_allocator_mt::size() const noexcept {
  return m_memory_size;
}
