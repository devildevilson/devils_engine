...
namespace utils {
stack_allocator::stack_allocator(const size_t size, const size_t aligment) noexcept : m_aligment(aligment),
...
stack_allocator::stack_allocator(stack_allocator&& move) noexcept : m_aligment(move.m_aligment),
                                                                    m_size(move.m_size),
...
size_t stack_allocator::offset_of(const void* ptr) const noexcept {
  return static_cast<const char*>(ptr) - m_memory;
}
void* stack_allocator::at(const size_t offset) noexcept {
  return m_memory + offset;
}
const void* stack_allocator::at(const size_t offset) const noexcept {
  return m_memory + offset;
}
size_t stack_allocator::capacity() const noexcept {
  return m_size;
}
size_t stack_allocator::aligment() const noexcept {
  return m_aligment;
}
size_t stack_allocator::size() const noexcept {
  return m_allocated;
}
char* stack_allocator::data() noexcept {
  return m_memory;
}
const char* stack_allocator::data() const noexcept {
  return m_memory;
}

stack_allocator_mt::stack_allocator_mt(const size_t size, const size_t aligment) noexcept : m_aligment(aligment),
                                                                                            m_size(utils::align_to(size, m_aligment)),
                                                                                            m_memory(reinterpret_cast<char*>(operator new(m_size, std::align_val_t{m_aligment}))),
                                                                                            m_allocated(0) {}

stack_allocator_mt::~stack_allocator_mt() noexcept {
  operator delete(m_memory, std::align_val_t{m_aligment});
}

stack_allocator_mt::stack_allocator_mt(stack_allocator_mt&& move) noexcept : m_aligment(move.m_aligment),
                                                                             m_size(move.m_size),
                                                                             m_memory(move.m_memory),
...
void stack_allocator_mt::clear() noexcept {
  m_allocated = 0;
}
size_t stack_allocator_mt::size() const noexcept {
  return m_size;
}
size_t stack_allocator_mt::aligment() const noexcept {
  return m_aligment;
}
size_t stack_allocator_mt::allocated_size() const noexcept {
  return m_allocated;
}
char* stack_allocator_mt::data() noexcept {
  return m_memory;
}
const char* stack_allocator_mt::data() const noexcept {
  return m_memory;
}

fixed_pool_mt::fixed_pool_mt(const size_t size, const size_t block_size, const size_t aligment) noexcept : m_aligment(aligment), m_block_size(utils::align_to(block_size, m_aligment)), m_size(utils::align_to(size, m_aligment)), m_memory(nullptr), m_stack(nullptr) {
...
fixed_pool_mt::fixed_pool_mt(fixed_pool_mt&& move) noexcept : m_aligment(move.m_aligment), m_block_size(move.m_block_size), m_size(move.m_size), m_memory(move.m_memory), m_stack(nullptr) {
...
size_t fixed_pool_mt::size() const noexcept {
  return m_size;
}
size_t fixed_pool_mt::block_size() const noexcept {
  return m_block_size;
}
size_t fixed_pool_mt::aligment() const noexcept {
  return m_aligment;
}
} // namespace utils
} // namespace devils_engine
