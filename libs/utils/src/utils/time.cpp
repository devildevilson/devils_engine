...
void time_log::update_tp() {
  tp = std::chrono::steady_clock::now();
}

trace_time_log::trace_time_log(const std::string_view& str, std::source_location loc) noexcept : str(str), tp(std::chrono::steady_clock::now()), l(loc) {
...
static const std::vector<date_system::month> default_months({date_system::month{"january", 31, 0},
...
static const std::vector<std::string> default_week({"monday", "tuesday", "wednesday", "thursday", "friday", "saturday", "sunday"});
...
timestamp_t date_system::add(const timestamp_t cur, int32_t years, int32_t months, int32_t days) const {
  ...
  while (days != 0) {
    ...
    if (uint32_t(abs_days) >= m_month_days) {
      days = s * int32_t(uint32_t(abs_days) - m_month_days);
    } else {
      days = 0;
...
date date_system::initial_date() const {
  return start_date;
}
std::string_view date_system::month_str(const uint32_t index) const {
  return index < months.size() ? std::string_view(months[index].name) : std::string_view();
}
std::string_view date_system::week_day_str(const uint32_t index) const {
  return index < week.size() ? std::string_view(week[index]) : std::string_view();
}
std::string_view date_system::month_str(const date& d) const {
  return month_str(d.month);
}
std::string_view date_system::week_day_str(const date& d) const {
  return week_day_str(d.week_day);
}
...
bool date_system::is_leap_year(const date& d) const {
...
const date_system& game_date::get() {
  return s;
}
...
void game_date::set_start_date(const date& d) {
  s.set_start_date(d);
}

date_system game_date::s;

size_t app_clock::resolution() {
  return 1000000;
}

app_clock::app_clock(const timestamp_t start_time) noexcept : start_time(start_time), elapsed_time(0) {}
void app_clock::update(const size_t delta) {
  elapsed_time += delta;
}

timestamp_t app_clock::timestamp() const {
...
double app_clock::seconds() const {
  return double(this->timestamp()) / double(resolution());
}

size_t app_timer::resolution() {
  return 1000000;
}
double app_timer::seconds(const int64_t time) {
  return double(time) / double(resolution());
}
...
void app_timer::update(const size_t delta) noexcept {
  elapsed_time += delta;
}
...
std::tuple<timestamp_t, timestamp_t, timestamp_t> app_timer::status() const noexcept {
  return std::make_tuple(timestamp(), start_time, end_time);
}
bool app_timer::elapsed() const noexcept {
  return elapsed_time != 0 && timestamp() >= elapsed_time;
}
timestamp_t app_timer::timestamp() const noexcept {
  return start_time + elapsed_time;
}
int64_t app_timer::diff(const timestamp_t timestamp) const noexcept {
  return int64_t(timestamp) - int64_t(this->timestamp());
}
} // namespace utils
} // namespace devils_engine
