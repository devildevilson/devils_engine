...
typename T::state typed_init(const uint64_t seed) {
  ...
  for (size_t i = 1; i < state_size; ++i) {
    splitmix_states[i] = splitmix64::next(splitmix_states[i - 1]);
  }
  for (size_t i = 0; i < state_size; ++i) {
    new_state.s[i] = splitmix64::value(splitmix_states[i]);
  }
  return new_state;
}
...
typename T::state typed_init_with_p(const uint64_t seed) {
  ...
  splitmix_states[0] = splitmix64::next(splitmix64::state{{seed}});
  for (size_t i = 1; i < T::state_size; ++i) {
    splitmix_states[i] = splitmix64::next(splitmix_states[i - 1]);
  }
  for (size_t i = 0; i < T::state_size; ++i) {
    new_state.s[i] = splitmix64::value(splitmix_states[i]);
  }
...
static uint64_t pcg_output_xsl_rr_rr_64_64(uint64_t state) {
  const auto rot1 = static_cast<uint32_t>(state >> 59u);
  const auto high = static_cast<uint32_t>(state >> 32u);
  const auto low = static_cast<uint32_t>(state);
  uint32_t xored = high ^ low;
  uint32_t newlow = pcg_rotr_32(xored, rot1);
  uint32_t newhigh = pcg_rotr_32(high, newlow & 31u);
  return (static_cast<uint64_t>(newhigh) << 32u) | newlow;
}
...
pcg_rxs_m_xs64unique::state pcg_rxs_m_xs64unique::next(const state& s) {
  state new_s = s;
  new_s.s[0] = new_s.s[0] * PCG_DEFAULT_MULTIPLIER_64 + static_cast<uint64_t>(reinterpret_cast<intptr_t>(&s) | 1u);
  return new_s;
}
...
#  define PCG_128BIT_CONSTANT(high, low) ((static_cast<__uint128_t>(high) << 64) + low)
static __uint128_t pcg_output_rxs_m_xs_128_128(__uint128_t state) {
...
static __uint128_t pcg_output_xsl_rr_rr_128_128(__uint128_t state) {
  const auto rot1 = static_cast<uint32_t>(state >> 122u);
  const auto high = static_cast<uint64_t>(state >> 64u);
  const auto low = static_cast<uint64_t>(state);
  uint64_t xored = high ^ low;
  uint64_t newlow = pcg_rotr_64(xored, rot1);
  uint64_t newhigh = pcg_rotr_64(high, newlow & 63u);
  return (static_cast<__uint128_t>(newhigh) << 64u) | newlow;
}
...
pcg_rxs_m_xs128unique::state pcg_rxs_m_xs128unique::next(const state& s) {
  state new_s = s;
  new_s.s[0] = new_s.s[0] * PCG_DEFAULT_MULTIPLIER_128 + static_cast<uint128_t>(reinterpret_cast<intptr_t>(&s) | 1u);
  return new_s;
}
...
cmwc128::state cmwc128::next(state s) {
  const auto t = 0xff8fa3db04bb588e * static_cast<uint128_t>(s.s[0]) + s.s[1];
  s.s[0] = 0xd81fdde4eba3aae9 * static_cast<uint64_t>(t);
  s.s[1] = (t + 0xadca32a7 * static_cast<uint128_t>(s.s[0])) >> 64;
  return s;
}
