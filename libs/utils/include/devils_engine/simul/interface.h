...
// да нужно определить ровно одну систему которая будет отвечать вообще за все ресурсы на свете
// как минимум есть собственно ресурс и есть информация о нем
// у ресурса как минимум есть состояние готовности
// мьютекс у interface может и не пригодиться вообще (да и наверное не должен так то)

// думаю что начать нужно с того чтобы просто списком определить какие данные потребуются 
// классифицировать данные на несколько групп, например:
// 1. сырые буферы с вершинами и индексами и инстансами (например для интерфейса)
// 2. сырые буферы с данными например позиций 
// 3. буферы заполняемые последовательно (инстансы с индексами данных например)
// 4....
// думаю что неправильно классифицирую, нужно исходить от системы которая требует какие то данные 
// и для нее заполнять... ну то есть это будут структуры вида:
// struct data1 { std::vector<...> vec1, vec2, ...; };
// к этим структурам должен прилагаться механизм синхронизации, который что сделает?
// по идее повесит мьютекс и полностью пробежится по всем нуждающимся 
// так ну положим как нибудь это сделаем, что дальше?
// делаем систему которая обновит как раз одну из структур
// то есть система получит набор компонентов и пробегая их составит data1
// затем этап когда мы закроем хранилище и в другом потоке прочитаем его 
// мы как то должны собрать все данные тика прежде чем что то куда то копировать 
// первое что приходит в голову это мьютекс повесить почти на весь тик
// ну или держать дубликат в который сливать в конце кадра 
// (идея такая себе с учетом что у нас и так и так будет скорее всего дубликат в виде ЦПУ буферов)
// инкрементировать переменную для синхронизации? будет что то вроде пинпонга для двух систем
// если что то третье добавится то все по идее сломается, но нам как будто и нужно только 2 системы
// то есть основная система + рендер или основная система + звук
// со стороны звука и рендера... для звука поди вообще проще и лусше LIFO завести
