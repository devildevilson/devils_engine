...
struct basic_deadline {
  ...
  constexpr bool elapsed(const basic_timestamp<Domain> now) const noexcept {
    return now >= at;
  }
};
...
public:
  ...
  constexpr uint32_t game_ticks() const noexcept {
    return game_ticks_;
  }
  constexpr uint32_t engine_ticks() const noexcept {
...
struct turn_duration {
  uint64_t turns = 0;
};
constexpr turn_index operator+(const turn_index t, const turn_duration d) noexcept {
  return {t.value + d.turns};
}

struct turn_deadline {
  turn_index at{};
  constexpr bool elapsed(const turn_index now) const noexcept {
    return now >= at;
  }
};
...
public:
  ...
  constexpr game_time_scale game_scale() const noexcept {
    return scale_;
  }
  ...
  constexpr void set_turn(const turn_index value) noexcept {
    turn_ = value.value;
  }
...
public:
  ...
  uint32_t hours_per_day() const noexcept {
    return hours_per_day_;
  }
  uint32_t months_per_year() const noexcept {
    return static_cast<uint32_t>(days_in_month_.size());
  }
  ...
  uint64_t seconds_per_day() const noexcept {
    return uint64_t(hours_per_day_) * 60u * 60u;
  }
  uint64_t days_per_year() const noexcept {
    return days_per_year_;
  }
  bool has_calendar() const noexcept {
    return !days_in_month_.empty();
  }
...
enum class calendar_source : uint8_t { game_time,
                                       turn };

calendar_source parse_calendar_source(std::string_view value);
...
public:
  ...
  calendar_fields date(const timelines& clocks) const {
    return policy_.project(now(clocks));
  }
  calendar_source source() const noexcept {
    return source_;
  }
  const calendar_policy& policy() const noexcept {
    return policy_;
  }
  calendar_timestamp epoch() const noexcept {
    return epoch_;
  }
  calendar_step per_turn() const noexcept {
    return per_turn_;
  }
