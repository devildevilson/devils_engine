...
struct list {
  ...
  list(list&& move) noexcept : m_next(move.m_next) {
    move.invalidate();
  }
...
size_t list_count(T* cur) noexcept {
  size_t counter = 0;
  for (auto c = cur; c != nullptr; c = list_next<t>(c)) {
    ++counter;
  }
  return counter;
}
...
struct list {
  ...
  bool empty() const noexcept {
    return m_next == static_cast<const T*>(this) && m_prev == static_cast<const T*>(this);
  }
};
...
size_t list_count(T* cur) noexcept {
  size_t counter = 0;
  for (auto c = cur; c != nullptr; c = list_next<t>(c, cur)) {
    ++counter;
  }
  return counter;
}
