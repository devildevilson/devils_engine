...
// Fixed-block allocator used by engine containers that need stable storage.
...
public:
  ...
  T* create(Args&&... args) {
    if (sizeof(T) > allocation_size()) {
      utils::error{}("Type size {} must be less than or equal to allocation size {}", sizeof(T), allocation_size());
    }
    auto ptr = allocate();
