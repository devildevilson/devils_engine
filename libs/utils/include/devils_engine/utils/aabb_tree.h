...
public:
  ...
  size_t size() const noexcept { return leaf_count_; }
  bool empty() const noexcept { return leaf_count_ == 0; }
  void reserve(const size_t n) { nodes_.reserve(n * 2); }
  void clear() noexcept { nodes_.clear(); free_.clear(); root_ = null; leaf_count_ = 0; }
  ...
  bvh_handle insert(const aabb& box, const T& payload) { return insert_impl(box, payload); }
  bvh_handle insert(const aabb& box, T&& payload) { return insert_impl(box, std::move(payload)); }

  void remove(const bvh_handle h) {
    if (!alive(h) || !is_leaf(h.index)) return;
    detach_leaf(h.index);
    free_node(h.index);
    --leaf_count_;
  }
  ...
  void query(const Shape& shape, Visit&& visit) const {
    ...
    int32_t stack[64]; int sp = 0; stack[sp++] = root_;
    while (sp > 0) {
      const int32_t idx = stack[--sp];
      const node& n = nodes_[idx];
      if (!geom::test_overlaps<UpAxis>(shape, n.box)) continue;
      if (is_leaf(idx)) { visit(n.payload); continue; }
      // защита от переполнения стека (высота ограничена балансировкой; 64 → ~огромное дерево)
      if (sp + 2 <= int(sizeof(stack) / sizeof(stack[0]))) { stack[sp++] = n.child1; stack[sp++] = n.child2; }
      else { query(shape, visit, n.child1); query(shape, visit, n.child2); } // редкий фолбэк на рекурсию
    }
  }
  ...
  void rebuild() {
    ...
    for (const auto& n : nodes_) if (n.height != free_mark && n.child1 == null) leaves.push_back({ n.box, n.payload });
    clear();
    for (auto& l : leaves) insert_impl(l.first, std::move(l.second));
  }
...
private:
  ...
  struct node { aabb box; T payload; int32_t parent; int32_t child1; int32_t child2; int32_t height; uint32_t gen; };

  bool is_leaf(const int32_t i) const noexcept { return nodes_[i].child1 == null; }

  int32_t alloc_node() {
    if (!free_.empty()) { const int32_t i = free_.back(); free_.pop_back(); return i; }
    const int32_t i = int32_t(nodes_.size());
    nodes_.push_back(node{ aabb::empty(), T{}, null, null, null, 0, 0 });
    return i;
  }
  void free_node(const int32_t i) noexcept { ++nodes_[i].gen; nodes_[i].height = free_mark; nodes_[i].parent = null; free_.push_back(i); }

  template <typename U>
  bvh_handle insert_impl(const aabb& box, U&& payload) {
    const int32_t leaf = alloc_node();
    nodes_[leaf].box = box; nodes_[leaf].payload = std::forward<U>(payload);
    nodes_[leaf].child1 = null; nodes_[leaf].child2 = null; nodes_[leaf].height = 0;
    attach_leaf(leaf);
    ++leaf_count_;
  ...
  void attach_leaf(const int32_t leaf) {
    if (root_ == null) { root_ = leaf; nodes_[leaf].parent = null; return; }
  ...
  void detach_leaf(const int32_t leaf) {
    if (leaf == root_) { root_ = null; nodes_[leaf].parent = null; return; }
    ...
    if (grand != null) {
      if (nodes_[grand].child1 == parent) nodes_[grand].child1 = sibling; else nodes_[grand].child2 = sibling;
      nodes_[sibling].parent = grand;
      free_node(parent);
      refit_from(grand);
    } else {
      root_ = sibling; nodes_[sibling].parent = null; free_node(parent);
    }
  ...
  scalar descent_cost(const int32_t child, const aabb& leaf_box, const scalar inherit) const noexcept {
    ...
    if (is_leaf(child)) return merged.surface() + inherit;
  ...
  int32_t balance(const int32_t iA) {
    if (is_leaf(iA) || nodes_[iA].height < 2) return iA;
    const int32_t iB = nodes_[iA].child1, iC = nodes_[iA].child2;
    const int32_t bal = nodes_[iC].height - nodes_[iB].height;

    if (bal > 1) return rotate_up(iA, iC, iB, /*C is child2*/true);   // поднять C
    if (bal < -1) return rotate_up(iA, iB, iC, /*B is child1*/false); // поднять B
    return iA;
  }
  ...
  int32_t rotate_up(const int32_t iA, const int32_t iP, const int32_t iOther, const bool pivot_is_child2) {
    ...
    if (old != null) { if (nodes_[old].child1 == iA) nodes_[old].child1 = iP; else nodes_[old].child2 = iP; }
    else root_ = iP;
  ...
  void query(const Shape& shape, Visit& visit, const int32_t idx) const {
    const node& n = nodes_[idx];
    if (!geom::test_overlaps<UpAxis>(shape, n.box)) return;
    if (is_leaf(idx)) { visit(n.payload); return; }
    query(shape, visit, n.child1); query(shape, visit, n.child2);
  }
