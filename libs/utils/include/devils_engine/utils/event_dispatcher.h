...
void event_dispatcher2<ID>::reserve(const size_t n) {
  ...
  if (type_id >= memory.size()) {
    memory.resize(type_id + 1);
  }
  if (memory[type_id].get() == nullptr) {
    memory[type_id] = std::make_unique<bucket<final_evt_t>>();
  }
...
std::span<const Event_T> event_dispatcher2<ID>::read() const {
  ...
  auto ptr = get_bucket<final_evt_t>();
  if (ptr == nullptr) {
    return std::span<const final_evt_t>{};
  }
...
template <size_t ID>
template <typename Event_T>
void event_dispatcher2<ID>::bucket<Event_T>::clear() {
  data.clear();
}
...
void event_dispatcher2_mt<ID>::reserve(const size_t n) {
  ...
  const size_t type_id = utils::sequential_type_id<ID, final_evt_t>();
  if (type_id > memory.size()) {
    utils::error{}("Missed several types before registration of '{}'???", utils::type_name<final_evt_t>());
  }
  if (type_id == memory.size()) {
    memory.emplace_back(new bucket<final_evt_t>());
  }
  auto ptr = get_bucket<final_evt_t>();
  ptr->data.reserve(n);
}
...
void event_dispatcher2_mt<ID>::submit(Event_T&& event) {
  ...
  const size_t type_id = utils::sequential_type_id<ID, final_evt_t>();
  if (type_id >= memory.size()) {
    utils::error{}("Register a type '{}' before use it in 'event_dispatcher2_mt'", utils::type_name<final_evt_t>());
  }
  auto ptr = get_bucket<final_evt_t>();
