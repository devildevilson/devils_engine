...
constexpr std::string_view get_type_name() noexcept {
  ...
  if constexpr (substr.find(function_type_pattern) == std::string_view::npos) {
    ...
    if constexpr (class_seq_start == 0) {
      return substr.substr(class_char_seq.size());
    }
    if constexpr (struct_seq_start == 0) {
      return substr.substr(struct_char_seq.size());
    };
  }
  return substr;
...
constexpr uint8_t to_u8(const char c) noexcept {
  return uint8_t(c);
}

constexpr uint64_t u8_to_u64_le(const char* data) noexcept {
...
constexpr uint64_t u8_to_u64_le(const uint8_t* data) noexcept {
...
constexpr uint64_t to_u64(const char c) noexcept {
  return uint64_t(to_u8(c));
}
...
constexpr uint64_t murmur_hash64A(const std::string_view& in_str, const uint64_t seed) noexcept {
  ...
  for (size_t i = 0; i < end; i += 8) {
    uint64_t k = u8_to_u64_le(&in_str[i]);
    k *= m;
    k ^= k >> r;
    k *= m;
...
template <typename>
constexpr bool is_optional_impl_v = false;
template <typename T>
constexpr bool is_optional_impl_v<std::optional<T>> = true;
...
consteval detail::template_string_impl<NA + NB - 1> template_string_concat(
  ...
  for (size_t i = 0; i + 1 < NA; ++i) {
    out.value[k++] = a.value[i];
  }
  for (size_t i = 0; i + 1 < NB; ++i) {
    out.value[k++] = b.value[i];
  }
...
consteval auto template_string_cat(
  ...
  if constexpr (sizeof...(NRest) == 0) {
    return template_string_concat(a, b);
  } else {
    return template_string_cat(template_string_concat(a, b), rest...);
  }
}
