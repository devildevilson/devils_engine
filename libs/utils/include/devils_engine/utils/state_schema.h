#ifndef DEVILS_ENGINE_UTILS_STATE_SCHEMA_H
#define DEVILS_ENGINE_UTILS_STATE_SCHEMA_H
...
// Секционный документ состояния, независимый от транспорта и ECS. Стабильные id/version
// задаёт домен; порядок канонический, политика совместимости exact. Decode/validate работают
// только с кандидатом, публикация — один noexcept callback владельца на границе тика.
// Ограниченный путь запрещает рост байтовых буферов, но не аллокации внутри adapters/staging.

namespace devils_engine::utils::serial {

using state_writer = writer;
using state_reader = reader;
...
public:
  static_assert(section_count > 0, "utils::serial::state_schema requires at least one section");
  static_assert(section_count <= std::numeric_limits<std::uint32_t>::max(),
                "utils::serial::state_schema has too many sections");
  static_assert(unique_state_section_ids_v<Sections...>,
                "utils::serial::state_schema section IDs must be unique");
  static_assert(((std::uint32_t(Sections::version) != 0) && ...),
                "utils::serial::state_schema section versions start at one");
  static_assert((requires(const Host& host, Writer& writer, staging_type& staging,
                 ...
                 ...),
                "utils::serial::state_schema section does not satisfy its policy contract");
  ...
  [[nodiscard]] static bool emit_canonical(const Host& host, Sink& sink, ObserveSection observe_section) {
    std::vector<std::byte> payload;
    return emit_impl<false>(host, sink, observe_section, payload);
  }
...
private:
  ...
  static bool emit_impl(const Host& host, Sink& sink, ObserveSection& observe_section,
    ...
    if constexpr (requires { sink.good(); }) {
    ...
    for (const descriptor& value : descriptors_) {
      ...
      if constexpr (requires { payload_writer.good(); }) {
        if (!payload_writer.good()) {
          if constexpr (requires { sink.fail(); }) sink.fail();
          return false;
        }
      }
...
public:
  // Growing convenience path: both byte buffers are retained by the caller. Use try_write
  // below when the prepared byte capacities are a hard limit instead of a reserve hint.
  [[nodiscard]] static bool write(const Host& host, std::vector<std::byte>& output,
                                  std::vector<std::byte>& payload)
    requires std::same_as<Writer, state_writer>
  {
    ...
    auto observe = [](std::uint32_t, std::uint32_t, std::span<const std::byte>) {};
    return emit_impl<false>(host, writer, observe, payload) && writer.good();
  }
  ...
  [[nodiscard]] static bool emit_canonical(const Host& host, Sink& sink) {
    return emit_canonical(host, sink, [](std::uint32_t, std::uint32_t, std::span<const std::byte>) {});
  }

  [[nodiscard]] static bool write(const Host& host, Writer& writer) {
    return emit_canonical(host, writer);
  }

  [[nodiscard]] static std::vector<std::byte> write(const Host& host) {
    std::vector<std::byte> result;
    Writer writer{result};
    if (!write(host, writer)) return {};
