...
public:
  using point = Vec;
  struct entry { point pos; T payload; int32_t next; int32_t prev; uint32_t gen; };

  size_t size() const noexcept { return live_; }
  bool empty() const noexcept { return live_ == 0; }
  const entry& get(const uint32_t i) const noexcept { return entries_[i]; }
...
protected:
  void clear_pool() noexcept { entries_.clear(); free_.clear(); free_flag_.clear(); live_ = 0; }
  void reserve_pool(const size_t n) { entries_.reserve(n); free_flag_.reserve(n); }
  ...
  uint32_t alloc(const point& p, T payload_in) {
    uint32_t idx;
    if (!free_.empty()) {
      idx = free_.back(); free_.pop_back(); free_flag_[idx] = 0;
      entries_[idx].pos = p; entries_[idx].payload = std::move(payload_in);
    } else {
      idx = uint32_t(entries_.size());
      entries_.push_back(entry{ p, std::move(payload_in), -1, -1, 0 });
      free_flag_.push_back(0);
    }
    entries_[idx].next = -1; entries_[idx].prev = -1;
    ++live_;
    return idx;
  }
  void dealloc(const uint32_t idx) noexcept {
    ++entries_[idx].gen; free_flag_[idx] = 1; free_.push_back(idx); --live_;
  }
  grid_handle handle_of(const uint32_t idx) const noexcept { return grid_handle{ idx, entries_[idx].gen }; }

  void link_front(int32_t& head, const uint32_t idx) noexcept {
    entries_[idx].prev = -1; entries_[idx].next = head;
    if (head >= 0) entries_[head].prev = int32_t(idx);
    head = int32_t(idx);
  }
  void unlink(int32_t& head, const uint32_t idx) noexcept {
    const int32_t p = entries_[idx].prev, n = entries_[idx].next;
    if (p >= 0) entries_[p].next = n; else head = n;
    if (n >= 0) entries_[n].prev = p;
  }
  ...
  void visit_chain(const int32_t head, const Shape& shape, Visit& visit) const {
    for (int32_t i = head; i >= 0; i = entries_[i].next)
      if (geom::test_contains<UpAxis>(shape, entries_[i].pos)) visit(entries_[i].payload);
  }

  bool free_pos(const uint32_t idx) const noexcept { return idx < free_flag_.size() && free_flag_[idx] != 0; }
...
public:
  ...
  dense_grid(const point& origin, const cell_coord& dims, const scalar cell_size)
    ...
    size_t n = 1; for (int i = 0; i < Dim; ++i) n *= size_t(dims_[i] > 0 ? dims_[i] : 1);
    heads_.assign(n, -1);
  }

  scalar cell_size() const noexcept { return cell_size_; }
  void clear() noexcept { base::clear_pool(); std::fill(heads_.begin(), heads_.end(), int32_t(-1)); }
  void reserve(const size_t n) { base::reserve_pool(n); }

  grid_handle insert(const point& p, const T& payload) { return insert_impl(p, payload); }
  grid_handle insert(const point& p, T&& payload) { return insert_impl(p, std::move(payload)); }

  void remove(const grid_handle h) {
    if (!base::alive(h)) return;
  ...
  void update(const grid_handle h, const point& p) {
    ...
    if (oc != nc) { base::unlink(heads_[lin(oc)], h.index); base::link_front(heads_[lin(nc)], h.index); }
  }
...
private:
  ...
  int32_t clamp_cell(const scalar v, const int i) const noexcept {
    if (std::isinf(v)) return v < scalar(0) ? 0 : dims_[i] - 1;
  ...
  size_t lin(const cell_coord& c) const noexcept {
    ...
    for (int i = 0; i < Dim; ++i) { idx += size_t(c[i]) * stride; stride *= size_t(dims_[i]); }
    return idx;
  }
...
public:
  ...
  scalar cell_size() const noexcept { return cell_size_; }
  void set_cell_size(const scalar s) { if (!base::empty()) clear(); cell_size_ = s; }
  void clear() noexcept { base::clear_pool(); heads_.clear(); occ_min_ = cell_coord{}; occ_max_ = cell_coord{}; }
  void reserve(const size_t n) { base::reserve_pool(n); }
  ...
  void update(const grid_handle h, const point& p) {
    ...
    const cell_coord nc = cell_of(p);
    base::entries_[h.index].pos = p;
    if (oc != nc) { unlink_from_cell(oc, h.index); link_to_cell(nc, h.index); }
  }
  ...
  void query(const Shape& shape, Visit&& visit) const {
    ...
    for (int i = 0; i < Dim; ++i) box_cells *= double(hi[i] - lo[i] + 1);
    if (box_cells > double(heads_.size())) { // перебор занятых ячеек дешевле
      for (const auto& [c, head] : heads_) {
        bool inside = true;
        for (int i = 0; i < Dim; ++i) if (c[i] < lo[i] || c[i] > hi[i]) { inside = false; break; }
...
private:
  ...
  grid_handle insert_impl(const point& p, U&& payload) {
    ...
    link_to_cell(c, idx);
    if (base::size() == 1) { occ_min_ = c; occ_max_ = c; }
    else for (int i = 0; i < Dim; ++i) { occ_min_[i] = std::min(occ_min_[i], c[i]); occ_max_[i] = std::max(occ_max_[i], c[i]); }
    return base::handle_of(idx);
  }

  cell_coord cell_of(const point& p) const noexcept {
    cell_coord c{}; for (int i = 0; i < Dim; ++i) c[i] = int32_t(std::floor(p[i] / cell_size_)); return c;
  }
  aabb cell_bounds(const cell_coord& c) const noexcept {
    aabb b{}; for (int i = 0; i < Dim; ++i) { b.min[i] = scalar(c[i]) * cell_size_; b.max[i] = scalar(c[i] + 1) * cell_size_; } return b;
  }
  int32_t clamp_axis(const scalar v, const int i, const bool is_min) const noexcept {
    if (std::isinf(v)) return is_min ? occ_min_[i] : occ_max_[i];
  ...
  void unlink_from_cell(const cell_coord& c, const uint32_t idx) {
    ...
    if (it->second < 0) heads_.erase(it); // ячейка опустела
  }
  ...
  struct coord_hash {
    size_t operator()(const cell_coord& c) const noexcept {
      ...
      for (int i = 0; i < Dim; ++i) { h ^= size_t(uint32_t(c[i])); h *= 1099511628211ull; }
      return h;
    }
  };
