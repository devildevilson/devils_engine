...
executor.begin_record(source_capacity, max_effects_per_source);
// worker:
{
...
Детерминированный порядок не может зависеть от worker scheduling или общего
atomic ticket. Каждый source несёт стабильный `source_id` и монотонный
`local_sequence`, назначенный порядком обхода его script-call'ов. Базовый ключ
collect: `(group_key, source_id, local_sequence)`; дополнительные tie-breakers
задаёт strategy. Переполнение sequence/буфера — ошибка, не drop.
...
он заранее размеряет таблицу `(source_index, local_sequence)` и потому сохраняет
несколько эффектов одного source без общей atomic-нумерации.
...
1. Перед MT cognition слайс биндит два executor-а и вызывает `begin_record`.
...
serializer/replay format. Первый срез принимает только free `void` functions с
copyable value-аргументами; `string_view` превращается в owned `string`, остальные
значения копируются. Ссылки запрещены compile-time; сырые указатели внутри
value-handle допустимы лишь при гарантированном lifetime до commit. Member
functions, custom codecs и arena/SBO storage — следующие расширения. Сейчас на
каждый payload создаётся отдельный type-erased объект; детерминированная slot-table
заранее размерена, но zero-allocation record path ещё не готов.
...
- Перевести type-erased payload с per-call heap allocation на переиспользуемую
  arena/SBO, не меняя `(source_index, local_sequence)` и публичный wrapper API.
