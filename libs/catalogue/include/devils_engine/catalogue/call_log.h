...
// и (б) исполнить контейнер позже. Ядро deferred-call механизма взаимодействий (ROADMAP п.16): think-фаза
// записывает вызовы (record), commit-фаза обходит их (dispatch); ГЕЙТ победителей и сам reduce живут в
...
// пишут разные слоты ⇒ MT-запись БЕЗ ЛОКОВ (инвариант map-фазы), dispatch по индексу ⇒ детерминизм
