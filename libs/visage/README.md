...
**Стенсил-эффекты (Стадия 2, 2026-07-06):** `nk.image_gradient{img, mask, fill}` (cooldown — картинка проявляется по градиент-маске, `mask.r<=fill` = проявлено, остальное ×0.35) и `nk.image_mix{comps={≤4 картинки/цвета}, mask}` (4-blend: веса `mask.rgb`+`1-r-g-b`, per-comp картинка-или-цвет, `mask.a`=непрозрачность). Типы `gui_draw_mode` cooldown(4)/mix(5); именованные SDF-поля в `gui_draw_command_t`/`ui_push_t` заменены на generic `uint32 payload[6]` (интерпретация по типу в `ui.frag`); параметры едут тем же `effect_arena`/`userdata`-каналом, что SDF-текст (`ui_image_effect`, `convert()` — диспетчер по типу). Маска и компоненты — индексы в том же bindless (`ui.frag` `tex[8]→tex[16]`). **Bindless v2 (2026-07-06):** asset-текстуры это раздельные `SAMPLED_IMAGE` массив + immutable `SAMPLER`-пул (`sampler_pool=[linear,nearest]`); шейдер собирает `sampler2D(tex[index], samp[sampler_id])`, где `sampler_id` — 4-битное поле в `tex_id`. mix-маска берёт nearest (резкие зоны), cooldown-градиент — linear; контент — по `sampler_id` (дефолт linear). Так выбор семплера per-draw без per-slot immutable (семплеры статичны/малочисленны → пул; слоты динамичны → пара по индексу). GLSL-грабля: собранный `sampler2D` из раздельных допустим ТОЛЬКО в точке использования (в `texture`/`textureSize`) — не вернуть из функции, не в локал, не аргументом; поэтому `S(...)` = `#define`. grad*.png — заглушки (нужен градиент кодом). Тайловый gather сужен до `"textures/grass"`, чтобы маски не попадали в террейн.
