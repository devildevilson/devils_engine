...
НЕ сделано (Стадия 2): стенсил-эффекты (cooldown/4-blend) через маск-текстуру; старые заглушки `nk_image*` в `bindings` (`tree_image_push*`, button-image) не мигрированы на `visage::image`.
