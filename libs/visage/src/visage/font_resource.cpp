...
font_t* font_resource::font() const noexcept {
  return font_ptr.get();
}
...
void font_resource::load_step(const int32_t from, const utils::safe_handle_t& handle) {
  switch (from) {
    case 0: { // ttf → память (через модуль реестра, как любой demiurg-ресурс)
      if (module == nullptr) {
        utils::error{}("font_resource '{}': no demiurg module (resource must come from a registry)", id);
      }
      ttf_bytes = module->load_binary(path);
      if (ttf_bytes.empty()) {
        utils::error{}("font_resource '{}': could not read ttf '{}'", id, path);
      }
    } break;
    ...
    case 1: { // MSDF: атлас (RGBA) + метрики глифов
      ...
      if (fonts.empty()) {
        utils::error{}("font_resource: packer produced no fonts");
      }
      font_ptr = std::move(fonts.front());
      if (img.channels != 4) {
        utils::warn("font_resource: expected 4 channels (mtsdf), got {}", img.channels);
      }
