...
sol::state& system::script_state() noexcept {
  return lua;
}

sol::environment& system::script_env() noexcept {
  return env;
}

std::span<const uint8_t> system::vertices() const noexcept {
  return vertices_;
}

std::span<const uint8_t> system::indices() const noexcept {
  return indices_;
}

std::span<const gui_draw_command_t> system::commands() const noexcept {
  return commands_;
}
...
static void seed_default_theme(nk_color* t) {
  auto set = [t](int i, nk_byte r, nk_byte g, nk_byte b, nk_byte a) {
    t[i] = nk_color{r, g, b, a};
  };
  set(NK_COLOR_TEXT, 175, 175, 175, 255);
  set(NK_COLOR_WINDOW, 45, 45, 45, 255);
  set(NK_COLOR_HEADER, 40, 40, 40, 255);
  set(NK_COLOR_BORDER, 65, 65, 65, 255);
  set(NK_COLOR_BUTTON, 50, 50, 50, 255);
  set(NK_COLOR_BUTTON_HOVER, 40, 40, 40, 255);
  set(NK_COLOR_BUTTON_ACTIVE, 35, 35, 35, 255);
  set(NK_COLOR_TOGGLE, 100, 100, 100, 255);
  set(NK_COLOR_TOGGLE_HOVER, 120, 120, 120, 255);
  set(NK_COLOR_TOGGLE_CURSOR, 45, 45, 45, 255);
  set(NK_COLOR_SELECT, 45, 45, 45, 255);
  set(NK_COLOR_SELECT_ACTIVE, 35, 35, 35, 255);
  set(NK_COLOR_SLIDER, 38, 38, 38, 255);
  set(NK_COLOR_SLIDER_CURSOR, 100, 100, 100, 255);
  set(NK_COLOR_SLIDER_CURSOR_HOVER, 120, 120, 120, 255);
  set(NK_COLOR_SLIDER_CURSOR_ACTIVE, 150, 150, 150, 255);
  set(NK_COLOR_PROPERTY, 38, 38, 38, 255);
  set(NK_COLOR_EDIT, 38, 38, 38, 255);
  set(NK_COLOR_EDIT_CURSOR, 175, 175, 175, 255);
  set(NK_COLOR_COMBO, 45, 45, 45, 255);
  set(NK_COLOR_CHART, 120, 120, 120, 255);
  set(NK_COLOR_CHART_COLOR, 45, 45, 45, 255);
  set(NK_COLOR_CHART_COLOR_HIGHLIGHT, 255, 0, 0, 255);
  set(NK_COLOR_SCROLLBAR, 40, 40, 40, 255);
  set(NK_COLOR_SCROLLBAR_CURSOR, 100, 100, 100, 255);
  set(NK_COLOR_SCROLLBAR_CURSOR_HOVER, 120, 120, 120, 255);
  set(NK_COLOR_SCROLLBAR_CURSOR_ACTIVE, 150, 150, 150, 255);
  set(NK_COLOR_TAB_HEADER, 40, 40, 40, 255);
#ifdef NK_COLOR_KNOB
  set(NK_COLOR_KNOB, 38, 38, 38, 255);
  set(NK_COLOR_KNOB_CURSOR, 100, 100, 100, 255);
  set(NK_COLOR_KNOB_CURSOR_HOVER, 120, 120, 120, 255);
  set(NK_COLOR_KNOB_CURSOR_ACTIVE, 150, 150, 150, 255);
#endif
}
...
static struct nk_rect image_placement_rect(const struct nk_rect& b, const float iw, const float ih, const uint32_t flags) {
  if (!(flags & img_placement::scale_ratio) || iw <= 0.0f || ih <= 0.0f) {
    return b;
  }
  const float s = std::min(b.w / iw, b.h / ih);
  const float w = iw * s, h = ih * s;
  float x, y;
  if (flags & img_placement::left) {
    x = b.x;
  } else if (flags & img_placement::right) {
    x = b.x + b.w - w;
  } else {
    x = b.x + (b.w - w) * 0.5f; // center по умолчанию
  }
  if (flags & img_placement::top) {
    y = b.y;
  } else if (flags & img_placement::bottom) {
    y = b.y + b.h - h;
  } else {
    y = b.y + (b.h - h) * 0.5f;
  }
...
system::system(const font_t* default_font) : default_font(default_font), effect_arena(64 * 1024, 16) {
  ...
  nk_tbl.set_function("push_font", [this](const sol::object arg) {
    ...
    if (arg.is<double>()) {
      ...
      if (font_obj.valid() && font_obj != sol::nil) {
        ...
        if (font_obj.is<demiurg::resource_handle>()) {
          ...
          if (auto* fr = h.get<font_resource>()) {
            picked = fr->font();
          } else {
            static bool warned = false;
            if (!warned) {
              warned = true;
              utils::warn("nk.push_font: 'font' handle does not resolve to a font resource");
            }
          }
        } else {
          static bool warned = false;
          if (!warned) {
            warned = true;
            utils::warn("nk.push_font: 'font' must be a demiurg resource handle (see request('fonts/...'))");
          }
        }
        if (picked != nullptr) {
          base = picked;
      ...
      if (outline.has_value()) {
        ...
        if (col.has_value()) {
          eff.outline_color = pack_color01(col.value());
        }
      }
    }
    ...
    nk_handle h;
    h.id = int(effect_arena.offset_of(e));
  ...
  nk_tbl.set_function("style_default", [this]() {
    nk_style_from_table(ctx.get(), nullptr);
  });
  nk_tbl.set_function("style_from_table", [this](const sol::table& colors) {
    ...
    for (const auto& kv : colors) {
      ...
      if (idx < 0) {
        utils::warn("nk.style_from_table: unknown element '{}'", kv.first.as<std::string>());
        continue;
      }
  ...
  nk_tbl.set_function("image_gradient", [this](sol::table t) {
    ...
    struct nk_rect bounds;
    if (nk_widget(&bounds, ctx.get()) == NK_WIDGET_INVALID) {
      return;
    }
    const struct nk_rect region = img->region[2] != 0
                                    ? nk_rect(float(img->region[0]), float(img->region[1]), float(img->region[2]), float(img->region[3]))
    ...
    nk_handle ph;
    ph.id = prev_ud;
    nk_set_user_data(ctx.get(), ph); // вернуть прежний userdata
  });
  ...
  nk_tbl.set_function("image_mix", [this](sol::table t) {
    ...
    if (comps) {
      for (int i = 0; i < 4; ++i) {
        ...
        if (c.is<visage::image>()) {
          eff.comp[i] = c.as<visage::image>().texture_id;
          eff.is_image |= (1u << i);
        } else if (c.is<sol::table>()) {
          eff.comp[i] = pack_color01(c.as<sol::table>());
        } // RGBA8-цвет
...
void system::set_entry_point(const sol::object& value) {
  if (!value.is<sol::function>()) {
    utils::error{}("visage: entry point must be a lua function");
  }
...
void system::input(const input_snapshot_t& in) {
  ...
  if (in.scroll_x != 0.0f || in.scroll_y != 0.0f) {
    nk_input_scroll(c, nk_vec2(in.scroll_x, in.scroll_y));
  }
  ...
  for (size_t i = 0; i < in.text_count; ++i) {
    nk_input_unicode(c, nk_rune(in.text[i]));
  }
...
bool system::convert() {
  ...
  nk_draw_foreach(cmd, c, cmds.get()) {
    ...
    if (type == gui_draw_mode::msdf) {
      ...
      ui_image_effect eff{};
      if (eoff + sizeof(ui_image_effect) <= effect_arena.size()) {
        eff = *reinterpret_cast<const ui_image_effect*>(effect_arena.at(eoff));
      }
...
nk_buffer* system::cmds_native() const noexcept {
  return cmds.get();
}
