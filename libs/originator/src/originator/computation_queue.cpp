...
// Реализация очереди: проверка целиком до первого вызова и исполнение со слиянием проходов.
//
// ПРОВЕРКА ЗАДАЁТ ОДИН И ТОТ ЖЕ ВОПРОС ТРИЖДЫ — «читает ли следующий проход то, что записал
// предыдущий». Из него выводятся: мёртвая работа (никто не читает и не назван на границе),
// группировка слияния и — в устройственном плане — барьеры. Поэтому ответ здесь один, а не три
// набора правил.
//
// СЛИЯНИЕ — ПЕРЕСТАНОВКА ОБХОДА, а не оптимизация тела: группа подряд идущих `pointwise` вызовов с
// равными диапазонами обходится «плитка, потом все вызовы группы». Размер плитки считается по
// рабочему набору группы, причём поле, которое один вызов пишет, а следующий читает, учитывается ОДИН
// раз — оно и есть то, что слияние оставляет в кэше.
//
// ДИАПАЗОН РАЗРЕШАЕТСЯ В МОМЕНТ ИСПОЛНЕНИЯ, а не при объявлении: у вызова со счётчиком число
// элементов пишет предыдущий элемент этой же очереди, и прочитанное раньше было бы значением с
// прошлого раза.
