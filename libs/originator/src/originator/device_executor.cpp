...
// Исполнитель устройственных очередей: кэш ПЛАНОВ по подписи очереди.
//
// ПОДПИСЬ — ЭТО СТРУКТУРА, А НЕ ЧИСЛА: инструменты, привязки, роды полей и диапазоны. Параметры в неё
// не входят намеренно — они переписываются в push-константу перед каждой отправкой, поэтому смена
// порога не строит второй план. У стримингового генератора один и тот же шаг зовётся в каждом чанке,
// и без этого кэша каждый чанк заводил бы буферы устройства и наборы дескрипторов заново.
//
// СЧЁТЧИКИ ПОСТРОЕНИЙ И ПЕРЕИСПОЛЬЗОВАНИЙ ОТДАЮТСЯ НАРУЖУ затем, чтобы переиспользование можно было
// ПРОВЕРИТЬ: план, строящийся заново на каждый вызов, считает ровно то же самое и молчит.
