...
void tool_registry::add_graph_tools() {
  ...
  // УСТРОЙСТВЕННОГО ТЕЛА У НЕГО НЕ БУДЕТ, и это тоже ИЗМЕРЕННЫЙ отказ, но по другой причине, чем у
  // `sphere_points`. Здесь дело не в спирали, а в ТОЧНОСТИ РАССТОЯНИЯ.
  //
  // Порядок соседей канонический: расстояние, при равенстве — номер клетки. Значит любая другая
  // структура поиска даёт ТОТ ЖЕ граф, пока совпадает МНОЖЕСТВО ближайших. Хост считает расстояния в
  // `double`, шейдер считал бы во `float32`, и весь вопрос в том, далеко ли граница между шестым и
  // седьмым соседом от последнего бита. Замерено на самой решётке (тест «float32 neighbour search»):
  //
  //   16384 клетки — ни одной клетки под одним ULP, самый узкий зазор 116 ULP;
  //   65536 клеток — уже одна клетка под ULP, самый узкий зазор 0.81 ULP.
  //
  // Множества соседей на обоих размерах совпали, но ЗАПАС ИСЧЕЗ при четырёхкратном росте: на миллионе
  // клеток таких клеток будет несколько, и совпадут они или нет — везение, а не свойство. Порт во
  // `float32` означает «граф может отличиться на горстке клеток, и неизвестно, на какой», то есть
  // другой мир под тем же зерном. Считать на устройстве в `double` можно только с `Float64`, которого
  // Vulkan не гарантирует.
  //
  // ЦЕНА ОТКАЗА ИЗМЕРЕНА И МАЛА: 411 мс, 1.4% часов GN02, один вызов в шаге 'topology', приклеиться в
  // прогон не к чему (вокруг `sequential`-заливки). `label_adjacency` дешевле настолько, что в профиле
  // не виден вовсе: вся апертура `scatter` вместе — 2.8%, и половина из них это соседство сферы.
  ...
  add(tool_description{.name = "label_adjacency", .shape = aperture::scatter, .input_count = 3, .output_count = 2,
                       ...
                       // ТЕЛА НЕТ, И ЭТО ЦЕНА ВХОДА, А НЕ ЗАПРЕТ. Отказ здесь не про детерминизм —
                       // работа целочисленная и во `float32` не упирается, — а про ФОРМУ: канонический
                       // CSR это счёт с повторами, префиксная сумма, заполнение, чистка повторов и
                       // ещё одна префиксная сумма со сжатием. Пять проходов, две развёртки и три
                       // временных буфера, которых в объявлении вызова нет: у инструмента сегодня
                       // ОДНО тело и ОДИН проход, а свёртки-развёртки на устройстве в библиотеке нет
                       // вовсе. Делать это надо вместе с формой «инструмент из нескольких проходов»,
                       // а не вместо неё. Цена промедления измерена: в профиле GN02 этот инструмент
                       // не виден — вся апертура `scatter` вместе даёт 2.8% часов.
