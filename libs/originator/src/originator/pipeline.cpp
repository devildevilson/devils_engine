...
// Разбор конфига генератора (`tavl`) и сам пайплайн: построение буферов, проверка привязок, обход
// шагов.
//
// ФОРМА ДОКУМЕНТА КЛАССИФИЦИРУЕТСЯ ДО РАЗБОРА. `tavl` читает документ либо как одну структуру, либо
// как список значений, и строка `values = ...` рядом с блоком `{ ... }` молча превращает его в
// структуру, у которой блоки становятся лишними значениями. Отказа при этом не было — документ просто
// читался не тем обходом, поэтому обе путаницы теперь называются вслух, с именем документа.
//
// ПРОВЕРКА ПАЙПЛАЙНА ИДЁТ ДО ПЕРВОГО ШАГА и отвечает на один вопрос: читает ли шаг то, чего до него
// никто не писал. Исключение ровно одно и оно ОБЪЯВЛЕНО — входы пайплайна, которые заполняет хост;
// невыведенное исключение отличить от опечатки в имени было бы нечем.
//
// БУФЕРЫ ЖИВУТ ДОЛЬШЕ ЧАНКА: `set_chunk` не перевыделяет память, потому что один пайплайн генерирует
// много чанков подряд, и аллокация на каждый чанк была бы платой ни за что.
