...
// Реализация реестра и запуска инструментов: параметры, ссылки на поля, проверки вызова, разбиение
// работы по чанкам.
//
// РАЗБИЕНИЕ ЗАВИСИТ ОТ АПЕРТУРЫ, И ЭТО ЕДИНСТВЕННОЕ МЕСТО, ГДЕ ОНО РЕШАЕТСЯ. `pointwise` и `gather`
// раскладываются по пулу, `reduce` считается чанками ФИКСИРОВАННОГО размера и объединяется строго в
// их порядке (иначе сумма плавающих чисел зависела бы от числа потоков), а `scatter` и `sequential`
// получают весь диапазон целиком и организуют свои фазы сами.
//
// ДИАПАЗОН ОТНОСИТСЯ К РАЗНОМУ У РАЗНЫХ АПЕРТУР: у `pointwise`/`reduce` — и ко входам, и к выходам; у
// `gather` — только к выходам (входы читаются целиком, там и лежат соседи); у `scatter` — только ко
// входам, потому что выход это ДРУГАЯ структура; у `sequential` он покрывает первый выход, а входы
// читаются целиком — у решателя вход это несколько десятков слов правил, а диапазон миллион клеток.
//
// ФОРМА БЕРЁТСЯ У ПРИВЯЗКИ, и только если буфер её не объявил — из параметров по прежнему написанию.
// Оба сразу — громкая ошибка: два источника одной истины однажды разъедутся.
