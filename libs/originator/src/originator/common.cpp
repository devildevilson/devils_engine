...
// Реализация словаря типов: разбор написания поля, перевод компоненты в канонический `double` и
// обратно, имена режимов.
//
// ЗДЕСЬ ЖИВЁТ ЕДИНСТВЕННАЯ ЗАПИСЬ ЗНАЧЕНИЯ В ПОЛЕ (`store_component`), и её повторяют все остальные
// пути — в том числе перевод в GLSL. Две разные записи одного и того же значения дали бы два разных
// поля, а заметно это стало бы только на границах диапазона: у целых родов значение ЗАЖИМАЕТСЯ и
// усекается, а не переполняется молча.
//
// ПОЛОВИННАЯ ТОЧНОСТЬ (`sf`) ПЕРЕВОДИТСЯ ВРУЧНУЮ, без аппаратной инструкции: она нужна на всех
// платформах одинаково, а `_Float16` есть не везде. Нормализованный байт (`c`) — то же самое, только
// проще: 0..1 в 0..255 с округлением.
