...
void run_chunk(const script_program::implementation& impl,
  ...
  // SAVED-СЛОТЫ СБРАСЫВАЮТСЯ НА КАЖДЫЙ ЭЛЕМЕНТ, и без этого программа с `ctx_save` не является
  // pointwise, хотя вся библиотека на это опирается.
  //
  // `context::clear()` сбрасывает операндный стек и базы, но СОДЕРЖИМОЕ saved-стека не трогает, а
  // контекст здесь живёт на ЧАНК. Значит слот, сохранённый на элементе i, физически читается на
  // элементе i+1: программа, читающая слот, который на этом элементе не сохраняла (например
  // `ctx_save` внутри непройденной ветки `value_or`), взяла бы значение ПРЕДЫДУЩЕГО элемента. Граница
  // чанка зависит от числа потоков, поэтому ломалось бы «параллельно == последовательно побитово» —
  // то самое обещание, которое библиотека проверяет тестом.
  //
  // Делается это ровно так, как devils_script делает сам для свежего фрейма ПОД-скрипта (opcode
  // execute чистит типы своего saved-фрейма): пустой тип означает «слот не записан». Данные
  // сбрасываются тоже, потому что система собрана с небезопасными опкодами — чтение слота типа не
  // проверяет, поэтому определённость обязана лежать в самих байтах, а не в типе. Временная мера до
  // релиза ds, где это сделает сам `clear()`.
  //
  // Цена платится ТОЛЬКО программами, которые `ctx_save` действительно используют: у остальных
  // объявлено ноль слотов, и цикл не выполняется ни разу.
