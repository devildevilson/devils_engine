...
// Исполнение `devils_script` над плотным буфером: компиляция против имён привязок и проход по
// диапазону.
//
// СКОУП СКРИПТА — ОДИН ЭЛЕМЕНТ (аксессоры плюс индекс), и он влезает в 16 байт стека ds. Больше в нём
// ничего нет намеренно: из этой пары нельзя добраться ни до соседа, ни до записи, поэтому программа
// СТРУКТУРНО pointwise, а не по договорённости.
//
// SAVED-СЛОТЫ СБРАСЫВАЮТСЯ НА КАЖДЫЙ ЭЛЕМЕНТ. `context::clear()` трогает операндный стек, но не
// содержимое saved-стека, а контекст живёт на ЧАНК: слот, сохранённый на элементе i, физически
// читался бы на элементе i+1. Граница чанка зависит от числа потоков, поэтому ломалось бы
// «параллельно == последовательно побитово» — то самое обещание, которое проверяет тест. Делается это
// ровно так, как ds делает сам для свежего фрейма под-скрипта; цена платится только программами,
// которые `ctx_save` действительно используют.
//
// ТИПЫ ПРОВЕРЯЮТСЯ НА РАЗБОРЕ, а не на исполнении (система собирается с небезопасными опкодами):
// платить за проверку типа стека на каждом из миллионов элементов незачем, а программа, не сошедшаяся
// по типам, до исполнения не доходит вовсе.
...
std::unique_ptr<script_program> script_program::compile(const std::string_view& name,
  ...
  // Небезопасные варианты опкодов: проверка типов остаётся на разборе (см. шапку файла).
...
void run_chunk(const script_program::implementation& impl,
  ...
  // Сброс saved-слотов на каждый элемент (см. шапку файла). Пустой тип означает «слот не записан», и
  // данные сбрасываются тоже: система собрана с небезопасными опкодами, поэтому определённость обязана
  // лежать в самих байтах, а не в типе. Временная мера до релиза ds, где это сделает сам `clear()`.
  const size_t saved_count = std::min(impl.program.max_saved, vm.saved_stack._size);
  ...
  for (size_t i = begin; i < end; ++i) {
    ...
    vm.clear();
    for (size_t slot = 0; slot < saved_count; ++slot) {
      vm.saved_stack._data[slot] = ds::stack_element{};
      vm.saved_stack._types[slot] = std::string_view();
    }
