...
// Точка входа генератора: ОДИН документ, который называет свои части.
//
// Без неё хост обязан знать имена трёх файлов, то есть знать УСТРОЙСТВО чужого генератора — а мод,
// который добавляет свой генератор, назвать эти файлы иначе уже не может. Поэтому точку входа
// называют один раз, а она называет остальное; всё, что здесь написано — demiurg-id, то есть путь от
// корня модуля без расширения (см. demiurg::absolute_resource_path).
//
// Документ ОДИН, и шаги лежат внутри него списком (`steps = [ { ... } { ... } ]`), а не отдельными
// блоками верхнего уровня. Это не оформление: tavl читает документ либо как одну структуру, либо как
// список значений, и строка `values = ...` рядом с блоком `{ ... }` превращает документ в структуру,
// у которой все блоки оказываются лишними значениями.
...
struct pipeline_description {
  ...
  // ВХОДЫ ПАЙПЛАЙНА: буферы, которые заполняет ХОСТ до запуска, а не какой-то шаг.
  //
  // Понятие появилось из ДВУХМАСШТАБНОЙ генерации, и без него она не выражается. Грубый мировой
  // проход считается один раз и отдаёт каркас — маршруты, узлы, сводки; чанковый пайплайн читает из
  // каркаса то, что попадает в его область. Кто именно попадает, знает только хост: он владеет
  // каркасом и его резидентностью, потому что каркас бывает много больше видимого мира. Значит шага,
  // который такой буфер пишет, нет и быть не должно, а правило «читать раньше первой записи нельзя»
  // без объявления входа отклонило бы весь пайплайн.
  //
  // Объявляется вход АВТОРОМ (`inputs = [ ... ]` в точке входа), а не выводится из того, что буфер
  // никто не пишет: именно это библиотека и обязана считать ошибкой, а отличить «привезли извне» от
  // «опечатался в имени» можно только по объявлению.
  //
  // Контракт для хоста: заполненный вход обязан быть ФУНКЦИЕЙ ОБЛАСТИ чанка, а не того, что успело
  // подгрузиться. Неполный вход — это не «чуть хуже», это другой мир: тот же ключ дал бы разный
