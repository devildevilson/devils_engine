...
// (смещение, шаг) один раз, поэтому ни lua, ни devils_script не видят разницы между aos и soa:
// можно переключить раскладку в конфиге и померить, не тронув ни одной строки скрипта.
//
// SoA здесь означает один непрерывный план на ПОЛЕ (компоненты внутри плана чередуются), а не на
// каждую компоненту: поле — это именованная единица, по которой рассуждает автор конфига.
//
// Владение: буфер владеет своей памятью и не меняет размер после создания. Размер объявляется в
// конфиге заранее, потому что генератор должен знать свою стоимость по памяти до запуска.
