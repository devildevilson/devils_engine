...
public:
  ...
  // КОПИЯ ОБЯЗАНА ПЕРЕСЧИТАТЬ `data_`, и написана она поэтому руками.
  //
  // Внутри лежат две вещи сразу: сама память (`memory_`) и выровненное начало ВНУТРИ неё (`data_`).
  // Копия по умолчанию копировала вектор — и тут же копировала указатель, который смотрит в ЧУЖУЮ
  // память: у копии был свой буфер, а писала она в оригинал. Оба при этом оставались валидными, и по
  // результату это невидимо — хуже того, сверка «двух путей» через копию буфера сравнивала бы память
  // с самой собой и всегда сходилась.
