...
struct tool_call {
  ...
  // Привязан ли необязательный вход. Тело обязано спрашивать, а не полагаться на счёт.
  bool has_input(const size_t index) const noexcept;
  // Привязан ли необязательный выход. Тело обязано спрашивать, а не полагаться на счёт: не
...
struct device_param {
  ...
  // параметра занято в GLSL: `remap` принимает `min`/`max`, а `min` и `max` — встроенные функции
  // языка, и поле с таким именем закрыло бы их собой внутри тела. Пусто => имена совпадают.
  std::string symbol;

  // Как параметр называется в шейдере.
...
struct tool_description {
  ...
  // у решателя ограничений это заранее занятые клетки, и заставлять объявлять буфер нулей ради того,
  // что обычно пусто, значило бы платить памятью за отсутствие условия.
  uint32_t optional_inputs = 0;
  uint32_t output_count = 0;
  // Сколько ПОСЛЕДНИХ выходов можно не привязывать. Нужно там, где инструмент считает попутную
  // величину, которая одному потребителю нужна, а другому нет: `voronoi_label` за тот же обход знает
  // и метку, и близость к границе, но требовать второе поле со всех — значит заставить объявлять
  // буфер ради результата, который никто не прочитает. Пропущенный выход в теле проверяется через
  // `tool_call::has_output`.
  ...
  // ФОРМА ИНСТРУМЕНТА НА УСТРОЙСТВЕ: ТЕЛО вычислительного шейдера и порядок параметров в
  ...
  //
  // Здесь лежит именно ТЕЛО, а не весь текст: привязки собирает `build_device_shader`, уже зная
  // ВЫВЕДЕННЫЙ род каждого поля (буфер или картинка, §6.2). Инструмент этого знать не может — род
  // зависит от того, как поле читают ДРУГИЕ вызовы очереди, — поэтому тело написано против
  // аксессоров `in_<i>_at` / `out_<j>_set`, а не против `layout(...) buffer`.
  //
  // Соглашение, одно на все инструменты и все переводы:
  ...
  //
  // ТЕЛО НАТИВНОГО ИНСТРУМЕНТА ПИШЕТСЯ ПРОТИВ `float`, и иначе быть не может: оно написано один раз
  // на все будущие привязки, а какого рода поля к нему придут, автор не знает. Поэтому устройственный
  // план отклоняет нативный инструмент над целым полем — прежде такой вызов молча читал биты `uint`
  // как `float`, потому что текст объявлял `float data[]` у любого поля.
  ...
  // обязано стать картинкой, а всё остальное остаётся буфером и выигрывает по всем пунктам (§6.3).
  std::vector<uint32_t> device_filtered_inputs;

  // ТЕЛО ГОДИТСЯ ДЛЯ ЛЮБОГО ДОПУЩЕННОГО РОДА, а не только для плавающего.
  //
  // По умолчанию устройственный план отклоняет нативный инструмент над целым полем: тело написано
  // ОДИН раз на все будущие привязки, а какого рода поля к нему придут, автор не знает. Этот флаг —
  // объявление автора, что тело написано кинд-агностично; тогда сборщик добавляет переводящие
  // перегрузки аксессоров, и `float`-выражение честно преобразуется в целое, как это делает аксессор
  // на хосте.
  //
  // Цена названа: перевод идёт через `float32`, точный до 2^24, а на хосте то же поле читается через
  // double и точно до конца диапазона. Ставить флаг стоит там, где значение по природе мало или
  // приходит параметром (`fill`), а не там, где полем ходят широкие идентификаторы.
  bool device_integer_ready = false;

  // ЗАПИСИ НЕ ЗАВИСЯТ ОТ ПОРЯДКА. Объявление, которое пускает `scatter` в очередь.
  //
  // Апертура `scatter` отклоняется не за то, что пишет по чужим индексам, а за то, что порядок этих
  // записей задаёт алгоритм. Целочисленное накопление — тот случай, где порядка нет вовсе: сложение
  // целых коммутативно, и результат повторяется прогон в прогон, что у GN04 измерено. Свойство это
  // принадлежит КОНКРЕТНОМУ инструменту, а не апертуре, поэтому и объявляет его инструмент.
  //
  // Плавающее накопление сюда не годится: §4.3 называет порядок сложения float единственным больным
  // местом свёрток, и объявить его независимым от порядка было бы просто неправдой.
...
public:
  ...
  // Решатель ограничений (wave function collapse). Отдельный набор потому, что это не проход по
  // полю, а РЕШЕНИЕ ЗАДАЧИ: он один во всей библиотеке может не найти ответа и попросить ещё попытку.
