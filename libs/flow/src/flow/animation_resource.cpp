...
animation_resource::animation_resource(library* lib, const demiurg::resource_system* resources) : lib_(lib),
                                                                                                  resources_(resources) {
  set_flag(demiurg::resource_flags::warm_and_hot_same, true);
  set_flag(demiurg::resource_flags::binary, false);
}

const std::vector<uint32_t>& animation_resource::state_indices() const noexcept {
  return indices_;
}
...
void animation_resource::load_cold(const utils::safe_handle_t&) {
  ...
  if (is_list_entry() && list_start_line > 0) {
    options.line_offset = list_start_line - 1;
  }
  ...
  if (is_list_entry() && parsed.size() == 1) {
    ...
    if (!next_names.empty() && !next_names[0].empty()) {
      lib_->set_next(index, next_names[0]);
    }
