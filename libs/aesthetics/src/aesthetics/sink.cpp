...
struct le_writer {
  ...
  void raw(const uint8_t* p, const size_t n) {
...
struct le_reader {
  ...
  size_t pos = 0;
  bool bad = false;
  bool can(const size_t n) const noexcept {
    return pos + n <= b.size();
  }
  uint8_t u8() {
    if (!can(1)) {
  ...
  std::span<const uint8_t> take(const size_t n) {
    if (!can(n)) {
      bad = true;
      return {};
    }
...
std::vector<uint8_t> seal(const std::span<const std::byte> payload_raw, const sink_policy& policy, const std::span<const uint8_t> screenshot) {
  ...
  wr.u64(checksum);
  if (with_shot) {
    wr.u32(uint32_t(screenshot.size()));
    wr.raw(screenshot.data(), screenshot.size());
  }
...
bool unseal(const std::span<const uint8_t> data, std::vector<std::byte>& raw_out, std::vector<uint8_t>* screenshot_out) {
  ...
  if (rd.bad || magic != container_magic) {
    utils::warn("snapshot container: bad magic 0x{:08x}", magic);
    return false;
  }
  if (version != container_version) {
    utils::warn("snapshot container: version {} != {}", version, container_version);
    return false;
  }
  ...
  if (flags & flag_screenshot) {
    ...
    if (rd.bad) {
      utils::warn("snapshot container: truncated screenshot");
      return false;
    }
    if (screenshot_out != nullptr) {
      screenshot_out->assign(shot.begin(), shot.end());
    }
  }

  const auto payload = rd.take(payload_size);
  if (rd.bad) {
    utils::warn("snapshot container: truncated payload");
    return false;
  }
  ...
  if (flags & flag_compressed) {
    ...
    if (n == SIZE_MAX || n != raw_size) {
      utils::warn("snapshot container: decompress failed ({} vs {})", n, raw_size);
      return false;
    }
    raw.resize(n);
  } else {
    if (payload.size() != raw_size) {
      utils::warn("snapshot container: raw size mismatch");
      return false;
    }
  ...
  if (got != checksum) {
    utils::warn("snapshot container: checksum mismatch 0x{:016x} vs 0x{:016x}", got, checksum);
    return false;
  }
...
bool unpack(const std::span<const uint8_t> data, world* w, std::vector<uint8_t>* screenshot_out) {
  ...
  if (!unseal(data, raw, screenshot_out)) {
    return false;
  }
  in_t in{raw};
