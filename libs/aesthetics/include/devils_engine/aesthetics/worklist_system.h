...
//
// ИНВАРИАНТ ПОЛОСЫ: полоса — ЧИСТЫЙ БЛОКНОТ. Какому потоку достанется чанк, от запуска к запуску
// разное, поэтому ничто в полосе не имеет права переживать одну сущность и влиять на следующую.
// Полоса существует ради переиспользования ПАМЯТИ, а не ради переноса состояния. Нарушение этого
// инварианта не даёт гонки за данные (память непересекающаяся, санитайзер чист) — оно делает
// невоспроизводимым ВХОД следующей сущности, и симуляция начинает зависеть от планировщика ОС.
// Именно так проявился дефект 2026-09-11: двойное освобождение в acumen::astar замыкало список
// свободных узлов пула в цикл, отравляя полосу до конца её жизни (см. astar.h, free_all).
//
