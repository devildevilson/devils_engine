...
Сейчас query живет как объект и в деструкторе отписывается от событий. После
загрузки снапшота компоненты создаются мимо обычного `world::create`, поэтому
старые query не получают create-событий. Для этого `load_world` в конце шлет
`serial::snapshot_loaded_event`, а владелец систем должен пересобрать свои query
и другие кэши. Технически `query_t`/`lazy_query_t` могут делать это сами:
подписаться на `snapshot_loaded_event`, очистить свой `container` и заново
заполнить его через `view<T...>()` / `lazy_view<T...>()`. Текущий blocker только
структурный: `snapshot_loaded_event` объявлен в `serialization.h`, который
включает `world.h`, поэтому для такой подписки событие нужно перенести в
`common.h` или другой низкоуровневый заголовок.
