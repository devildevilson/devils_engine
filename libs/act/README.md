...
**Блокировки (арбитраж взаимодействий) — НЕ в упаковщике.** self-claim / atomic-min (elect) — это
pipeline-шаг (`aesthetics::interaction_arena`), драйвится дескриптором `act::interaction`. Ключ
контенции: **(interaction_id, target-entityid)** — `interaction_id` (= fn_id эффекта) выбирает НЕЗАВИСИМЫЙ
контест (свой elect-буфер), `target-entityid` — оспариваемый слот внутри него; self-claim ключуется по
**claimant-entityid** (флаг «инициатор в этот тик»). Т.е. основная ось блокировки — entityid (ресурс),
партиционированная по fn_id (тип взаимодействия). Держать act ECS-агностичным ⇒ упаковщик про арену не
знает. **Out-of-scope:** если через handle/аргументы в функцию приходит система, ВНЕШНЯЯ по отношению к
ECS (глобальный сервис, аудио-микшер, кэш ресурсов), pipeline-локер её НЕ арбитрирует — он сериализует
только контенцию ECS-сущностей (elect по entityid); потокобезопасность внешней системы — забота самой
функции/системы (или однопоточный commit).
