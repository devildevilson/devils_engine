...
Ресурс объявляет, сколько у него уровней: `mips = 4` либо `mips = auto` (полная цепочка до 1x1, число уровней считается от размера уровня 0 и пересчитывается при resize). Это свойство РЕСУРСА, потому что определяет аллокацию; «какой уровень мне нужен» — свойство биндинга: `mip = 2`, а отсутствие поля означает всю цепочку.

Правило вытекает из Vulkan, а не из вкуса: у `imageLoad`/`imageStore` нет параметра LOD, поэтому storage-вид покрывает ровно один уровень. Значит `texel_write`, `texel_read`, `general` и вложения ОБЯЗАНЫ называть уровень (иначе loud error), а `sampled` без уровня даёт вид на цепочку, который шейдер читает через `textureLod`. Для трилинейной выборки сэмплеру нужен `mipmap = linear`; диапазон LOD снят (`maxLod = VK_LOD_CLAMP_NONE`), иначе выборка любого уровня выше нулевого молча возвращала бы нулевой.

Каждой копии ресурса создаются вид на всю цепочку и по виду на каждый уровень. Layout отслеживается ПО УРОВНЯМ (`resource_inst::usage_levels`): цепочке понижений нужно, чтобы уровень `k` лежал в `sampled`, пока `k+1` пишется, и одним состоянием на ресурс это невыразимо. Проверка конфликта юсаджей в шаге тоже ведётся по паре (ресурс, уровень). Барьеры выводятся из биндингов: `mip = k` — на уровень, без `mip` — на все уровни; подряд идущие уровни в одинаковом состоянии склеиваются в один барьер, поэтому картинка без мипов стоит столько же, сколько стоила раньше.

Не поддержано пока: рисование в произвольный уровень (render target не умеет называть уровень — падает громко) и автоматическая генерация цепочки блитами.
...
- держать ДИСКОВЫЙ КЭШ SPIR-V (`shader_compiler::set_cache_directory`) рядом с кэшем пайплайнов.

**Два кэша закрывают разные половины.** Кэш пайплайнов снимает трансляцию SPIR-V в код устройства,
дисковый кэш SPIR-V — трансляцию GLSL в SPIR-V, и одного без другого мало. Главное в втором не
экономия на самой компиляции (`1`–`2.3` мс на шейдер), а то, что при полном попадании состояние
glslang НЕ ПОДНИМАЕТСЯ ВОВСЕ: конструктор `shaderc::Compiler` стоит около `90` мс независимо от того,
что компилируется. Замерено на GN04 (время до первого результата): `143` -> `37` мс, `3.9x`.

Ключ записи — текст, стадия, точка входа, дефайны, оптимизатор, debug info, корень включений.
Включённые файлы в ключ НЕ входят (их состав известен только после препроцессинга, то есть после
подъёма glslang), поэтому запись хранит их СПИСОК с хешем содержимого и сверяет при чтении: правка
включённого файла обязана быть промахом, иначе движок исполнит не тот шейдер, который написан.
Непригодный каталог ведёт себя как отсутствие кэша — кэш это ускорение, а не причина отказа собрать
шейдер.

**Компилятор передаётся снаружи** (`painter::shader_compiler`), а не живёт скрытым состоянием на
поток: владелец назван — `graphics_base` для потока графики, `compute_context` для вычислений, один
объект на пачку у системы ресурсов. `shader_crafter` остаётся одноразовой настройкой ОДНОЙ компиляции;
эти две жизни разной длины.
