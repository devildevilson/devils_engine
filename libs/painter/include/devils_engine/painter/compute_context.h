...
struct compute_context_config {
  ...
  // Каталог дискового кэша SPIR-V. Пусто => компиляция GLSL идёт каждый прогон. Отдельно от кэша
  // пайплайнов, потому что закрывают они РАЗНЫЕ половины: тот снимает трансляцию SPIR-V в код
  // устройства, этот — трансляцию GLSL в SPIR-V. У генератора вторая заметнее: шейдеры собираются
  // из очереди, и на холодном прогоне их компиляция это первое, что видно.
...
public:
  ...
  struct device_limits {
    ...
    // Предел стороны двумерного образа. Держится рядом с остальными по той же причине: на Iris Xe это
    // 16384, а миллион клеток и 4096x4096 проходят — то есть у автора картинка соберётся, а у игрока
    // с меньшим пределом нет, и узнать об этом надо ДО исполнения.
    uint32_t max_image_2d = 0;
  };
  ...
  public:
    ...
    // ПЕРЕДАЧА МЕЖДУ БУФЕРОМ И КАРТИНКОЙ. Нужна ровно на границе очереди: поле, которое кто-то читает
    // фильтром, лежит на устройстве картинкой, а на хосте оно по-прежнему плотный отрезок байт, и
    // приехать туда-обратно иначе не может. Внутри очереди эта передача не платится вовсе — картинка,
    // рождённая и умершая внутри, границы не пересекает.
    void copy_to_image(const buffer_id from, const image_id to);
    void copy_from_image(const image_id from, const buffer_id to);
