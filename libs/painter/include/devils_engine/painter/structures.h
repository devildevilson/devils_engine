...
struct resource {
  ...
  uint32_t usage_mask;
  // Число сэмплов на пиксель (MSAA). Свойство РЕСУРСА, потому что определяет аллокацию — как и mips. Один
  // сэмпл у обычной картинки; больше одного делает её многосэмпловой, а значит она может быть только
  // вложением и источником resolve: обычной выборкой её читает не sampler2D, а sampler2DMS, и решает это
  // шейдер, а не движок.
  uint32_t samples;
...
struct execution_pass_base {
  ...
  // Условный счётчик: пасс записывает свои команды только в кадрах, где счётчик СДВИНУЛСЯ с прошлого
  // исполнения. Объявляется ЯВНО, а не выводится из счётчиков ресурсов: «исполнение зависит от внешних
  // событий» — это то, что читатель конфига обязан видеть сразу, а не сверять по ресурсам. Двигает счётчик
  // хост (`inc_counter`), потому что условие инвалидации — знание хоста, а не GPU.
  // invalid_resource_slot => пасс исполняется каждый кадр.
  uint32_t counter;
