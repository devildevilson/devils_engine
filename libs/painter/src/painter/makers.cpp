...
void descriptor_set_updater::update() {
  for (size_t i = 0; i < infos.size(); ++i) {
    infos[i].descriptorCount = writes[i].type == 2 ? writes[i].bufferViews.size() : writes[i].type == 1 ? writes[i].buffers.size()
                                                                                                        : writes[i].images.size();
...
vk::Pipeline pipeline_maker::create(
  ...
  if (res != vk::Result::eSuccess) {
    utils::error{}("Could not create graphics pipeline '{}'", name);
  }
  if (!name.empty()) {
    set_name(device, p, name);
  }
...
device_maker& device_maker::createQueue(const uint32_t queue_family_index, const uint32_t maxCount, const float* priority) {
  const auto& props = phys.getQueueFamilyProperties();
  if (queue_family_index >= props.size()) {
    utils::error{}("Invalid queue family index {}, max is {}", queue_family_index, props.size());
  }
