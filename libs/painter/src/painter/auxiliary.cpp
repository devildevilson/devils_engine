...
void load_dispatcher1(const bool require_glfw_surface) {
  ...
  if (require_glfw_surface) {
    if (!input::vulkan_supported()) {
      utils::error{}("Vulkan is not supported by this GLFW context");
    }
  ...
  if (vkGetInstanceProcAddr == nullptr) {
    utils::error{}("Could not load vkGetInstanceProcAddr");
  }
...
void destroy_debug_messenger(VkInstance inst, VkDebugUtilsMessengerEXT handle) {
  ...
  if (VULKAN_HPP_DEFAULT_DISPATCHER.vkDestroyDebugUtilsMessengerEXT == nullptr) {
    return;
  }
...
VkSurfaceKHR create_surface(VkInstance instance, GLFWwindow* window) {
  ...
  if (res != VK_SUCCESS) {
    utils::error{}("Could not create surface from window '{}', result: {}", input::window_title(window), vk::to_string(vk::Result(res)));
  }
  return surf;
}
...
std::vector<const char*> check_instance_extension(std::vector<const char*> input) {
  ...
  for (const auto name : input) {
    if (std::find(exts.begin(), exts.end(), name) != exts.end()) {
      out.push_back(name);
    }
  }
  return out;
...
std::string format_to_string(const uint32_t format) {
  return vk::to_string(vk::Format(format));
}
...
bool format_is_int(uint32_t format) {
  return (format_is_sint(format) || format_is_uint(format));
}
...
bool format_has_depth(uint32_t format) {
  return (format_is_depth_only(format) || format_is_depth_and_stencil(format));
}
bool format_has_stencil(uint32_t format) {
  return (format_is_stencil_only(format) || format_is_depth_and_stencil(format));
}
bool format_is_multiplane(uint32_t format) {
  return ((format_plane_count(format)) > 1u);
}
bool format_is_color(uint32_t format) {
  return !(format_is_undef(format) || format_is_depth_or_stencil(format) || format_is_multiplane(format));
}
