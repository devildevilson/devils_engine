...
- Cardgame grouped combat slice (2026-07-22): `subprojects/cardgame` is the first live `libs/resolve` consumer and owns the pointer-free hierarchy `effect_program → beat → authored_effect/effect_ref → emitted effect_instance_ref → typed outcomes`. Every beat freezes all target snapshots before cue; authored cues share one gameplay barrier, scripts execute in stable order, and one aggregated result per authored effect carries its outcome range. Targeters are target/random/all; effects select independently by default, while an explicit nonzero binding key reuses one snapshot and mismatched reuse fails loudly. Death is checked after every damage/status outcome and gates only the next beat. Primary hits still use an owned frontier; elemental children and thorns retaliation retain resolve provenance. The serializable project cursor now runs the settled groups: ActionBegin/card execution → player/enemy party passes → source ActorStateTick → explicit countdown boundary → mirrored enemy execution/party passes/tick → ActionEnd; a stolen card produces `executed=false`, skips card/follow-ups, but still ticks its actor and advances countdown. Forced end-turn enemy executions receive their own monotonic action-cycle token and skip player ActionEnd. `execution_report` carries a category mask for one-shot enablers; party order is frozen from `(combat_seed, action_token, side_domain, stable entity id)`. The current native attack listener intentionally emits zero work and ActorStateTick only records its boundary; real rule programs and DoT/negative/positive work remain next. `cardgame_action_pipeline_test` covers normal/theft/mirrored/forced/resume order in addition to the effect/follow-up/headless tests.
- Cardgame typed effect envelope and routed damage (2026-07-23): attack/healing/agility-damage/status stores feed one semantic-order `outcome_ref {kind,index}` without a thick variant; sign never changes kind (`attack(-N)` remains damage, `healing(-N)` remains healing), and death is checked after every typed outcome. One unrouted damage root gathers modifiers/resistance exactly once into `damage_preparation`, then emits at most two bounded provenance children in fixed order: shield leaf followed by residual health leaf. Only leaves mutate stats/become outcomes; shield-only hits have no synthetic HP outcome, terminal shield/HP drives elemental continuation once, and thorns listens only to committed primary HP loss. Routed leaf IDs share the frontier allocator, presentation exposes `shield_damage`, and retaliation keeps its separate cue while returning all of its routed leaves. `cardgame_typed_effect_test` covers shield-only, vulnerability overflow without double resistance, signed attack/healing, self targeting, attribute resistance, report/death boundaries and animated/headless identity.
- Cardgame DS reaction contract (2026-07-22): retaliation is an immediate continuation of one committed instance inside the outer execution/report; follow-up is a fixed FSM-step execution over a frozen report with `opens_follow_up=false`. A retaliation always owns a nested authored attack cue/result/finished (no inline merge) and a full attack route, but receives retaliation lineage, no action/countdown/tick and no separate follow-up window. Cardgame DS is deliberately two-phase: a rule/program prepare scope runs before cue and materializes targets/authored-effect requests; an authored-effect emit scope runs after the gameplay marker and records typed instances only. `emit_*` never resolves synchronously, lifecycle channel is invocation-owned, and direct reflected `add_*` stat mutation is forbidden because it bypasses outcomes/death/presentation. Preserve the existing mutable `register_stats`; add a separate read-only registration entry point for combat scopes.
- Cardgame DS leaf/resource slice (2026-07-23): devils_script is pinned to v1.2.1. `act::register_stats_readonly` sits beside unchanged mutable `register_stats` (plus separable reader/writer registration). `combat_effect_scope` executes a compiled ds script against a transient writer; its `each_target` iterator traverses the frozen authored target snapshot in project order and installs one `combat_target_scope`. Each `emit_attack/healing/attribute_damage/status` call appends exactly one bounded pointer-free instance plus a semantic plan ref directly into `resolution_work`; target-major callback order is gameplay order, signed literals such as `emit_healing = -2` are native syntax, and attack root IDs remain unresolved until the resolver selects the plan item. `combat_effect_script_compiler` is the project adapter for generic `act::script_resource {ret=void,scope=combat_effect}`; an authored script recipe snapshots only the stable demiurg resource hash, while `combat_effect_script_resources` resolves the externally owned compiled container at invocation. The shipped `scripts/scripted_strike` is the first resource-backed card effect, and `cardgame_combat_script_test` resumes it from the pre-invocation presentation barrier with an identical typed trace.
