...
- Cardgame grouped effect program (2026-07-22): `subprojects/cardgame` is the first live `libs/resolve` consumer and now owns the pointer-free hierarchy `effect_program → beat → authored_effect/effect_ref → emitted effect_instance_ref → typed outcomes`. Every beat freezes all target snapshots before cue; authored cues share one gameplay barrier, scripts execute in stable order, and one aggregated result per authored effect carries its outcome range. Targeters are target/random/all; effects select independently by default, while an explicit nonzero binding key reuses one snapshot and mismatched reuse fails loudly. Death is checked after every damage/status outcome and gates only the next beat. Primary hits still use an owned frontier; elemental children and thorns retaliation retain resolve provenance. `execution_report` now carries a category mask for one-shot follow-up enablers, and `materialize_follow_up_order` freezes a container-independent, subset-stable pseudo-random party order from `(combat_seed, action_token, side_domain, stable entity id)` with loud duplicate/invalid-id rejection. `cardgame_effect_program_test`, `cardgame_follow_up_test` and `cardgame_headless_smoke` cover these contracts. The full grouped follow-up FSM/rule registry remains next: `execution → source-side party pass (including actor) → opposing-side party pass → source ActorStateTick`; multiple rules inside one actor retain ordinary project order.
