...
- tile_frontier: `food` and `actor` both spawn through the registry. Prefab TEXT comes from disk via
  owner-level `prefab::prefab_resource` (`prefab/*.tavl`; filtered in `simulation.cpp` → `brain_config.prefabs`);
