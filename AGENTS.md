...
- **Spatial audio A/B verdict (2026-08-12):** the archived `audio_spatial_lab` compared production miniaudio with direct OpenAL Soft on matched PCM, listener, attenuation, constant-radius orbits and front/up distance pulses. Front `-Z` and up `+Y` attenuation both work; the apparent OpenAL change on the vertical orbit was a very small direction-dependent coloration, not distance drift. Overall miniaudio sounds almost the same as the remembered historical OpenAL path (which likely did not enable HRTF), so miniaudio is final for current production. OpenAL adds only a small useful front/back coloration in this comparison; future `AUD-17` should prototype a subtle miniaudio-native behind/above/below coloration with strict gain/EQ bounds and an off switch. Full HRTF via Steam Audio is a distant `submarine_coop` pre-release evaluation, not current engine debt.
