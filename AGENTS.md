...
- Cardgame typed effect envelope (2026-07-22): attack/healing/agility-damage/status stores feed one semantic-order `outcome_ref {kind,index}` without a thick variant; sign never changes kind (`attack(-N)` remains damage, `healing(-N)` remains healing), and death is checked after every typed outcome. Healing has its own effectiveness/cap route and agility damage its own resistance/stat route. `cardgame_typed_effect_test` covers signed attack/healing, self targeting, attribute resistance, report ranges, death boundaries and animated/headless identity. Shield and residual HP are still one aggregate damage commit and MUST be split/audited before fixing the damage API, without applying resistance twice.
