...
- Cardgame DS leaf slice (2026-07-23): devils_script is pinned to v1.2.1. `act::register_stats_readonly` sits beside unchanged mutable `register_stats` (plus separable reader/writer registration). `combat_effect_scope` executes a real compiled ds script against a transient writer; its `each_target` effect iterator traverses the frozen authored target snapshot in project order and installs one `combat_target_scope`. Each `emit_attack/healing/attribute_damage` call appends exactly one bounded pointer-free instance plus a semantic plan ref directly into `resolution_work`; target-major callback order is gameplay order, signed integer literals such as `emit_healing = -2` are native syntax, and attack root IDs remain unresolved until the resolver selects the work.
