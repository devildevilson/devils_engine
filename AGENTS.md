...
- **Sound resource/task pinning (2026-08-12):** each `sound_resource` atomically publishes an immutable shared `resource_blob`. Normal Lua/tile producers take the pin before enqueueing `command_sound_play`; `resource2` and the copied `system2::task` retain it through queued/active/draining lifetime. `unload_warm()` only drops the resource's reference, so an in-flight command/task keeps the old bytes and id alive while new views become empty. The fallback handle resolution path is compatibility-only. `sound_system_test` covers unload while queued/active data remains readable.
- **Spatial audio A/B verdict (2026-08-12):** optional target `audio_spatial_lab` compares the production miniaudio `system2` path with direct OpenAL Soft using the same deterministic mono S16 PCM, listener frame and linear attenuation. Real-headphone listening found OpenAL HRTF on materially easier to localize; built-in miniaudio sounds close to OpenAL HRTF off and has almost no above/below distinction. This is not a lost project coordinate: miniaudio 0.11.25 preserves `Y` through its listener transform and distance calculation, but its default stereo SIDE_LEFT/RIGHT channel directions are `(-1,0,0)`/`(+1,0,0)`, so equal-radius `+Y/-Y` positions receive identical gains. The vertical orbit itself stays at radius 4; perceived OpenAL near/far changes there may be direction-dependent HRTF/panner coloration rather than distance error. The lab is now 28 seconds and follows both orbits with matched front `-Z` and up `+Y` pulses `4→10→1→4`, printing radius explicitly. `AUD-LAB-02` is the manual axis-parity pass. Steam Audio is deferred as likely overkill unless later requirements justify an external HRTF DSP.
