...
- Cardgame follow-up prepare slice (2026-07-23): generic `act::script_resource` now accepts `scope = follow_up_rule`. Its transient context exposes the invoking actor/selected target, aggregate input category checks/count and `each_execution`, which installs the existing read-only `execution_report_scope` for every work in the sealed action-report prefix. `emit_follow_up_attack` is the first bounded authored-output building block: it prepares beats/recipes only, then the ordinary resolution FSM owns target freeze, cue, leaf emission, outcomes and presentation. A completed follow-up `resolution_work` is appended to the still-open party segment; output is invisible to the current party pass but visible after seal to the opposing pass. The temporary native registry fixture is still one attack enabler per participant and looks up the optional `scripts/follow_up_attack`; real component/resource rule lists remain next. The fixture emits damage equal to input execution count, proving 4→5→6 visibility, fresh enemy reports and snapshot resume at a follow-up cue in `cardgame_combat_script_test`.
