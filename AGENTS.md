...
- `libs/simul`: shared app/runtime layer. It owns `interface`/`advancer`, `app_runtime<Traits>`, standard broker/messages/systems, jthread lifecycle and window/settings/loading/render/sound/assets helpers. `subprojects/tile_frontier` now supplies traits, project-local broker channels and gameplay policy.
