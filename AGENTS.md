...
- **`libs/sound`**: `.pcm` removed from `type_from_ext` (system2/resource2 can't decode PCM yet — play would've hard-errored). `command_sound_play` carries `type` (sound::type; `UINT32_MAX`→sfx); sound actor no longer hardcodes sfx.
...
- **`libs/catalogue`**: `demo.h` include guard fixed (was a copy-paste of `..._REGISTRY_H`).
...
- `libs/catalogue`: prototype recording/replay/RPC/audit layer around selected function calls. Not stable netcode and not the main persistence layer.
...
- `libs/flow`: documented future animation/visual-state subsystem sketch, but NOT a CMake target yet (`README.md` + `system.h` only). Do not treat it as active runtime API.
...
- Root `README.md` was replaced on 2026-07-05 with a technical map of `libs/` only. It is intentionally organized as separate sections per library: role, internal/external dependencies, current usage, and status. Keep future `tests/` documentation separate unless explicitly asked.
...
- `libs/flow` is documented but is NOT a CMake target yet: it has `README.md` + `system.h` only. `libs/bindings` has a CMake target but no local README, so its root README section is reconstructed from headers/sources.
...
- Current focused contract tests live in `tests/thread_general_test.cpp`, `tests/utils_contract_test.cpp`, and `tests/sound_system_test.cpp`. `thread_general_test` covers `atomic.h`, `atomic_pool.h`, `lock.h`, legacy `queue1`, and the new `thread::spsc_queue`; `utils_contract_test` covers `memory_pool`, `stack_allocator`, and `fixed_pool_mt` size/alignment checks; `sound_system_test` covers sound math/format helpers, task/resource defaults, playback-device enumeration, and a guarded `system2` construction smoke test.
