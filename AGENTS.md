...
- `tile_frontier` actor update phase timing now uses `libs/catalogue` instead of `utils::perf`: `actor_simulation.cpp` defines `catalogue_domains::actor_update_perf`, wraps `build_sense_tree`/`cognition`/`apply`/combined `integration+drives`/`resolve_eating`/`maintain_food`/`actor_batch::build` with `catalogue::domain<...>::fn_traits`, and installs a tiny `actor_update_perf_introspection` that logs elapsed microseconds in `exit()`. Private `actor_world_slice` member wrappers are declared inside `actor_world_slice::update()` so access control stays valid.
