...
- Config-loaded void actions are live: GOAP action objects accept optional `effect = <devils_script>` co-parsed into an owned container. `actor_simulation` registers an `act::script_function<void>` under the semantic action name, while `script_environment` exposes the same catalogue `fn_deferred_ptr` building blocks. Shipped `flee`, `chase`, and `think` use this path; structural/RNG-dependent actions remain direct typed effects until their extra-scope/value syntax is defined. `tile_frontier_config_effect_smoke` loads the shipped `goap/actor` resource and requires 1-vs-4 worker byte identity.
