...
- `libs/visage`: Lua/Nuklear UI runtime plus MSDF fonts and POD render-output buffers consumed by painter's `draw_ui` path.
  ...
  - **UI images (Stage 1, 2026-07-05):** `visage::image` (`image.h`) is a POD lua-facing handle — `{texture_id (bindless slot), w, h, region[4]}`, no nuklear/vulkan dep so the HOST builds it. `visage::system` registers (same seam as `push_font`/`style_*`, NOT in bindings which can't depend on visage): the `image` usertype, an `nk.placement` bitmask table (`fill`/`stretch`/`scale_ratio`(=fit)/`center`/`left`/`right`/`top`/`bottom`), and `nk.image(img [, placement] [, color])` — takes the widget slot via `nk_widget(&bounds,ctx)`, computes the target rect (`image_placement_rect`: fit-by-min-scale + align, default center), draws `nk_draw_image(&ctx->current->buffer, rect, &nkimg, color)`. This OVERRIDES the dead `bindings` `nk.image` stub. Render side unchanged — `gui_draw_mode::image` + `ui.frag` mode 2 already sample `tex[tex_id] * color`; `convert()` classifies a non-font non-zero texture as image. tile_frontier host: `app.image("name")` → `visage::image` from a loaded `gpu_texture_resource` (`gpu_index`+`width`/`height`) via an `image_by_name` map, `nil` until `usable()`; designed as the bridge to a future demiurg `require`. Constraint: image slot must be 0–7 (project-wide `tex[8]` clamp) and not collide with a font-atlas slot. Stage 2 (deferred): stencil effects (cooldown / 4-blend) via a mask texture, riding the `effect_arena`/`userdata` per-draw channel like SDF text effects.
